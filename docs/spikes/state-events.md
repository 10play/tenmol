...
|---|---|---|---|
| GL idle timer | `packages/engine/modules/pmg_qt/pymol_gl_widget.py:114-116`, rearmed at `:250` with `self._timer.start(20)` | **50 Hz** (20 ms, single-shot self-rearming); `start(0)` at `:207` after input events | `self.pymol.idle()` then `self.pymol.getRedisplay()`; if either is true → `self.update()` (schedules `paintGL`) |
| Feedback timer | `packages/engine/modules/pmg_qt/pymol_qt_gui.py:391-394` | **2 Hz** (`start(100)` once at startup, then `start(500)` at `:958` after every tick); `start(0)` at `:964` right after a typed command | `update_progress()` + `_get_feedback()` + `get_setting_updates()` |
...
```
packages/engine/modules/pmg_qt/pymol_qt_gui.py:944          self.cmd._get_feedback()
packages/engine/modules/pmg_qt/pymol_qt_gui.py:952          self.cmd.get_setting_updates()          # global
packages/engine/modules/pmg_tk/Setting.py:146               self.cmd.get_setting_updates(sele, state)  # per-object
packages/engine/modules/pmg_tk/skins/normal/__init__.py:489 self.cmd._get_feedback(self.cmd)
```
...
2. **`_get_feedback()` returns `None`, not `[]`, when it cannot get the lock**
   (`packages/engine/modules/pymol/internal.py:596-606`, `lock_attempt` = `acquire(blocking=0)`,
   `packages/engine/modules/pymol/locking.py:29-30`). Treating `None` as "no output" is silently correct today
...
3. **`get_setting_updates()` returns `[]` on a lock miss** (`packages/engine/modules/pymol/setting.py:440-447`),
...
4. **The feedback queue is unbounded.** `OrthoFeedbackIn` (`packages/engine/layer1/Ortho.cpp:492-499`) pushes into
...
1. **It is not a push at all — it is draw-pumped.** `p.idle()` delivers nothing. Only
   `cmd.refresh()` (`packages/engine/layer4/Cmd.cpp:4715-4731` → `SceneInvalidateCopy` + `ExecutiveDrawNow`) or
...
4. **Fatal: there is exactly one wizard stack and the user owns it.** Verified:
   ...
   (`packages/engine/layer1/Wizard.cpp:218-227`), so real wizards would not re-emit them either.
...
|---|---|---|---|
| **status** | `lock_api_status` / `lock_attempt` (non-blocking) | **yes** | `cmd.get_progress()` (`packages/engine/modules/pymol/monitoring.py:5-7`), `cmd._get_feedback()`, `cmd.get_setting_updates()` |
...
value/cost. **All of these touch upstream files and are owned by whoever owns `packages/engine/layer1/`,
...
performance. `SettingRec::setChanged()` (`packages/engine/layer1/Setting.h:67-70`) is the single write chokepoint
...
Add `uint64_t NamesVersion{1};` to `struct CExecutive` (`packages/engine/layer3/ExecutiveDef.h:54`) and
...
* **Do not add a Notify/observer bus.** The measured poll cost (67.7 µs at 30 Hz = 0.25 % core)
  ...
  `packages/engine/layer3/Executive.cpp:11533-11534`; the wizard header itself warns
  `event_mask_dirty = 128 # anything changed (BEWARE FEEDBACK!)`,
  `packages/engine/modules/pymol/wizard/__init__.py:13`).
...
6. Build/bootstrap owner: `pcatch._install()` must be called by the bridge at startup, mirroring
   `packages/engine/modules/pmg_qt/pymol_gl_widget.py:105`, or script `print()` output never reaches the client.

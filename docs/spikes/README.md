...
[`04-picking.md`](./04-picking.md) §2 and says so in a comment, and `scripts/bootstrap.sh` and
...
> [`07-cross-platform-gl.md`](./07-cross-platform-gl.md) §2.8, reproduced independently in §2.9,
...
| --- | --- | --- |
| [`00-build.md`](./00-build.md) | Does PyMOL build and import from this tree, headless, on macOS arm64? | **Yes.** `scripts/bootstrap.sh` is this recipe. §6.1 is **amended** (see its STATUS block). |
| [`02-feedback.md`](./02-feedback.md) | Can the PyMOL console be captured with no Qt and no GL? | **Yes, unchanged.** Both rules are implemented verbatim in `packages/bridge/tenmol_bridge/engine.py`. |
| [`03-geometry.md`](./03-geometry.md) | Is new C++ needed to get renderable geometry out? | **Yes** — and everything it measured about the *exporters* stands. Its §0/§9 "no server raster" conclusion is **overturned**; see its STATUS block. |
| [`04-picking.md`](./04-picking.md) | Can a headless backend pick? | **Yes, with a GL context.** §2/§3 is the CGL recipe `glcontext/cgl.py` and `scripts/doctor.mjs` both carry. |
| [`05-state-events.md`](./05-state-events.md) | Is polling fast enough to be the change feed? | **Yes** (0.25 % of a core at 30 Hz) — but its §8 C++ wish-list was **superseded** by spike 08, which built something different on purpose. |
| [`06-geometry-accessor.md`](./06-geometry-accessor.md) | What does `_cmd.web_get_rep_geometry` return? | **Yes** — §7 (the API) and §5 (the fidelity numbers) are the contract `packages/bridge/tenmol_bridge/render/modeg.py` consumes. §0 counts and §10 gaps have moved. |
| [`07-cross-platform-gl.md`](./07-cross-platform-gl.md) | Linux EGL? Windows WGL? | **Linux: verified. Windows: unverified.** See the box above. |
| [`08-native-changes.md`](./08-native-changes.md) | Exact invalidation, and pick data without a GL pick pass. | **Yes** — but `packages/engine/layer4/CmdWebGeometry.cpp` has grown since; see its STATUS block. |

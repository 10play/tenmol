...
| | |
|---|---|
| Files changed | `packages/engine/layer4/CmdWebGeometry.cpp` (+1012/−5), `packages/engine/layer3/Executive.cpp` (+69/−0), `packages/engine/layer3/ExecutiveDef.h` (+20/−0), `packages/engine/layer4/Cmd.cpp` (+4/−0) |
...
| `ExecutiveInvalidateRep` | :14053 | as the plan said — and it is reached from `packages/engine/layer1/Setting.cpp` (113 call sites), which is **why `SettingRec::setChanged()` is NOT patched** (§1.3) |
...
`u`11` is byte-for-byte what `packages/engine/layer1/SceneMouse.cpp:245` builds for a real backend pick
...
```
packages/engine/layer1/ScenePicking.cpp:15    #define cRange 7
packages/engine/layer1/ScenePicking.cpp:190   const int cRangeVal = DIP2PIXEL(cRange);
                              const int h = (cRangeVal * 2 + 1), w = (cRangeVal * 2 + 1);
packages/engine/layer1/ScenePicking.cpp:200   for (int d = 0; (d < cRangeVal); ++d)
                                for (int a = -d; a <= d; ++a)
                                  for (int b = -d; b <= d; ++b)
...
`SceneSetupGLPicking` does `glShadeModel(GL_FLAT)` (`packages/engine/layer1/Scene.cpp:5186`), the pick colour
...
* **`dashes` / `angles` / `dihedrals`** read the raw `float* V` of `RepDistDash` / `RepAngle` /
  ...
  exactly one `float*`, `packages/engine/layer0/vla.h:42`). The dash **pattern is already baked into `V`** by the
...
```
 ...
 packages/engine/layer4/Cmd.cpp            |    4 +
 packages/engine/layer4/CmdWebGeometry.cpp | 1012 +++++++++++++++++++++++++++++++++++++++++++
...
packages/engine/layer3/Executive.cpp   blocks=10 guarded_lines=64  unterminated=False
packages/engine/layer3/ExecutiveDef.h  blocks=1  guarded_lines=19  unterminated=False
packages/engine/layer4/Cmd.cpp         blocks=2  guarded_lines=10  unterminated=False
...
| mirror | upstream |
|---|---|
| `mirror::RepDistLines` | `packages/engine/layer2/RepDistDash.cpp:40-55`, `packages/engine/layer2/RepAngle.cpp:36-51`, `packages/engine/layer2/RepDihedral.cpp:35-48` |
...
$ packages/bridge/.venv/bin/pymol -ckq packages/engine/testing/testing.py --run all

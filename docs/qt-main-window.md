...
- `packages/engine/modules/pmg_qt/pymol_qt_gui.py` (1267 lines) — the `QMainWindow` shell, menu builder, command line, feedback area, quick buttons, app startup.
- `packages/engine/modules/pymol/_gui.py` (1033 lines) — **the actual menu-bar data model** (`PyMOLDesktopGUI.get_menudata`), command history, movie-program helpers, recent-files sqlite DB.
- `packages/engine/modules/pmg_qt/mimic_pmg_tk.py` (177 lines) — Pmw/Tk menu-bar shim so legacy plugins can add menu items.
- `packages/engine/modules/pmg_qt/mimic_tk.py` (128 lines) — `tkMessageBox`/`tkFileDialog` → Qt shim, injected into `sys.modules`.
- `packages/engine/modules/pmg_qt/__init__.py` (3 lines) — empty placeholder docstring only, no code.
- Supporting: `packages/engine/modules/pmg_qt/pymol_gl_widget.py`, `packages/engine/modules/pmg_qt/keymapping.py`, `packages/engine/modules/pymol/gui.py`, `packages/engine/modules/pymol/Qt/utils.py`, `packages/engine/modules/pymol/invocation.py`, `packages/engine/modules/pymol/save_shortcut.py`, `packages/engine/modules/pymol/colorprinting.py`, `packages/engine/modules/pymol/parser.py`, `packages/engine/data/pmg_qt/styles/pymol.sty`, `packages/engine/modules/pmg_qt/forms/*.ui`.
...
- The only other consumer of `get_menudata` is the legacy Tk skin at `packages/engine/modules/pmg_tk/skins/normal/__init__.py:1072` (grep result; not part of this area).
- `packages/engine/modules/pymol/menu.py` is imported by the **C++ layer**, at `packages/engine/layer1/P.cpp:2020` (`P_menu = PImportModuleOrFatal("pymol.menu")`), and drives the *viewport-internal* popup menus via `MenuActivate*` in `packages/engine/layer3/Executive.cpp` (e.g. `packages/engine/layer3/Executive.cpp:15019`, `:15069`, `:783`). It also exports `rep_setting_lists` consumed by `packages/engine/modules/pymol/viewing.py:1965`.

So: **menu bar = `packages/engine/modules/pymol/_gui.py`; internal-GUI right-click menus = `packages/engine/modules/pymol/menu.py` (a different agent's area).** Both must be cloned, but they are separate data sources with different serialization needs.
...
| Initial size = `win_x + (220 if internal_gui) , win_y + (246 if external_gui else 18)` | `pymol_qt_gui.py:94-97` | option defaults `win_x=640`, `win_y=480` (`packages/engine/modules/pymol/invocation.py:144-145`) |
...
| Stylesheet loaded from `$PYMOL_DATA/pmg_qt/styles/pymol.sty` | `pymol_qt_gui.py:407-416` | file is `packages/engine/data/pmg_qt/styles/pymol.sty`; only 3 rules: `#builder QPushButton` padding, `QPushButton[quickbutton=true]` padding, `QMainWindow::separator {width/height 5px}` |
| Window title tracks setting `session_file` (index 440) | `pymol_qt_gui.py:114-116`; setting id confirmed at `packages/engine/layer1/SettingInfo.h:540` | title becomes `PyMOL (basename)` |
...
### 3.2 History model (`packages/engine/modules/pymol/_gui.py:895-941`)
...
`complete()` (`_gui.py:899-904`) calls `self.cmd._parser.complete(self.command_get())`. The implementation is `Parser._complete` at `packages/engine/modules/pymol/parser.py:524-593`:
...
- Monospace font via `getMonospaceFont()` (`pymol_qt_gui.py:137`; impl `packages/engine/modules/pymol/Qt/utils.py:249-266` — Monaco/Consolas/Monospace by platform, size 9, +3 on macOS).
...
- `update_feedback` (`pymol_qt_gui.py:941-958`):
  1. `update_progress()`;
  2. `feedback = self.cmd._get_feedback()` (`packages/engine/modules/pymol/internal.py:593`) — drains the C++ feedback queue, returns a list of lines;
  3. `colorprinting.text2html(...)` (`packages/engine/modules/pymol/colorprinting.py:17` — escapes `&<>`, converts spaces to `&nbsp;` and newlines to `<br>`) then `browser.appendHtml(html)`;
  4. auto-scroll to `verticalScrollBar().maximum()`;
  5. `for setting in self.cmd.get_setting_updates()` (`packages/engine/modules/pymol/setting.py:440`) → for each registered index, read `cmd.get_setting_tuple(setting)[1][0]` and invoke every callback in `self.setting_callbacks[setting]`. **This is the entire mechanism that keeps checkable/radio menu items and the window title in sync with the backend.**
...
Progress row (`pymol_qt_gui.py:273-284`): `QProgressBar` + a red `Abort` `QPushButton` (`background: #FF0000; color: #FFFFFF`) wired to `cmd.interrupt` (`packages/engine/modules/pymol/locking.py:88`). `update_progress` (`pymol_qt_gui.py:931-939`) computes `int(cmd.get_progress() * 100)` (`packages/engine/modules/pymol/monitoring.py:5`) and shows/hides both widgets when the value is `>= 0` / `< 0`.
...
### 6.2 `File` — `packages/engine/modules/pymol/_gui.py:80-134`
...
| `Get PDB...` | command | `file_dialogs.file_fetch_pdb` (`packages/engine/modules/pmg_qt/file_dialogs.py:444`) |
| — separator — | | |
...
(`_gui.py:140-161`; backend `packages/engine/modules/pymol/editor.py:51 attach_fragment`.)

`Build ▸ Residue` (`_gui.py:162-195`) — 23 commands, each `cmd.editor.attach_amino_acid('pk1', <code>)` (`packages/engine/modules/pymol/editor.py:98`):
...
- `Quality ▸` `Maximum Performance`=`util.performance(100)`, `Reasonable Performance`=`util.performance(66)`, `Reasonable Quality`=`util.performance(33)`, `Maximum Quality`=`util.performance(0)` (`packages/engine/modules/pymol/util.py:571`).
...
Commands: `3 Button Motions`=`cmd.config_mouse('three_button_motions')`, `3 Button Editing`=`config_mouse('three_button_editing')`, `3 Button Viewing`=`cmd.mouse('three_button_viewing')`, `3 Button Lights`=`cmd.mouse('three_button_lights')`, `3 Button All Modes`=`config_mouse('three_button_all_modes')`, `2 Button Editing`=`config_mouse('two_button_editing')`, `2 Button Viewing`=`config_mouse('two_button')`, `1 Button Viewing Mode`=`cmd.mouse('one_button_viewing')`, `Emulate Maestro`=`cmd.mouse('three_button_maestro')`. (`cmd.config_mouse` at `packages/engine/modules/pymol/controlling.py:168`; `cmd.mouse` is marked INTERNAL at `controlling.py:609`.)
...
Backing store — **sqlite**, `packages/engine/modules/pymol/_gui.py:975-1032`:
...
3. `plugins.legacysupport.addPluginManagerMenuItem()` → adds `Plugin Manager` (`packages/engine/modules/pymol/plugins/legacysupport.py:72-87`), which opens the Qt `PluginManager` when `pymol.gui.get_qtwindow()` returns a window;
...
5. `plugins.HAVE_QT = True; plugins.initialize(app)` (`packages/engine/modules/pymol/plugins/__init__.py:408`).
...
`PmwMenuBar` (`mimic_pmg_tk.py:29-91`) is the Pmw-compatible façade plugins call through `pymol.plugins.addmenuitem` (`packages/engine/modules/pymol/plugins/__init__.py:112-129`) and `addmenuitemqt` (`:101-109`):
...
Tries `importlib.import_module('.forms.<name>', 'pmg_qt')` (pre-generated `Ui_Form`/`Ui_Dialog`), else falls back to `pymol.Qt.utils.loadUi(<dir>/forms/<name>.ui, widget)` (`packages/engine/modules/pymol/Qt/utils.py:269-320`). `dialog='floating'` wraps the widget in a floating `QDockWidget`. Available forms (`packages/engine/modules/pmg_qt/forms/`): `askpartial, change_confirm, colors, create_shortcut, fetch, help_shortcut, load_aln, load_mae, load_map, load_mtz, load_traj, movieexport, pluginitem, pluginmanager, png, props, render, save_molecule, save_object, shortcut_menu`.
...
### 9.6 Splash
There is **no Qt splash screen**. The splash is drawn inside the OpenGL viewport: `options.show_splash` (`packages/engine/modules/pymol/invocation.py:142`) causes `cmd.splash(1)` to be inserted as the first deferred command (`invocation.py:529-530`); the command is `packages/engine/modules/pymol/commanding.py:297`, and the C++ side consumes it via `OrthoInit(G, G->Option->show_splash)` (`packages/engine/layer5/PyMOL.cpp:1954`). `-q` clears it (`invocation.py:488`).
...
Reached from `cmd.window(action, x, y, w, h)` (`packages/engine/modules/pymol/viewing.py:1430-1457`) which resolves the action name through `window_dict` (`packages/engine/modules/pymol/constants.py:150-152`): `hide`0, `show`1, `position`2, `size`3, `box`4, `maximize`5, `fit`6, `focus`7, `defocus`8. `fit` clamps the frame geometry into `screen().availableGeometry()` and no-ops when Maximized/FullScreen.
...
4. `cmd.get_setting_updates()` being consume-once also means the internal GUI (`packages/engine/layer2/3` ortho widgets) and the web client compete for the same queue. Verify with the internal-GUI agent.
...
12. The stylesheet `packages/engine/data/pmg_qt/styles/pymol.sty` is nearly empty; almost all visual identity comes from the native Qt theme. The React shell has no design source of truth to copy — expect a design pass.

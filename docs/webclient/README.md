...
```
tenmol/
├─ layer0/ … layer5/  modules/  data/  ov/  setup.py   # upstream PyMOL — do not modify
├─ apps/web/                                           # @tenmol/web — the React app (Vite)
├─ packages/                                           # @tenmol/protocol, client, ui, viewport
├─ bridge/                                             # Python bridge process (PyMOL side)
├─ scripts/dev-bridge.sh                               # starts the bridge for `pnpm dev`
└─ docs/webclient/                                     # this directory
```
...
and looks for its interpreter in `$TENMOL_VENV`, then `bridge/.venv`, then `.venv`. Build PyMOL
...
- **The upstream PyMOL tree is never linted or formatted.** `eslint.config.js` ignores
  `layer0/`–`layer5/`, `modules/`, `data/`, `ov/`, `contrib/`, `examples/`, `include/`, `test/`,
  `testing/`, `build/`, `_custom_build/`, `docs/`; the Prettier scripts name their targets
...
From `02-completeness-critique.md`, still unresolved and owned by the bridge/viewport work:

...
* **YES**, PyMOL keeps a complete, CPU-resident, GL-free description of every representation's geometry.
  ...
  (`layer2/RepSurface.cpp:71-83`), and (b) **CGO** buffers — a flat `float*` bytecode with a documented
  opcode set — for every other rep (`layer1/CGO.h:82-270`, `layer1/CGO.h:765-773`).
* **NO**, none of that is exposed to Python today for molecular reps. The only Python-visible geometry
  ...
  (`modules/pymol/exporting.py:1012-1019`), and **all six of them route through the ray tracer**
  (`layer1/SceneRay.cpp:336-491`), not through the GL/CGO buffers. They are ASCII, per-primitive,
...
* The **exception** is surfaces: `cache_mode >= 2` already serializes the surface triangle mesh
  ...
  (`layer2/RepSurface.cpp:3217-3229`, `layer2/RepSurface.cpp:4559-4574`, `modules/pymol/internal.py:101`).
...
* The correct engineering answer is a **new pybind/CPython accessor** that walks
  `CoordSet::Rep[cRepCnt]` (`layer2/CoordSet.h:107`), runs the existing CPU tessellators
  `CGOSimplify()` (`layer1/CGO.cpp:4444`) + `CGOCombineBeginEnd()` (`layer1/CGO.cpp:1539`), and hands
  ...
  `PyBytes_FromStringAndSize` (`layer1/PConv.cpp:971-977`). That block is *literally* a three.js
  `BufferGeometry`.
...
PyMOL has three renderers behind one `Rep::render(RenderInfo*)` virtual (`layer1/Rep.h:198`):
...
The critical structural fact, verified in `layer2/RepSphere.cpp:149-207`: `renderCGO` is generated
**lazily inside `render()` and only when `G->HaveGUI && G->ValidContext`** (line 164). The
build-time artifact (`primitiveCGO`) is generated in `RepSphereNew` (`layer2/RepSphere.cpp:523-582`)
with **no GL involvement at all**.

Same shape for cartoon: `I->preshader` is built in `RepCartoonNew`
(`layer2/RepCartoon.cpp:4287-4300`) with no GL, then consumed and destroyed by
`RepCartoonCGOGenerate()` at first GL render (`layer2/RepCartoon.cpp:263-274`, dispose at `:240`).
Note the salvage path `RepCartoon::disposePreshaderCGO()` (`layer2/RepCartoon.cpp:83-89`): if
...
stack (`layer1/CGO.cpp:2947-2948`), fills it via `CGOProcessCGOtoArrays()` (`:2968`), uploads with
`SetVertexBufferData()` (`:2998`), and lets the vector die at scope exit. `layer1/CGO.h:183-186`
...
`SceneUpdate(G, force)` (`layer1/Scene.cpp:4664`) drives `obj->update()` for every object
(`layer1/Scene.cpp:4757-4760` single-threaded branch, `:4745-4751` threaded branch), which reaches
`Rep::update()` → `Rep::rebuild()` → `fNew(cs, state)` (`layer1/Rep.cpp:42-57, 65-101`).
**After `SceneUpdate` returns, every Rep's CPU geometry is final and no GL context was required.**
`cmd.rebuild()` (`modules/pymol/viewing.py:1837-1866` → `_cmd.rebuild`) and
`cmd.refresh()` (`modules/pymol/viewing.py:1750-1772`) are the Python triggers.

---

## 2. Per-Rep geometry inventory (what each Rep actually produces)

Enumerated from `enum cRep_t` (`layer1/Rep.h:48-74`). Reps live in `CoordSet::Rep[cRepCnt]` with an
`Active[cRepCnt]` flag array (`layer2/CoordSet.h:107-108`).

### cRepSurface (2) — `layer2/RepSurface.cpp:59-101`
...
`T` is produced by `TrianglePointsToSurface()` (`layer2/RepSurface.cpp:4035-4036`, declared
`layer0/Triangle.h:25-27`). Colors `VC`/`VA`/`RC` are (re)computed in `RepSurface::recolor()`
(`layer2/RepSurface.cpp:2363`). Per-triangle culling uses
`visibility_test(proximity, Vis, &T[3i])` (`layer2/RepSurface.cpp:209-216`) — the client MUST replicate
...
### cRepSphere (1) — `layer2/RepSphere.h:29-40`
`primitiveCGO` (`:38`) holds `CGO_SPHERE` ops (center xyz + radius) interleaved with `CGO_COLOR`,
`CGO_ALPHA`, `CGO_PICK_COLOR` (`layer2/RepSphere.cpp:290-313`). Built GL-free at
`layer2/RepSphere.cpp:523-582`. `spheroidCGO` (`:39`) is the anisotropic-spheroid variant built by
`RepSphereGeneratespheroidCGO()` (`layer2/RepSphere.cpp:366`) which emits real triangles.
GL path forks on `sphere_mode` (`layer2/RepSphereGenerate.cpp:24-102`):
- mode 9 (default) → GLSL impostor quads, `CGOOptimizeSpheresToVBONonIndexed` (`layer1/CGO.cpp:4357`)
- mode 0 / cube / tetrahedron → real triangles via `CGOSimplify(primitiveCGO, 0, sphere_quality)` (`layer2/RepSphereGenerate.cpp:36`)
- modes 1,2,3,6,7,8 → point sprites, `CGOConvertSpheresToPoints` (`layer2/RepSphereGenerate.cpp:75`)

### cRepCyl (0, sticks) — `layer2/RepCylBond.cpp:39-49`
`primitiveCGO` (`:47`) built GL-free (`layer2/RepCylBond.cpp:635` alloc, `:903` `CGOStop`). Contains
...
### cRepCartoon (5) — `layer2/RepCartoon.cpp:65-92`
`preshader` (`:77`) is the CPU CGO. Built by `GenerateRepCartoonCGO()`
(`layer2/RepCartoon.cpp:4287-4290`), then immediately normalized with `CGOCombineBeginEnd()`
(`layer2/RepCartoon.cpp:4292-4294`) — meaning **cartoon geometry is already in `CGO_DRAW_ARRAYS`
interleaved-array form before anything GL happens.** The actual extrusion emits
`GL_TRIANGLE_STRIP` / `GL_TRIANGLE_FAN` / `GL_LINE_STRIP` blocks in `layer1/Extrude.cpp`
(`:839, :932, :1036, :1129, :1363, :1450, :1483, :1569, :1631, :1671, :1793, :1912, :1987, :2047, :2128, :2153`)
plus `cgo::draw::shadercylinder2ndcolor` for nucleic-acid-as-cylinders (`layer1/Extrude.cpp:1219`),
plus `CGOSphere` for ring centers (`layer2/RepCartoon.cpp:1295`) and `GL_TRIANGLES` for rings (`:1346`).

### cRepRibbon (6) — `layer2/RepRibbon.cpp:37-50`: `primitiveCGO` (`:48`), CPU.
### cRepLine (7, wire) — `layer2/RepWireBond.cpp:34-45`: `primitiveCGO` (`:43`), CPU. `CGO_LINE`/`CGO_SPLITLINE`.
### cRepNonbonded (11) — `layer2/RepNonbonded.cpp:33-44`: `primitiveCGO` (`:41`), CPU.
### cRepNonbondedSphere (4) — `layer2/RepNonbondedSphere.cpp:35-44`: `primitiveCGO` (`:43`), CPU.
### cRepEllipsoid (19) — `layer2/RepEllipsoid.cpp:33-44`: `ray` (`:41`) and `std` (`:42`) CGOs. `CGO_ELLIPSOID` → `CGOSimpleEllipsoid` in `CGOSimplify` (`layer1/CGO.cpp:4535`).
### cRepMesh (8) — `layer2/RepMesh.cpp:39-63`: raw `pymol::vla<float> V` (`:51`), `N` strip lengths (`:49`), `NTot` (`:50`), `VC` colors (`:52`), `Dot`/`NDot` (`:53-54`). CPU, permanent.
### cRepDot (9) — `layer2/RepDot.h:26-45`: `V`, `VC`, `A` (area), `VN`, `T`, `F`, `Atom` raw float/int arrays (`:34-42`). CPU, permanent.
### cRepDash (10) — `layer2/RepDistDash.cpp:40-55`: raw `float* V` (`:48`), `N` (`:49`). CPU.
### cRepAngle (17) — `layer2/RepAngle.cpp:36-50`: `pymol::vla<float> V` (`:44`). CPU.
### cRepDihedral (18) — `layer2/RepDihedral.cpp:35-48`: `float* V` (`:43`). CPU.
### cRepLabel (3) — `layer2/RepLabel.cpp:90-104`: `labelV` vector + lexer indices `L` (`:98-99`).
Geometry is **texture-atlas quads**, not mesh. Glyph bitmaps are reachable CPU-side via
`CharacterGetPixmapBuffer()` (`layer1/Character.cpp:122-130`) and metrics via
`CharacterGetGeometry` / `CharacterGetWidth` / `CharacterGetHeight` / `CharacterGetAdvance`
(`layer1/Character.h:80-85`). Ops are `CGO_DRAW_LABEL` (`layer1/CGO.h:224-225`) and
`CGO_DRAW_LABELS` (`:228`).
### cRepDistLabel — `layer2/RepDistLabel.cpp:41-56`: same texture-quad story.
### cRepCGO (13) — user CGOs. `ObjectCGOState::origCGO` (`layer2/ObjectCGO.h:25`) is CPU and
**already round-trips to Python** — see §3.
### cRepCallback (14) — arbitrary Python `glDraw*` callbacks. **Not extractable, by definition.**
### cRepVolume (20) / cRepSlice (16) — 3D-texture ray-marching (`data/shaders/volume.fs`),
...
`class CGO` (`layer1/CGO.h:765-773`) is a `float* op` array of length `c`. Opcodes are
`#define`s at `layer1/CGO.h:82-270`, each with a fixed `_SZ` payload length. The Python-side mirror
of the opcode table already exists at `modules/pymol/cgo.py:21-79`.
...
`struct cgo::draw::arrays` (`layer1/CGO.h:338-355`) carries `{mode, arraybits, narrays, nverts}` plus
a heap block. `CGOCombineBeginEnd()` lays that block out at `layer1/CGO.cpp:1645-1672`:
...
Sizes: `VERTEX_POS_SIZE 3` / `VERTEX_COLOR_SIZE 4` (`layer0/ShaderMgr.h:430-431`),
`VERTEX_NORMAL_SIZE 3` (`layer1/CGO.cpp:55`), `VERTEX_PICKCOLOR_SIZE = 1 + 2` and
`VERTEX_ACCESSIBILITY_SIZE 1` (`layer1/CGO.cpp:60-65`). Array bit flags at `layer1/CGO.h:272-277`.
...
enumerated in `layer1/CGO.h:68-72` and mirrored in `modules/pymol/cgo.py:22-27`).

### The two CPU tessellators (no GL required)

* **`CGOSimplify(const CGO*, est, sphere_quality, stick_round_nub)`** — `layer1/CGO.cpp:4444`.
...
* **`CGOCombineBeginEnd(const CGO*, est, do_not_split_lines)`** — `layer1/CGO.cpp:1539`.
...
* `CGOAsPyList(CGO*)` (`layer1/CGO.cpp:289-297`) → `[len, flat_float_list]`, built by
  `CGOArrayAsPyList` (`layer1/CGO.cpp:241-287`). **It explicitly handles `CGO_DRAW_ARRAYS`**
...
* `CGONewFromPyList()` (`layer1/CGO.cpp:392`) is the inverse.
* Reached from Python only via session save of `ObjectCGO` (`layer2/ObjectCGO.cpp:43, 151`,
  `layer3/Executive.cpp:5407`), `GadgetSet` (`layer2/GadgetSet.cpp:191, 197`), and
  `CoordSet::SculptCGO` (`layer2/CoordSet.cpp:392-394`).
* Import side: `cmd.load_cgo(list_of_floats, name, state)` (`modules/pymol/importing.py:307-324`) →
  `ObjectCGOFromFloatArray` (`layer2/ObjectCGO.h:47`) / `CGOFromFloatArray` (`layer1/CGO.h:1109`).

**`cmd.get_session()` does NOT contain molecular rep geometry.** `CoordSetAsPyList`
(`layer2/CoordSet.cpp:364-416`) writes coords, atom index maps, settings, symmetry, and `SculptCGO` —
...
`dump_binary` is true (`layer1/PConv.cpp:971-977`). Used today by `pse_binary_dump`
(`layer2/CoordSet.cpp:371-372, 377`). This is exactly the zero-copy-ish primitive a binary geometry
accessor needs — no new serialization machinery to invent.

Zero-copy numpy precedent: `CoordSetAsNumPyArray(cs, copy)` (`layer2/CoordSet.cpp:326-361`) —
...
(`layer4/Cmd.cpp:2033-2056`, table entry `layer4/Cmd.cpp:6457`).

---

## 4. Complete inventory of existing Python-exposed export paths

Registered in `savefunctions` at `modules/pymol/exporting.py:988-1020`.

| Python API | C impl | SceneRay mode | Output | Fidelity | Verdict |
|---|---|---|---|---|---|
| `cmd.get_vrml(version=2)` `modules/pymol/querying.py:632-646` | `CmdGetVRML` `layer4/Cmd.cpp:2675-2693` | 4 (VRML2) or 6 (VRML1) `layer1/SceneRay.cpp:450-475` | ASCII `.wrl` string | **Best text export.** Triangles → `IndexedFaceSet` with per-vertex color AND per-vertex normal (`layer1/Ray.cpp:1350-1390`). Spheres → VRML `Sphere` node with diffuse color (`:1398-1415`). Cylinders/sausages → `Cylinder` node (`:1419-1430`). **`cPrimCone` = `/* TO DO */`, emits nothing (`layer1/Ray.cpp:1416-1418`).** Labels/ellipsoids dropped. Coordinates are **camera space** unless `geometry_export_mode=1`. `%6.4f`/`%8.6f` precision. | Unusable per-frame; usable one-shot |
| `cmd.get_collada(version=2)` `modules/pymol/querying.py:648-662` | `CmdGetCOLLADA` `layer4/Cmd.cpp:2698-2719` | 8 `layer1/SceneRay.cpp:483-489` | ASCII `.dae` XML | Sphere tessellated per-primitive with `sphere_quality` strips (`layer1/COLLADA.cpp:1053-1210`); cylinder/sausage/cone tessellated with `DAE_MAX_EDGE 50` (`:1215+`); triangles (`:1530`). **`cPrimCharacter` and `cPrimEllipsoid` are empty `break;` — dropped (`layer1/COLLADA.cpp:1603-1618`).** Emits **one `<geometry><mesh>` per primitive** — a 5k-atom sphere scene is 5k XML meshes. Node splitting at `XML_NODE_SIZE_LIMIT 1000000` chars (`layer1/COLLADA.cpp:42, 85-99, 1582`). Requires `_HAVE_LIBXML` (`setup.py:631-633`), else hard error (`layer1/COLLADA.cpp:681-684`). | Unusable |
| `cmd.get_gltf(filename)` `modules/pymol/querying.py:664-690` | *(pure Python)* | — | file | **Not a real glTF exporter.** It shells out: `shutil.which('collada2gltf')`, writes a `.dae`, then `subprocess.call([exe, '-i', f, '-o', f])` (`modules/pymol/querying.py:676-688`). Raises `CmdException('could not find collada2gltf')` if the binary is absent. Forces `collada_geometry_mode=1`. | Unusable |
| `cmd.get_povray()` `modules/pymol/querying.py:547-561` | `CmdGetPovRay` `layer4/Cmd.cpp:2742-2759` | 1 `layer1/SceneRay.cpp:391-412` | `(header, geom)` POV-SDL strings | Analytic POV primitives + `smooth_color_triangle`. Highest *rendering* fidelity, but it's a POV-Ray scene description, not mesh data. | Unusable |
| `cmd.get_idtf(quiet=1)` `modules/pymol/querying.py:563-582` | `CmdGetIdtf` `layer4/Cmd.cpp:2722-2740` | `cSceneRay_MODE_IDTF` = 7 (`layer1/Scene.h:39`), `layer1/SceneRay.cpp:476-482` | `(node, rsrc)` IDTF strings for U3D/PDF | **Triangles only.** `case cPrimSphere:`, `case cPrimCone:`, `case cPrimCylinder:`, `case cPrimSausage:` are all bare `break;` (`layer1/Ray.cpp:2333-2341`). Sticks and spheres silently vanish. | Unusable |
| `cmd.get_mtl_obj()` `modules/pymol/querying.py:585-601` | `CmdGetMtlObj` `layer4/Cmd.cpp:2761-2778` | 5 `layer1/SceneRay.cpp:458-467` | `(mtl, obj)` | Docstring itself says *"incomplete and unsupported"*. Triangles get `v`/`vn`/`f` (`layer1/Ray.cpp:2459-2491`); **spheres are written as three identical vertices forming a degenerate face** (`layer1/Ray.cpp:2492-2506`); **cylinders emit nothing**; **no colors at all** (the color `sprintf` is commented out, `layer1/Ray.cpp:2483-2489`). `.mtl` is never written — `_get_mtl_obj` raises `CmdException('.MTL export not implemented')` for `format=='mtl'` (`modules/pymol/exporting.py:981-986`). | Useless |
| `cmd.save('x.stl')` → `pymol.lazyio:get_stlstr` | — | — | — | **Raises `pymol.IncentiveOnlyException("STL export not supported by this PyMOL build")`** (`modules/pymol/lazyio.py:224-231`). Not available in this fork. | N/A |
| `cmd.get_session(...)` `modules/pymol/exporting.py:371-476` → `_cmd.get_session` (`layer4/Cmd.cpp:6491`) | — | — | dict | Atoms, coords, settings, `ObjectCGO` CGOs, gadgets. **No rep geometry** (`layer2/CoordSet.cpp:364-416`). | Not geometry |
| `cmd.get_coords(sel, state)` `layer4/Cmd.cpp:2009-2031` / `cmd.get_coordset(name, state, copy)` `:2033-2056` | — | — | numpy `(N,3)` float32/float64 | Atom coordinates only. `copy=0` gives a live **view** (`layer2/CoordSet.cpp:357`). | Not rep geometry, but the API-shape precedent |
| `cmd.png(...)` / `cmd.raw_image_callback` | `CmdPng`, `call_raw_image_callback` `layer1/Scene.cpp:4020-4051` | 0 | RGBA `numpy` uint8 `(W,H,4)` | Pixels, not geometry. Existing hook: set `cmd.raw_image_callback` (default `None`, `modules/pymol/cmd.py:384`) and PyMOL pushes every rendered frame to Python. | Pixel-streaming fallback |

**Shared cost model for all six geometry exporters.** Every one calls
`SceneRay(G, 0, 0, mode, …)` (`layer1/SceneRay.cpp:88`), which:
...
3. `RayExpandPrimitives()` (`layer1/Ray.cpp:561`) — allocates vertex/normal/radius VLAs sized to the
   full primitive count,
4. `RayTransformFirst()` (`layer1/Ray.cpp:822`) — transforms **all** vertices by `ModelView`
   (skipped only when `geometry_export_mode == 1`, `layer1/Ray.cpp:876-881`),
...
(`layer1/Basis.h:68-83`) and the header comment notes ~6.5M primitives/GB. Every call rebuilds
everything.
...
`cmd.cache('enable')` sets `cache_mode=2` (`modules/pymol/exporting.py:87-88`). Then, in
`RepSurfaceNew`, after `SurfaceJobRun()` succeeds and `cache_mode > 1`, PyMOL calls
`SurfaceJobResultAsTuple()` and `PCacheSet()` (`layer2/RepSurface.cpp:4566-4574`).

`SurfaceJobResultAsTuple` (`layer2/RepSurface.cpp:3217-3229`) builds:

```python
(N, V, VN, NT, T, S)   # V,VN: flat lists of floats; T,S: flat lists of ints
```

`PCacheSet` (`layer1/P.cpp:1349-1376`) stores it as `entry[3]` on a list
`[size, hash_list, input_tuple, output_tuple, access_count, timestamp]` appended to
`pymol._cache` by `cmd._cache_set` (`modules/pymol/internal.py:101-125`). It is directly readable:
...
**Cost:** `PConvToPyObject(std::vector<float>)` (`layer1/PConv.h:251-259`) builds a `PyList` of boxed
...
(`modules/pymol/viewing.py:634-679`, 18 floats: 3x3 rotation, camera-space origin, model-space origin,
...
(`layer4/Cmd.cpp:6446-6514`). Follow the exact shape of `CmdGetCoordSetAsNumPy`
(`layer4/Cmd.cpp:2033-2056`): `API_SETUP_ARGS` → `APIEnterBlocked` → `ExecutiveGetCoordSet`
(`layer3/Executive.h:869`) → build result → `APIExitBlocked`.

Resolution path (all already public):
```
ExecutiveFindObject<ObjectMolecule>(G, name)   layer3/Executive.h:338-343
  → ObjectMolecule::CSet[state]                (CoordSet*)
    → cs->Rep[cRepSurface] / cs->Active[...]   layer2/CoordSet.h:107-108
```
Preconditions: call `SceneUpdate(G, true)` (`layer1/Scene.cpp:4664`) first, or require the caller to
...
CGO* simple = CGOSimplify(src, 0, sphere_quality, stick_round_nub);   // layer1/CGO.cpp:4444
// 2. fold BEGIN/END into interleaved arrays (idempotent if already folded)
if (simple->has_begin_end) CGOCombineBeginEnd(&simple);              // layer1/CGO.cpp:1539

// 3. walk with the public iterator and emit PyBytes per CGO_DRAW_ARRAYS
for (auto it = simple->begin(); !it.is_stop(); ++it) {               // layer1/CGO.h:800-830
  if (it.op_code() == CGO_DRAW_ARRAYS) {
    auto* sp = it.cast<cgo::draw::arrays>();                         // layer1/CGO.h:338
    PyObject* blob = PConvFloatArrayToPyList(                        // layer1/PConv.cpp:971
...
`CGOOptimizeSpheresToVBONonIndexed` (`layer1/CGO.cpp:4357-4394`) does for GL.

### 6d. Bulk / streaming variant

Add `_cmd.get_scene_geometry(G, flags)` that iterates `G->Scene->Obj` the same way `SceneRay` does
(`layer1/SceneRay.cpp:284-323`) and returns a list of the per-rep dicts above, plus per-object
transforms from `cmd.get_object_matrix` (`layer4/Cmd.cpp:6480`) — required because
`ObjectMolecule::render` applies `ObjectStatePushAndApplyMatrix` when `matrix_mode != 0`
(`layer2/ObjectMolecule.cpp:11265-11269`).

### 6e. Invalidation events

The bridge must know when to re-pull. `Rep::invalidate(cRepInv_t)` (`layer1/Rep.h:199`) and the
`cRepInv_t` ladder (`layer1/Rep.h:133-184`) are the natural hook:
...
`call_raw_image_callback` at `layer1/Scene.cpp:4020-4051`).
...
* CGO accessor (CGOSimplify + iterate + PyBytes): **~300 lines**, mostly a variant of the already-existing
  `CGOArrayAsPyList` (`layer1/CGO.cpp:241-287`).
* Invalidation callback: **~80 lines**, plus threading care (`Rep::update()` runs on worker threads
  when `async_builds` is on, `layer1/Scene.cpp:4740-4757` — the callback must not touch Python from a
  non-GIL thread; use `PAutoBlock`/`PAutoUnblock` as `RepSurface` already does at
  `layer2/RepSurface.cpp:4569-4581`).

---

## 7. Shader parity assets already in the repo

`data/shaders/` contains PyMOL's GLSL, and the preprocessor already has WebGL/ES2 code paths:
`m_shaderPreprocessor.setVar("PURE_OPENGL_ES_2"/"PYMOL_WEBGL"/"PYMOL_WEBGL_IOS", true)`
under `#ifdef PURE_OPENGL_ES_2` (`layer0/ShaderMgr.cpp:604-607`), with
`data/shaders/webgl_header.vs` and `data/shaders/webgl_header.fs` supplying the uniform block
(`g_NormalMatrix`, `g_ModelViewMatrix`, `g_ProjectionMatrix`, `g_Fog_end`, `g_Fog_scale`).

Programs registered (`layer0/ShaderMgr.cpp:617-658`): `bg`, `label`, `default`, `surface`, `line`,
...
`GL_EXT_frag_depth` on WebGL1, native on WebGL2 (`data/shaders/sphere.fs:1-3, 55-60`).
...
1. **Ray tracing (`cmd.ray`) — irreproducible in three.js.** `RayRender` (`layer1/Ray.cpp`, 7827 lines)
   is a CPU ray tracer with real shadows, multi-light specular, interior colors, and
   `ray_trace_mode` cel-shading/outlines (`layer1/Ray.cpp:5570-5607, 6472-6474`). `ray_trace_mode`
   exists **only** in the ray tracer — grep of `layer1/SceneRay.cpp` and `layer1/SceneRender.cpp`
...
2. **Order-independent transparency (`transparency_mode 3`).** PyMOL binds an OIT accumulation
   ...
   (`layer1/SceneRender.cpp:939-995`, `data/shaders/oit.vs/.fs`, `layer0/PostProcess.cpp:60, 86`,
   `TM3_IS_ONEBUF` handling at `layer1/SceneRender.cpp:979, 1101, 1466`). WebGL2 has no
...
3. **CPU depth-sorted alpha triangles (`transparency_mode != 3`).** The other transparency path
   ...
   arrays into the CGO (`layer1/CGO.cpp:3395-3405`), and `RepSurface::render` allocates
   `t_buf`/`z_value`/`ix` sized `NT` and re-sorts every frame
   (`layer2/RepSurface.cpp:1969-1984`). **Reproducing this in JS means per-frame triangle sorting of
...
4. **Stereo.** PyMOL implements 9 stereo modes: quadbuffer, crosseye, walleye, geowall, sidebyside,
   ...
   (`layer1/SceneRay.cpp:154-175, 515-713`). Quad-buffer stereo and stencil-parity stereo are
...
5. **Post-process antialiasing.** `antialias_shader` selects FXAA (1 stage) or **SMAA (3 stages)**
   (`layer1/SceneRender.cpp:613-627, 1791`). Reproducible with effort, but PyMOL's exact SMAA
...
6. **Labels.** `cRepLabel` is texture-atlas quads driven by `CGO_DRAW_LABEL` / `CGO_DRAW_LABELS`
   (`layer1/CGO.h:224-228`) and a texture-size heuristic
   (`InvalidateShaderCGOIfTextureNeedsUpdate`, `layer2/RepLabel.cpp:117`,
   `MAX_LABEL_TEXTURE_SIZE 256`, `:114`). Glyphs are CPU-reachable
   (`layer1/Character.cpp:122`) but the atlas must be rebuilt client-side. Font rasterization will
...
7. **`cRepCallback` (14).** Objects created by `cmd.load_callback` execute arbitrary Python that
   issues raw OpenGL. `layer2/ObjectCallback.cpp` exists for exactly this. **There is no geometry to
   extract — by construction.** Any plugin using it is unsupported in the web client.

8. **`cRepVolume` (20) and `cRepSlice` (16).** 3D-texture ray marching (`data/shaders/volume.fs`,
   47 lines). Not mesh geometry. Requires a separate 3D-texture upload path
   (`cmd.get_volume_field` exists, `layer4/Cmd.cpp:6510`, `layer4/Cmd.cpp:727` →
...
9. **Ambient occlusion.** `VAO` per-vertex occlusion is computed on the surface
   (`layer2/RepSurface.cpp:2780-2800`) and shipped as a vertex attribute — that part transfers fine.
   But `ambient_occlusion_mode` also has a smoothing pass with its own map
   (`ambient_occlusion_map`, `layer2/RepSurface.cpp:2368`) whose result is baked into `VAO`, so as
...
10. **Grid mode.** `GridMode::ByObject / ByObjectStates / ByObjectByState`
    (`layer1/SceneRay.cpp:179-211, 306-320`) splits the viewport into N sub-viewports with adjusted
...
11. **Precision.** All PyMOL geometry is `float32` and all colors are `float32` RGB in [0,1]
    (`CLIP_COLOR_VALUE` / `CONVERT_COLOR_VALUE`, `layer1/CGO.h:54-55`). Optional byte-packed
    normals/colors are gated on `cgo_shader_ub_color` / `cgo_shader_ub_normal`
    (`layer1/CGO.cpp:2939-2961`, `CLIP_NORMAL_VALUE` at `layer1/CGO.h:58-61`). If the accessor emits
...
12. **Picking.** PyMOL picks by rendering a pick-color buffer (`CGORenderPicking`,
    `layer2/RepSphere.cpp:120`, `layer1/ScenePicking.cpp`). The `CGO_PICK_COLOR` payload
    (`atom index, bond index`, `layer1/CGO.h:141-142`) is in the extracted arrays, so client-side
    GPU picking is achievable — but the encoding (`Picking.h`, `cPickableAtom = -1` …
    `cPickableThrough = -5`, `modules/pymol/cgo.py:73-77`) must be replicated bit-for-bit or
...
* I found **no existing Python hook fired on Rep rebuild/invalidation**. Grepped `layer1/P.cpp`,
  `layer4/Cmd.cpp`, `modules/pymol/*.py`. If one exists under a name I did not guess, it would remove
  the need for §6e.
* I did **not** verify whether `RepSurface::AT` is populated in all surface modes or only when
  `pick_surface` is on. `AT` is declared at `layer2/RepSurface.cpp:83` but I did not trace every
  write site.
* I did **not** confirm the exact byte layout of the pick-color sub-block inside `CGO_DRAW_ARRAYS`
  ...
  `layer1/CGO.cpp:60-64`). The offset arithmetic at `layer1/CGO.cpp:1664-1666`
...
* `_PYMOL_NO_RAY` guards `SceneRay` entirely (`layer1/SceneRay.cpp:94-96`). I did **not** find a
...
* Whether `pymol._cache` surface entries survive `cmd.delete` / rebuild cycles cleanly enough to be a
  reliable feed — `_cache_purge` (`modules/pymol/internal.py:48-78`) evicts by size and access time.

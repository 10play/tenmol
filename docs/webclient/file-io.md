...
1. Every load/save path in PyMOL is a **real server-side path string**. `cmd.load`
   calls `_self.exp_path(filename)` (`modules/pymol/importing.py:751`) which expands
   `~` and `$VARS` (`modules/pymol/cmd.py:112`), then hands the *path* to C
   (`modules/pymol/internal.py:362`). `cmd.save` does the same
   (`modules/pymol/exporting.py:836-838`). Feeding it browser `File` blobs would require
   rewriting every loader.
2. Format dispatch is done on **filename/extension**, not content
   (`modules/pymol/importing.py:41-109`, `modules/pymol/exporting.py:836-844`). A browser
...
3. Several operations *cannot* be expressed as a download at all: `mpng` writes N
   numbered files (`modules/pymol/moving.py:366`), `movie.produce` creates a `.tmp`
   sibling directory and shells out to `ffmpeg`/`mpeg_encode`/`convert`
   (`modules/pymol/movie.py:770-800, 946-985`), `log_open` holds an open file handle for
   the whole session (`modules/pymol/commanding.py:107-155`), `fetch` writes into
   `fetch_path` (`modules/pymol/importing.py:1213-1248`), and `cd`/`system`/`run`
   operate on the server process (`modules/pymol/externing.py:32,112`,
   `modules/pymol/parsing.py:427`).
4. The GUI already has a notion of a sticky "current directory": `initialdir`
   (`modules/pmg_qt/pymol_qt_gui.py:496-506`) which defaults to `os.getcwd()` and is
...
| Browse for output file | Same component in "save" mode: directory tree + filename field + extension-filter dropdown that mirrors the Qt `filter` strings, plus the `getSaveFileNameWithExt` auto-extension rule (`modules/pymol/Qt/utils.py:229-246`). |
...
| Working directory | `cmd.cd` / `cmd.pwd` (`modules/pymol/externing.py:32,56`) are the source of truth; React shows it in a status bar and the picker starts there. "File Browser" menu item (`modules/pymol/_gui.py:121`) becomes "reveal in picker", not `open .`. |
| Plugin dialogs | Plugins import `tkinter.filedialog`, which is shimmed to Qt (`modules/pmg_qt/mimic_tk.py:36-90,108`). The web bridge must provide the same *blocking* shim backed by a round-trip to the React picker, or every legacy plugin breaks. |

---

## 1. Menu inventory (File menu) — must be reproduced 1:1

Source: `modules/pymol/_gui.py:80-133` (toolkit-independent menu data, consumed by
`modules/pmg_qt/pymol_qt_gui.py:295-357`).
...
(`modules/pmg_qt/pymol_qt_gui.py:387-388`). Note these are *extra* Qt shortcuts, not
...
### 2.1 `initialdir`
`modules/pmg_qt/pymol_qt_gui.py:496-506`. Falls back to `os.getcwd()` until the first
...
### 2.2 Recent files
`modules/pymol/_gui.py:975-1032`. SQLite DB at `~/.pymol/recent.db`, table
...
### 2.3 Drag & drop
- Onto the 3D viewport: `modules/pmg_qt/pymol_gl_widget.py:256-270` — accepts URLs,
  local files via `toLocalFile()`, remote URLs passed through as strings, each goes to
  `gui.load_dialog(url)`.
- Onto the command line: `modules/pmg_qt/pymol_qt_gui.py:1092-1119` — inserts the path
...
`file_read` (`modules/pymol/internal.py:279-297`). The Finder/new-window behaviour has
no web analogue (see risks).

### 2.4 Qt form loading (`load_form`)
`modules/pmg_qt/pymol_qt_gui.py:512-546` loads `pmg_qt/forms/<name>.ui|.py`. Every
`.ui` under `modules/pmg_qt/forms/` that this area owns is enumerated in §3/§4:
...
`getSaveFileNameWithExt` (`modules/pymol/Qt/utils.py:229-246`): if the typed basename has
no `.`, append the first `*.ext` from the selected filter. Reproduce exactly in the React
save picker.

### 2.6 Error surfacing
`PopupOnException` (`modules/pymol/Qt/utils.py:323-348`) wraps `file_save`,
...
`modules/pmg_qt/file_dialogs.py:33-77`:
...
`filename_to_format` (`modules/pymol/importing.py:41-109`) returns
...
### 3.4 `cmd.load` core
`modules/pymol/importing.py:643-827`. Signature
...
(`modules/pymol/internal.py:346-386`) and thence to C.
...
| `cmd.load_png` | `modules/pymol/viewing.py:1814-1834` | loads a PNG as the displayed image (movie overlay) |
...
Header parsed by `pymol.headering.MTZHeader` (`modules/pymol/headering.py:132-262`);
...
Legacy Tk twin: `modules/pmg_tk/PyMOLMapLoad.py` — a `Pmw.Dialog` with OK/Cancel/Help,
...
(`:281-283`, impl `modules/pymol/creating.py:176-274`), then builds a representation
...
### 4.1 `cmd.save` core
`modules/pymol/exporting.py:784-935`. Signature
...
(`modules/pymol/internal.py:272-274`, monkey-patched at `pymol_qt_gui.py:1242`), also
...
`modules/pymol/movie.py:824-844`). Preselect logic for the three menu entries at
...
`cmd.mpng` (`modules/pymol/moving.py:366-434`, internal `_mpng`
`internal.py:243-267`): writes `<prefix>NNNN.png` (or `.ppm`), strips an existing numeric
suffix, `first`/`last`/`preserve`/`mode`/`modal`.
`movie.produce` (`modules/pymol/movie.py:846-1000`): creates `<basename>.tmp/`, renders
...
- **`set_session`** (`importing.py:138-183`): accepts bytes (zlib+pickle) or dict,
  ...
  `modules/pymol/wizard/security.py:15-43` with accept/decline/mdump).
...
(`AsyncFunc`, `modules/pymol/Qt/utils.py:100-125`):
...
`layer1/Setting.cpp:644`). `async_<0` ⇒ async when interactive (`:1390-1391`).
...
| Plugin `tkinter.filedialog` calls | Bridge-side shim that blocks the calling thread while React shows the picker (same contract as `mimic_tk._qtFileDialog`, `modules/pmg_qt/mimic_tk.py:36-90`) |
...
(`modules/pymol/pymolhttpd.py:441-520`) already serves a document root and JSON-wrapped

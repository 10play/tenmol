...
1. **States vs. frames.** Objects have *states* (coordinate sets). The viewer has *frames*.
   ...
   (`modules/pymol/moving.py:691`, `layer1/Movie.cpp:979` `MovieFrameToIndex`).
...
3. **Scenes.** Named snapshots of view + colors + reps + enabled-state + frame + message
   + a PNG thumbnail (`layer3/MovieScene.h:73`, `layer3/MovieScene.cpp:173`).
4. **Sequence viewer ("Seeker").** An in-viewport text grid built in C++
   (`layer3/Seeker.cpp:969` `SeekerUpdate`) and drawn as GL text
   (`layer1/Seq.cpp:259` `CSeq::draw`). Selection interaction mutates the *active selection*
   via generated `cmd.select(...)` strings (`layer3/Seeker.cpp:169`, `:70`).

Two of these (movie panel, sequence viewer) are today **Ortho blocks drawn inside the GL
viewport** (`layer1/Movie.cpp:1741`, `layer1/Seq.cpp:259`, `layer1/Control.cpp:534`), plus
**scene buttons** drawn as an overlay on the Scene block
(`layer1/Scene.cpp:2885` `SceneDrawButtons`, gated by `scene_buttons`,
`layer1/Scene.cpp:3456`). All three must become React components.
...
- `cmd.ray` → `_cmd.render` (`modules/pymol/viewing.py:1581`) writes into `G->Scene->Image`.
- `cmd.draw` → `_cmd.draw` needs a live GL context (`modules/pymol/viewing.py:1652`,
  wrapped in `_self._call_with_opengl_context`).
- `cmd.mpng` / `movie.produce` render every frame to PNG on disk
  (`modules/pymol/moving.py:366`, `layer1/Movie.cpp:626` `MovieModalPNG`,
  `modules/pymol/movie.py:846`).
- Scene **thumbnails** are produced by `SceneDeferImage` into a 220×124 PNG buffer
  (`layer3/MovieScene.cpp:225-233`, dims at `layer3/MovieScene.h:97-99`).
...
(`modules/pymol/viewing.py:1630-1632`) — headless deployments must force `ray`.

Second contradiction: **movie playback is driven by the backend idle loop**
(`layer1/Scene.cpp:2432` `SceneIdle`, frame pacing at `:2453-2480`, rock at `:2477-2484`).
...
| cmd | source | returns |
|---|---|---|
| `cmd.get_frame()` | `modules/pymol/moving.py:984` | 1-based current frame; **no lock taken** |
| `cmd.get_state()` | `modules/pymol/moving.py:958` | 1-based current state; **no lock taken** |
| `cmd.count_frames()` | `modules/pymol/querying.py:759` | frames defined for the movie |
| `cmd.get_movie_length(images=-1)` | `modules/pymol/querying.py:730` | frames *explicitly* defined by `mset`; negative internal value is folded per `images` arg (`:746-753`) |
| `cmd.count_states(selection)` | `modules/pymol/querying.py:702` | states in selection |
| `cmd.get_movie_playing()` | `modules/pymol/moving.py:64` | bool |
| `cmd.get_movie_locked()` | `modules/pymol/querying.py:814` | bool (movie commands suppressed) |

`MoviePlaying()` returns true also while movie commands are being evaluated
(`layer1/Movie.cpp:540-551`), and returns false when `I->Locked`.

### 1.2 `SceneSetFrame(G, mode, frame)` — the whole navigation vocabulary

`layer1/Scene.cpp:2121-2184`. Modes:

| mode | meaning | Python entry |
|---|---|---|
| -1 | movie/frame override — go to this **state** absolutely | — |
| 0 | absolute frame | `cmd.set_frame(frame, 0)` (`modules/pymol/moving.py:898`) |
| 1 | relative frame | used internally after `mview` (`layer1/Movie.cpp:1372`) |
| 2 | end | — |
| 3 | middle + auto movie command | `cmd.middle()` (`modules/pymol/moving.py:934`) |
| 4 | absolute + auto movie command | `cmd.rewind()` = `set_frame(4,0)` (`modules/pymol/moving.py:874`) |
| 5 | relative + auto movie command | `cmd.forward()` = `(5,+1)` (`:819`), `cmd.backward()` = `(5,-1)` (`:846`) |
| 6 | end + auto movie command | `cmd.ending()` (`modules/pymol/moving.py:911`) |
| 7 | absolute + **forced** movie command | scrollbar drag (`layer1/Movie.cpp:1535`, `:1783`) |
| 8 | relative + forced movie command | — |
| 9 | end + forced movie command | — |
| 10 | seek forward to the frame carrying the current scene | `MovieSeekScene` (`layer1/Movie.cpp:1008`), used by scene recall while playing (`layer3/MovieScene.cpp:408-409`) |

`cmd.frame(n, trigger=-1, scene=0)` is a separate entry (`modules/pymol/moving.py:460`)
calling `_cmd.frame(COb, n-1, trigger)`.

Side effects of a frame change (`layer1/Scene.cpp:2185-2210`): clamp to `[0, NFrame)`,
...
`SceneIdle` (`layer1/Scene.cpp:2432`):
- FPS from `movie_fps`; `fps <= 0` → use `movie_delay` ms; `fps < 0` → full speed
  (`:2453-2464`; same logic duplicated in `SceneGetFPS` `layer1/Scene.cpp:342`).
...
`MoviePlay` (`layer1/Movie.cpp:555`): `cMovieToggle=-1`, `cMovieStop=0`, `cMoviePlay=1`
(`layer1/Movie.h:118-120`). When not looping and already at the last frame, play/toggle
...
`cmd.mset(specification, frame=1, freeze=0)` — `modules/pymol/moving.py:691`.
...
`cmd.madd` (`modules/pymol/moving.py:677`) is literally `mset(..., frame, freeze)` — it
appends using the same syntax. C side: `MovieSet` / `MovieAppendSequence`
(`layer1/Movie.cpp:877`, `:890`). Redefining the movie clears existing `mdo` commands
(documented at `modules/pymol/moving.py:307-308`, `:349-351`).

**Frame → state resolution**: `MovieFrameToIndex` (`layer1/Movie.cpp:979`) — a per-frame
...
- `cmd.mdo(frame, command)` → `_cmd.mdo(COb, frame-1, command, 0)` — replaces
  (`modules/pymol/moving.py:274`, C: `MovieSetCommand` `layer1/Movie.cpp:1074`).
- `cmd.mappend(frame, command)` → `_cmd.mdo(COb, frame-1, ";"+command, 1)` — appends
  (`modules/pymol/moving.py:323`, C: `MovieAppendCommand` `layer1/Movie.cpp:1378`).
- `cmd.mdump()` prints all defined commands to the feedback stream, format
  `"%5d: %s\n"` (`modules/pymol/moving.py:81`, C: `MovieDump` `layer1/Movie.cpp:378-403`).
  **There is no structured getter** — see §12 gaps.
- Execution: `MovieDoFrameCommand` (`layer1/Movie.cpp:1045`) — at frame 0 recall
...
(`modules/pymol/moving.py:772-816`; C `MovieMatrix` `layer1/Movie.cpp:589`,
constants `layer1/Movie.h:161-164`). Docstring warns not to mix with `mview`.

`cmd.mclear()` clears the cached frame images (`modules/pymol/moving.py:436`;
C `MovieClearImages` `layer1/Movie.cpp:1432`).
...
state=0, freeze=0)` — `modules/pymol/moving.py:160`.

Actions (`modules/pymol/moving.py:145-156`, mirrored in `layer1/MViewAction.h:3-14`):

| name | int | C behavior |
|---|---|---|
| `store` | 0 | `layer1/Movie.cpp:1155-1196` — writes camera/object matrix into frame(s), sets `specification_level=2`, optional `state`, `power`, `bias`, `scene_name` |
...
| `purge` | 9 | (object motions; menu `obj_motion` uses it, `modules/pymol/menu.py:136`) |

Python-side pre/post logic worth cloning verbatim:
- negative `first`/`last` are counted from `count_frames()` (`modules/pymol/moving.py:214-219`).
...
`layer1/View.h:24-63`: 4×4 `matrix` (+flag), `pre[3]`, `post[3]`, `front`/`back` clip
...
linearity, hand, cut)` (`layer1/View.h:93`). Smoothing: `ViewElemSmooth(first,last,window,loop)`
(`layer1/View.h:92`).
...
| cmd | action | source |
|---|---|---|
| `mdelete(count=-1, frame=0, …)` | -1 | `modules/pymol/moving.py:591` |
| `minsert(count, frame=0, …)` | 1 | `modules/pymol/moving.py:640` |
| `mmove(target, source=0, count=-1, …)` | 2 | `modules/pymol/moving.py:493` |
| `mcopy(target, source=0, count=-1, …)` | 3 | `modules/pymol/moving.py:545` |
...
means "to the end" (`:629-630`). C side: `MovieViewModify` (`layer1/Movie.cpp:89`),
enum `ViewElemAction {Delete=-1, Insert=1, Move=2, Copy=3}` (`layer1/View.h:100-106`).
When not frozen/localized it calls `ExecutiveMotionExtend` (`layer1/Movie.cpp:157-159`).

---

## 5. The Movie Panel (in-viewport, must become React)

`CMovie` is an Ortho `Block` (`layer1/Movie.h:52`). Height:
`MovieGetPanelHeight` (`layer1/Movie.cpp:1701`) = `movie_panel_row_height` ×
...
Rows come from `ExecutiveMotionDraw` (`layer3/Executive.cpp:692`): one row for the global
camera (`cExecAll`, drawn by `MovieDrawViewElem` → `ViewElemDraw(..., "camera", …)`,
`layer1/Movie.cpp:1728-1734`) and one row per object that has motions
(`ObjectDrawViewElem`, `layer3/Executive.cpp:722`). Row count = `ExecutiveCountMotions`
(`layer3/Executive.cpp:659`), which falls back to 1 when `SceneGetNFrame(G) > 1` (`:679-680`).

Per-frame cell painting (`layer1/View.cpp:158-260`): color by `specification_level` —
...
X↔frame mapping: `ViewElemXtoFrame(rect, frames, x, nearest)` (`layer1/View.h:98`,
wrapper `MovieXtoFrame` `layer1/Movie.cpp:72`).

A horizontal `ScrollBar` doubles as the frame slider (`layer1/Movie.cpp:1775-1793`);
...
Drag feedback rectangles (`layer1/Movie.cpp:1797-1840`): white outline on the source
...
Activated by `ExecutiveMotionMenuActivate` (`layer3/Executive.cpp:732`), which resolves
which row was hit and passes the 1-based frame number as a string.

**`camera_motion`** (`modules/pymol/menu.py:108-124`):
- `store` → `cmd.mview("store",first=F)`
- `store with scene` → submenu of up to 40 scene names, each
  `cmd.mview("store",scene=NAME,first=F)` (`modules/pymol/menu.py:54-59`)
- `store with state` → submenu `current` (`state=-1`), `1`, `n_state`, plus 8 evenly
  spaced states (`modules/pymol/menu.py:62-80`)
...
- `smooth key frames` → `a little` / `more (window=15)` / `a lot (window=30)`
  (`modules/pymol/menu.py:103-106`)
- `interpolate`, `reinterpolate`, `uninterpolate`

**`obj_motion`** (`modules/pymol/menu.py:126-143`): `drag`, `store`, `store with state`,
...
Source: `modules/pymol/_gui.py:234-376` (toolkit-independent; consumed by
`modules/pmg_qt/pymol_qt_gui.py:353` and `modules/pmg_tk/skins/normal/__init__.py:1072`).
...
(`modules/pymol/_gui.py:105-110`).

### 6.1 "Last program" bookkeeping

`PyMOLDesktopGUI.mvprg(command=None)` (`modules/pymol/_gui.py:958-968`): remembers
...
`modules/pymol/movie.py`. FPS helper: `get_movie_fps` returns `movie_fps` or 30 if ≤0 (`:26`).
...
height=0)` — `modules/pymol/moving.py:366`. `mode`: `2=ray`, `1=draw`, `0=normal`,
...
C side `MoviePNG` (`layer1/Movie.cpp:819`) drives the modal loop `MovieModalPNG`
(`layer1/Movie.cpp:626`) with stages in `CMovieModal` (`layer1/Movie.h:22-50`).

`movie.produce(filename, mode='', first=0, last=0, preserve=0, encoder='', quality=-1,
quiet=1, width=0, height=0)` — `modules/pymol/movie.py:846`:
...
`modules/pmg_qt/file_dialogs.py:691` `file_save_mpeg(parent, _preselect=None)`,
form `modules/pmg_qt/forms/movieexport.ui`:
...
Frames / Width / Height (`modules/pmg_tk/skins/normal/__init__.py:964-987`).
...
**Store** (`layer3/MovieScene.cpp:173`): key `""`/`"new"` → `getUniqueKey()`; new keys are
...
(`layer3/MovieScene.h:27-34`). Stores: message, `SceneGetView`, `SceneGetFrame`,
...
**Recall** (`layer3/MovieScene.cpp:458`, impl `:485`): each `recall_*` flag is ANDed with the
...
**Order** (`layer3/MovieScene.cpp:88`/`:95`, Python `cmd.scene_order`
`modules/pymol/viewing.py:961`): args `names` (space-separated string or list), `sort`
...
|---|---|---|
| `cmd.get_scene_list()` | `modules/pymol/viewing.py:919` → `_cmd.get_scene_order` | `list[str]` in display order |
| `cmd.get_scene_thumbnail(name)` | `modules/pymol/viewing.py:923` | **PNG byte buffer** (used as `QPixmap.loadFromData(buf,"PNG")`, `modules/pmg_qt/scene_bin_gui.py:189-192`) |
| `cmd.get_scene_message(name)` | `modules/pymol/viewing.py:927` | `str` |
| `cmd.set_scene_message(name, message)` | `modules/pymol/viewing.py:931` | — |
| `cmd.scene_order(names, sort, location)` | `modules/pymol/viewing.py:961` | — |

`cmd.scene_recall_message` (`modules/pymol/viewing.py:1013`) is INTERNAL: it drives the
...
`modules/pmg_qt/scene_bin_gui.py:29` `ScenePanel(QWidget)`:
...
`SceneDrawButtons` (`layer1/Scene.cpp:2885`), enabled by `scene_buttons`
(`layer1/Scene.cpp:3456`). Names come from `SceneSetNames` (`:2870`) into
`CScene::SceneVec` of `SceneElem{name, rect, drawn}` (`layer1/SceneElem.h:7-12`,
`layer1/SceneDef.h:145`). Layout: `internal_gui_control_size` line height (`:2901`),
...
Mouse (`layer1/SceneMouse.cpp:179` `SceneClickSceneButton`):
...
`scene_menu` (`modules/pymol/menu.py:1842-1850`): header `Scene <name>`,
...
`cmd.get_view(output=1, quiet=1)` — `modules/pymol/viewing.py:634`. `_cmd.get_view`
...
`cmd.set_view(view, animate=0, quiet=1, hand=1)` — `modules/pymol/viewing.py:734`.
...
`cmd.view(key, action='recall'|'store'|'clear', animate=-1)` — `modules/pymol/viewing.py:783`.
...
(`layer1/Scene.cpp:411-427`), with `timing` stamps for start and `start+duration`.

### 9.3 Camera commands

| cmd | signature | source |
|---|---|---|
| `zoom` | `(selection="all", buffer=0.0, state=0, complete=0, animate=0)` | `modules/pymol/viewing.py:66`; `animate<0` = default duration, `0` = none, `>0` = seconds (`:98-102`) |
...
`cmd.rock(mode=-1)` → `_cmd.rock` → `ControlRock(G, mode)` (`modules/pymol/viewing.py:1360`,
`layer1/Control.cpp:415-439`):
...
(`modules/pymol/viewing.py:1738-1739`).

`SceneUpdateCameraRock` (`layer1/Scene.cpp:2373-2427`) — `sweep_mode`:
...
`ControlIdling` (`layer1/Control.cpp:395-401`) keeps the idle loop alive while any of
...
`CControl` block, `NButton = 9` (`layer1/Control.h`/`layer1/Control.cpp:62`), hit test
`which_button` = `(NButton * x) / control_width` (`layer1/Control.cpp:243-252`).
Release actions (`layer1/Control.cpp:290-380`):
...
`cControlMinWidth` (`layer1/Control.cpp:449-464`); dragging it resizes.

External Qt quick-button rows (`modules/pmg_qt/pymol_qt_gui.py:223-253`) — the movie row is
...
`Draw/Ray`. Legacy Tk equivalents at `modules/pmg_tk/skins/normal/__init__.py:266-281`.

---

## 11. Keyboard bindings in this area

`modules/pymol/shortcut_dict.py` (defaults; user-overridable through `cmd.set_key`,
`modules/pymol/controlling.py:746-757`):
...
Bare **F1–F12 have no default binding**; `_special` (`modules/pymol/internal.py:447-484`)
...
`modules/pymol/internal.py:398-424`; modifier prefixes `'', SHFT, CTRL, CTSH, ALT`
(`:395-401`).

**Spacebar** (`layer1/Ortho.cpp:855-874`): only when the command line is empty. In
...
- **`CSeq`** — the Ortho block: layout, scrollbar, hit-testing, GL text drawing
  (`layer1/Seq.h:73`, `layer1/Seq.cpp:259` `CSeq::draw`).
  Metrics: `LineHeight = 13`, `CharWidth = 8`, `ScrollBarWidth = 16`,
  `ScrollBarMargin = 2`, `CharMargin = 2` (`layer1/Seq.h:84-88`), all scaled by `DIP2PIXEL`.
  Total height = `LineHeight*NRow + 4` (+ scrollbar) (`layer1/Seq.cpp:190-201`).
- **`CSeeker`** — the data model + interaction handler (`layer3/Seeker.cpp:38`),
  installed via `SeqSetHandler` at the end of `SeekerUpdate` (`layer3/Seeker.cpp:1948`).

Refresh protocol: `SeqChanged` marks "rebuild" and `SeqDirty` marks "recompute selection
highlight" (`layer1/Seq.cpp:136-148`); `SeqUpdate` runs `SeekerUpdate` then
`Handler->refresh` (`layer1/Seq.cpp:88-102`).

### 12.2 Row/column data model — exactly what the React component needs

`CSeqRow` (`layer1/Seq.h:42-57`): `txt` (one flat char buffer for the row), `col[]`,
...
`CSeqCol` (`layer1/Seq.h:25-37`): `start`/`stop` (slice into `txt`), `offset`
(aligned character column), `atom_at` (index into `atom_lists`), `inverse`
(**selected** — drives the inverted-video highlight, `layer1/Seq.cpp:465-482`),
...
`SeekerUpdate` (`layer3/Seeker.cpp:969`), `codes` read from `seq_view_format`
...
Menu labels (`modules/pymol/_gui.py:379-387`): Residue Codes (0), Residue Names (1),
Chain Identifiers (3), Atom Names (2), States (4).

### 12.4 Label modes (`seq_view_label_mode`, global, default 2)

`layer3/Seeker.cpp:987`, `:1028-1101`:
...
Residue-number labels are laid out in a third pass (`layer3/Seeker.cpp:1820-1914`):
...
`layer3/Seeker.cpp:1009`, `:1230-1258`. `GapMode::NONE / ALL(1) / SINGLE(2)`. Gaps are
...
Menu labels: No Gaps (0), All Gaps (1), Single Gap (2) (`modules/pymol/_gui.py:401-405`).

### 12.6 Alignment mode

When an alignment is active (`ExecutiveGetActiveAlignmentSele`,
`layer3/Executive.cpp:3375`, resolved from `seq_view_alignment` or the first enabled
alignment object, `:3391-3409`), the second pass lines rows up by `tag` instead of by
column index (`layer3/Seeker.cpp:1584-1818`). Tags come from `SeekerFindTag`
...
residues are colored (`layer1/Seq.cpp:320-339`, `:423-455`): modes 1/4 average the residue
...
(`layer1/Seq.cpp:316`, `:341-346`, `:488-505`).

### 12.7 Coloring

Per-column color (`layer3/Seeker.cpp:1313-1318` and mirrored in the other code paths):
...
Row label text uses `seq_view_label_color` (default `front`) (`layer1/Seq.cpp:269-271`).
Background is `bg_rgb`, or `bg_rgb_top`/`bg_rgb_bottom` when `bg_gradient` and depending on
`seq_view_location` (`layer1/Seq.cpp:273-281`).

### 12.8 Placement / overlay

`seq_view_location` — 0 = top, 1 = bottom (`layer1/Ortho.cpp:2181`, `:2418`).
`seq_view_overlay` — draw over the 3D scene instead of reserving space
(`layer1/Seq.cpp:282-289`, `layer1/Ortho.cpp:2425-2437`, `layer5/main.cpp:817`, `:1500`).
...
(`layer3/Seeker.cpp:991-993`). Max 50 rows (`max_row = 50`, `layer3/Seeker.cpp:980`, `:1013`).

### 12.9 Interaction — the full grammar

Hit test: `SeqFindRowCol` (`layer1/Seq.cpp:36-86`) maps (x,y) → (row, col) via
`char2col`, accounting for `NSkip` (horizontal scroll) and `label_flag` rows (which are
not clickable). During a drag the row is pinned to `LastRow` (`layer1/Seq.cpp:158`, `:175`).

`SeekerClick` (`layer3/Seeker.cpp:317`):
...
| **Right click on an unselected cell** | builds `cTempSeekerSele` and opens the `seq_option` menu, titled with `ObjectMoleculeGetAtomSele(...)` truncated at the last `/` (`:365-393`, menu at `modules/pymol/menu.py:1800-1840`) |
| **Right click outside any cell** | `pick_sele` menu for the active selection (`layer1/Seq.cpp:240-248`) |
| **Left double-click outside any cell** (<0.35 s, `cDoubleTime` `layer3/Seeker.cpp:315`) | clears the active selection: `cmd.select('<sele>','none', enable=1)` (`:328-341`) |
| **Wheel** | horizontal scroll by ±1 (`layer1/Seq.cpp:218-223`) |
| **Click on the scrollbar strip** | scrollbar drag (`layer1/Seq.cpp:226-231`) |
| Clicking a cell that carries a `state` | sets that object's `state` setting and `SceneChanged` (`layer3/Seeker.cpp:463-466`, `:408-411`) |
| State-mode guard | in `codes == 4` on non-discrete objects the columns are **not** selectable (`:427`) |

Selection algebra (`SeekerSelectionToggle`, `layer3/Seeker.cpp:169-246`;
range variant `:70-167`):
- The active selection name comes from `ExecutiveGetActiveSeleName(create_new=true)`
  (`layer3/Executive.cpp:3420`), auto-numbered `selNN` when `auto_number_selections`
  (`:3436-3443`).
- `sele_mode_kw` = `SceneGetSeleModeKeyword` (`layer1/Scene.cpp:504`), one of
  `"" | byresi | bychain | bysegi | byobject | bymol | bca.` indexed by
  `mouse_selection_mode` (`layer1/Scene.cpp:460-468`).
- Include: `((KW(?sele)) or KW(?tmp))`; Exclude: `((KW(?sele)) and not KW(?tmp))`;
  Fresh: `KW(?tmp)` (`layer3/Seeker.cpp:130-140`, `:202-221`).
...
Highlight sync: `SeekerRefresh` (`layer3/Seeker.cpp:475-525`) recomputes `col->inverse`
...
(`layer1/Seq.h:67-70`), drawn as a line loop with spacer trimming (`layer1/Seq.cpp:510-563`).

The scrollbar itself renders a **mini-map** of the selection: colored ticks per contiguous
`inverse` run, one horizontal band per non-label row (`layer1/Seq.cpp:564-696`).

---

## 13. Settings that this area owns

From `layer1/SettingInfo.h` (id, name, scope, default):
...
Also `draw_frames` (referenced by the menu, `modules/pymol/_gui.py:369`, and by
`cmd.mpng` mode resolution `modules/pymol/moving.py:392-393`).
...
Display menu entries that map to these: `modules/pymol/_gui.py:378-406`.
...
1. **No structured movie-panel data.** `cmd.mdump` only prints
   (`layer1/Movie.cpp:378-403`). `MovieGetSpecLevel(G, frame)` (`layer1/Movie.cpp:163`)
   is C-only; it is not in the `_cmd` method table (`layer4/Cmd.cpp:6549-6562`).
...
2. **No structured sequence-viewer data.** `SeekerUpdate` writes only into
   `G->Seq->Row` (`layer3/Seeker.cpp:1947`). `cmd.get_fastastr`
   (`modules/pymol/exporting.py:170`) gives sequences but **no colors, no per-cell atom
   indices, no selection state, no gaps, no alignment offsets**, and it only covers
   polymers (`:198`). `cmd.get_seq_align_str` exists (`layer4/Cmd.cpp:6490`) but is an
   alignment export, not the viewer model.
   → Need `bridge.get_seq_view()` returning the §12.2 payload, plus a `seq_changed` event
   wired to `SeqChanged`/`SeqDirty` (`layer1/Seq.cpp:136-148`).
...
4. **No event stream.** Everything today is polled or push-from-C via `OrthoDirty`.
   ...
   (`layer3/MovieScene.cpp:833`), `scene_current_name`, `seq_changed`, `viewport`,
   `view` (for camera sync while rocking/animating).
5. **Menus are Python data, not C.** `get_menudata` (`modules/pymol/_gui.py:55`) and
   `modules/pymol/menu.py` return plain nested lists of
   ...
   `modules/pmg_qt/pymol_qt_gui.py:298-342`).
...
| `<MovieTimeline/>` | `CMovie` block (`layer1/Movie.cpp:1741`) | new `get_movie_panel()`; writes via `cmd.mmove/mcopy/minsert/mdelete/mview` |
| `<TransportBar/>` | `CControl` (`layer1/Control.cpp:290`) + Qt row (`pymol_qt_gui.py:241-247`) | `rewind/backward/mstop/mplay/mtoggle/forward/ending/middle/mclear/rock/full_screen` |
| `<StateSlider/>` | scrollbar in `CMovie::draw` (`layer1/Movie.cpp:1775`) | `cmd.frame`, `cmd.set_frame`, `count_frames`, `count_states` |
| `<ScenePanel/>` | `scene_bin_gui.py:29` | new `get_scenes()`, `cmd.scene`, `cmd.scene_order`, `set_scene_message` |
| `<SceneButtons/>` | `SceneDrawButtons` (`layer1/Scene.cpp:2885`) | `get_scene_list`, `scene_current_name`, `cmd.scene`, `cmd.scene_order` |
| `<SceneContextMenu/>` | `menu.py:1842` | `cmd.scene(...,'update'/'delete'/'rename')` |
| `<SequenceViewer/>` | `CSeq`+`CSeeker` (`layer1/Seq.cpp:259`, `layer3/Seeker.cpp:969`) | new `get_seq_view()`; writes via `cmd.select(name, expr, enable=1)`, `cmd.center/zoom`, `cmd.set('state',…)` |

...
**Status:** authoritative plan. Every other document in `docs/webclient/` is an *area map* (a
...
|---|---|---|
| The engine is a single CPython extension `pymol._cmd`; the Python API is `modules/pymol/` | `pyproject.toml:20-27` (`build-backend = "backend"`, `backend-path = ["_custom_build"]`), `pyproject.toml:53-54` (`packages.find where = ["modules"]`) | ✓ |
| The headless engine is startable from Python with no GUI toolkit at all | `modules/pymol2/__init__.py:28-73` — `SingletonPyMOL.start()` calls `_cmd._new(pymol, pymol.invocation.options, True)` then `_cmd._start`; exposes `idle/getRedisplay/reshape/draw/button/drag` | ✓ |
| `pymol.launch()` already has a no-GUI branch driven purely by `idle()`/`draw()` | `modules/pymol/__init__.py:386-403` (`_launch_no_gui`), dispatched at `:416-417` | ✓ |
| **The feedback queue is gated on `pmgui`, and `pmgui = !no_gui`** | `layer1/Ortho.cpp:495` (`if (G->Option->pmgui)`) and `layer1/P.cpp:1820` (`rec->pmgui = !PyInt_AsLong(... "no_gui")`) | ✓ |
| `G->HaveGUI` is set from the same flag | `layer5/PyMOL.cpp:2248` (`I->G->HaveGUI = I->G->Option->pmgui`) | ✓ |
| A valid GL context is only asserted around draw/render paths, via an explicit push/pop | `layer5/PyMOL.cpp:2940-2949` (`PyMOL_PushValidContext` / `PyMOL_PopValidContext`), used at `:2115,:2281,:2303` | ✓ |
| Feedback is drained one string per call and returns `None` under lock contention | `modules/pymol/internal.py:593-606`; C side `layer4/Cmd.cpp:6463` (`{"get_feedback", CmdGetFeedback, METH_VARARGS}`) | ✓ |
| Setting-change notification is a **consume-once drain** | `modules/pymol/setting.py:440-447`; C side `layer4/Cmd.cpp:6495` | ✓ |
| Binary float blobs already have a first-class serializer | `layer1/PConv.cpp:971-977` (`PConvFloatArrayToPyList(..., bool dump_binary)` → `PyBytes_FromStringAndSize`), int variant at `:1061-1064` | ✓ |
| Surface geometry is CPU-resident on the Rep and never freed after upload | `layer2/RepSurface.cpp:59-101` — `std::vector<float> V, VN, VC, VA, VAO; std::vector<int> RC, Vis, T, S, AT` | ✓ |
| CGO ↔ Python round-trip already exists and already expands `CGO_DRAW_ARRAYS` | `layer1/CGO.cpp:241` (`CGOArrayAsPyList`), `:289` (`CGOAsPyList`), inverse noted at `:308` | ✓ |
| **A new `layer4/*.cpp` file needs zero build-file edits** | `setup.py:808-816` (`get_sources` globs `d + "/*" + s`), `setup.py:559-568` (`pymol_src_dirs` includes `"layer4"`), `CMakeLists.txt:7` (`add_library(${TARGET_NAME} SHARED ${ALL_SRC})`) | ✓ |
| GLSL sources that must be ported to WebGL2 are on disk and enumerable | `data/shaders/` — 46 files incl. `sphere.vs/.fs`, `cylinder.vs/.fs`, `surface.vs/.fs`, `default.vs/.fs`, `line.vs/.fs`, `trilines.vs/.fs`, `label.vs/.fs`, `oit.vs/.fs`, `volume.vs/.fs`, `ramp.vs/.fs` | ✓ |
| `cmd.do` swallows return values and exceptions | `modules/pymol/commanding.py:441-461` + parser behaviour documented in the `cmd-api-rpc` map (`modules/pymol/parser.py:465-481`) | ✓ |
| The existing HTTP bridge hard-rejects non-loopback peers and is single-threaded | `modules/pymol/pymolhttpd.py:35-38` (single-threaded comment), `:61-68` (`host[0:6] != '127.0.'` → 403) | ✓ |
...
* `pmgui = 0` (i.e. `--no-gui`) → `G->HaveGUI = false` (`layer5/PyMOL.cpp:2248`), which is what we
  want (no GL) — **but** `OrthoFeedbackIn` silently drops every message (`layer1/Ortho.cpp:495`),
...
draw/render paths via `PyMOL_PushValidContext` (`layer5/PyMOL.cpp:2940`, called at `:2115,:2281,
...
1. **The upstream PyMOL tree is not moved, not renamed, and not reorganised.** `layer0/`–`layer5/`,
   `modules/`, `data/`, `setup.py`, `pyproject.toml`, `CMakeLists.txt` stay exactly where they are.
   A `git merge` from upstream `pymol-open-source` must never touch a web-client file.
2. **All new code lives under `webclient/`**, plus exactly one new C++ file in `layer4/` and
   exactly one small edit to `layer4/Cmd.cpp` (§1.3).
3. **The bridge is a separate installable Python package**, `pymol-bridge`, not a new
   `modules/pmg_web/` package — so it version-skews independently and never collides with an
   upstream file.
...
├─ layer0/ … layer3/                      #   (untouched)
├─ layer4/
│  ├─ Cmd.cpp                             #   ONE edit: method-table rows (§1.3)
│  └─ CmdWebGeometry.cpp                  #   NEW — auto-globbed by setup.py:808-816
├─ layer5/  layerGraphics/  ov/           #   (untouched)
├─ modules/                               #   (untouched — read-only for this project)
├─ data/                                  #   (untouched; shaders read at build time by codegen)
├─ setup.py  pyproject.toml  CMakeLists.txt   # (untouched)
├─ docs/webclient/*.md                    # the 12 area maps + this document
└─ webclient/                             # ← everything new
   ...
   │
   ├─ bridge/                             # Python — the PyMOL-side service
   ...
   │  │  ├─ emit.ts
   │  │  └─ package.json
   │  ├─ gen-shaders/                     # data/shaders/*.{vs,fs} → WebGL2 GLSL modules
   │  │  ├─ transpile.mjs
   │  │  └─ package.json
   │  └─ parity/                          # parity-matrix generator + checker (§7)
   │     ├─ extract-features.mjs          # parses docs/webclient/*.md feature tables
   │     ├─ check.mjs
   │     └─ package.json
   │
   ├─ packages/
   ...
   │  └─ testing/                         # @pymol/testing — parity harness + fixtures
...
`get_mtl_obj`, `get_gltf`) goes through `SceneRay` (`layer1/SceneRay.cpp:88`), re-rendering the
...
1. **`layer4/CmdWebGeometry.cpp`** (new file). Implements `CmdGetRepGeometry`,
   ...
   `PConvFloatArrayToPyList(ptr, len, /*dump_binary=*/true)` (`layer1/PConv.cpp:971-977`) and the
   existing CGO walker `CGOArrayAsPyList` (`layer1/CGO.cpp:241`) as its reference implementation.
   **No build-file edit is required**: `setup.py:808-816` globs `layer4/*.cpp` and
   `setup.py:559-568` lists `"layer4"`, feeding `${ALL_SRC}` at `CMakeLists.txt:7`.
2. **`layer4/Cmd.cpp`**: additional rows in the method table next to
   `{"get_feedback", CmdGetFeedback, METH_VARARGS}` (`layer4/Cmd.cpp:6463`), plus the matching
   forward declarations. This is a *pure insertion*, ~10 lines, in one contiguous region.

**Nothing else in `layer0/`–`layer5/` or `modules/` is edited by this project.** Both files are
owned by exactly one work package (WP-06) so no merge conflict is possible between agents. If a
second C++ need appears later it goes into `layer4/CmdWebGeometry.cpp` too, and the owning WP is
...
│    • pymol2.SingletonPyMOL().start()      modules/pymol2/__init__.py:52-63 │
...
(`modules/pymol/locking.py:26-40`) and several drains fail *silently* under contention —
`cmd._get_feedback()` returns `None` when `lock_attempt` fails (`modules/pymol/internal.py:596,605`)
and `cmd.get_setting_updates()` returns `[]` the same way (`modules/pymol/setting.py:442-447`).
Worse, per the `input-mouse-keyboard` map, all viewport input is queued through `OrthoDefer`
(`layer1/Scene.cpp:4113-4155`) with captured timestamps, so **reordering input corrupts drag
...
2. `p = pymol2.SingletonPyMOL(); p.start()` (`modules/pymol2/__init__.py:52-63`).
3. `import pcatch; pcatch._install()` so Python `print`/tracebacks land in the Ortho queue
   (per the `cmd-api-rpc` map: `layer1/P.cpp:2713-2721`, `:2663-2699`).
4. Install the GUI seams the backend expects to exist, in `pymol_bridge/shims.py`:
   ...
   (per the `qt-main-window` map: `modules/pmg_qt/pymol_qt_gui.py:1245-1252`,
   `modules/pymol/viewing.py:1446-1457`).
...
`closeEvent` → `cmd.quit()` at `modules/pmg_qt/pymol_qt_gui.py:56-57` and that `confirm_quit`
...
| Uploads | **`POST /upload`** → server temp path | The backend takes *paths*, not blobs (`modules/pymol/importing.py:643-827`). |
| Auth | 256-bit token minted at startup (file mode `0600`), passed as `?token=`, **plus** `Origin` allow-list, **plus** the loopback peer check | The dispatcher can reach `cmd.system`/`cmd.run`; `modules/pymol/pymolhttpd.py:61-68` already establishes the loopback precedent and it alone is not enough against a hostile local page. |
| Bind | `127.0.0.1` only, never `0.0.0.0` | Same reason; `modules/pymol/rpc.py` binding to `''` is the anti-pattern we are replacing. |

**Both legacy bridges are replaced, not extended.** `modules/pymol/pymolhttpd.py` discards POST
bodies, has dead batch code, and no push; `modules/pymol/rpc.py` does
...
* `m` is a flat attribute name on `cmd`; dotted names (`util.cbag`, `movie.produce`) resolve by
  splitting on `.` — the same namespace modules exported at `modules/pymol/api.py:487-489`.
...
* Allow-list = symbols exported by `modules/pymol/api.py` ∪ `cmd.keyword`, minus a deny-list:
...
* `t:'do'` exists **only** for the console widget. It returns `null` because `cmd.do` returns
  `None` and prints exceptions instead of raising (`modules/pymol/commanding.py:441-461`).
...
(`modules/pymol/internal.py:593-606`, `modules/pymol/setting.py:440-447`), so a second consumer
...
`modules/pmg_qt/pymol_qt_gui.py:391-394`), then backing off 4 ms → 16 ms → 100 ms → 250 ms while
...
Feedback is one undifferentiated text stream: `modules/pymol/colorprinting.py` assigns `error`,
...
(it would be a `modules/` edit) and is recorded as a follow-up.
...
* **Surface** ships as raw `RepSurface` vectors (`layer2/RepSurface.cpp:59-101`, verified above):
...
* **CGO** ships as the interleaved `CGO_DRAW_ARRAYS` block that `CGOCombineBeginEnd` already
  produces (`layer1/CGO.cpp:1645-1672`): `[vertex 3n][normal 3n][color 4n][pickcolor 3n]
  [accessibility 1n]`, header `{mode, arraybits, narrays, nverts}`. Cartoon is already in this
  form before any GL (`layer2/RepCartoon.cpp:4287-4300`). This maps 1:1 to a three.js
  `InterleavedBuffer`.
* **Spheres and cylinders** ship as *impostor instance data*, not meshes, and the client ports
  `data/shaders/sphere.{vs,fs}` and `data/shaders/cylinder.{vs,fs}` (present on disk, verified
  ...
  (`layer1/CGO.cpp:4444`) server-side and ships baked triangles — bigger payload, zero client
  geometry math, used for debugging and for clients that fail the WebGL2 check.
* **Never read back a VBO.** `layer1/CGO.h:183-186` documents that the CPU copy is deliberately
...
* **Cartoon hazard.** `disposePreshaderCGO` frees the preshader at first GL render
  (`layer2/RepCartoon.cpp:83-89, 240`). In our process no GL render ever happens (§0.1), so the
...
* **Caching.** Every rep payload carries a content hash. The client sends `have` and the bridge
  ...
  `layer1/Rep.h:133-184`) re-ships only `VC`/`VA`.
...
* **Picking is authoritative server-side.** `SceneDoXYPick` renders a colour-index pass and reads
  back one pixel in the *backend* GL context (`layer1/ScenePicking.cpp:17-38,146-149`). We have no
  ...
  (`layer1/CGO.h:141-142`; sentinels `cPickableAtom` −1 … `cPickableThrough` −5) or selections
...
| 3D | **three.js r17x** + custom `ShaderMaterial`s | Mandated; we use it as a WebGL2 scene graph only — every material is a port of `data/shaders/*`. |
...
| Styling | **Tailwind CSS 4** + CSS variables for theming | `data/pmg_qt/styles/pymol.sty` has 3 rules — there is no design source of truth to copy, so we need a real token system. |
...
| Code editor | **CodeMirror 6** | Small, themeable, and we must author a PML language mode (ported from `modules/pmg_qt/syntax/`). |
...
* **Materials.** `packages/viewport/src/materials/*` are ports of `data/shaders/*`, generated into
...
bridge runs the real trackball math (`layer1/SceneMouse.cpp:1774-1854`, three
...
| Object list (Executive block) | `layer3/Executive.cpp:16116` | `features/objects/*` | WP-12 |
| A/S/H/L/C/M popups | `layer4/PopUp.cpp` + `pymol/menu.py` | `features/pymol-menu/*` | WP-13 |
| Wizard panel + prompt | `layer1/Wizard.cpp:227`, `layer1/Ortho.cpp:2124` | `features/wizards/*` | WP-15 |
| ButMode block | `layer1/ButMode.cpp:192` | `features/keyboard/MouseModeBlock.tsx` | WP-22 |
| Control bar (9 buttons) | `layer1/Control.cpp:298` | `features/movie/ControlBar.tsx` | WP-19 |
| Movie timeline | `layer1/Movie.cpp:1741` | `features/movie/MovieTimeline.tsx` | WP-19 |
| Scene buttons overlay | `layer1/Scene.cpp:2885` | `features/scenes/SceneButtons.tsx` | WP-19 |
| Sequence viewer | `layer3/Seeker.cpp:969` | `features/seqview/*` | WP-20 |
| In-viewport CLI + scrollback | `layer1/Ortho.cpp:841`, `:1623` | folded into `features/console/*` | WP-14 |
...
| **WP-06** | Native geometry accessor (`layer4/CmdWebGeometry.cpp`) + bridge extractor | WP-02 | 2 |
...
* `layer4/CmdWebGeometry.cpp` and the `layer4/Cmd.cpp` method-table insertion are **WP-06 only**.
...
```json
{
  ...
  "scripts": {
    ...
    "test": "vitest run",
    "test:watch": "vitest",
    "test:py": "python -m pytest bridge/tests -q",
...
`tools/parity/extract-features.mjs` parses the twelve area maps in `docs/webclient/*.md` and emits
...
1. **Record from Qt.** A recorder (`bridge/tests/record_qt.py`) opens `cmd.log_open(f, 'w')` —
...
* **Widget parity is structural, not pixel:** each Qt widget gets a Storybook story plus a
  ...
  cloning Qt's native theme; `data/pmg_qt/styles/pymol.sty` has three rules, so there is nothing to
  clone.
...
* How much of `data/shaders/` compiles unmodified under the `PURE_OPENGL_ES_2` preprocessor path?

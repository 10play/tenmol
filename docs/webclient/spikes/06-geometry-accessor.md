...
| | |
|---|---|
| New file | `layer4/CmdWebGeometry.cpp`, 1,451 lines |
| Upstream edit | `layer4/Cmd.cpp`, **+7 lines, 0 removed**, two sentinel-marked regions |
| Build-file edit | **none** — `setup.py:808-816` globs `layer4/*.cpp` (confirmed: it built with no change) |
...
Plan §4 task 1 says "emit `RepSurface::{V,VN,VC,VA,VAO,T,AT,Vis}` (`layer2/RepSurface.cpp:74-85`)".
The line reference is to the **.cpp**, and that is not an accident:

```
$ grep -rn "struct RepSurface" layer1 layer2 layer3 layer4 layer5
layer2/RepSurface.cpp:59:struct RepSurface : Rep {
```
...
unit**. Only `RepSphere` (`layer2/RepSphere.h:27`) and `RepDot` (`layer2/RepDot.h:28`) have public
struct definitions. A new file in `layer4/` therefore cannot see nine of the eleven rep types
without either (a) editing `layer2/*`, which this work package does not own, or (b) a layout mirror.
...
points to spheres (`layer1/CGO.cpp:6067-6130`), which reproduces **exactly** the degradation the
task forbids: 1UBQ `mesh` → 31,710 cylinders. `CPrimitive` (`layer1/Basis.h:66-82`) also has no
atom index field, so identity is lost too. Both of the accessor's reasons to exist die on this
route.

**Chosen: layout mirrors + the primitive CGO.** `layer4/CmdWebGeometry.cpp` declares a
`namespace mirror` struct per rep that repeats, in order and with identical types, the data members
the upstream derived class declares. The `Rep` base sub-object comes from the real `layer1/Rep.h`,
...
* **CGO reps** — `cgoLooksSane()` checks `cgo->G == G` (`CGO::G` is the first data member,
  `layer1/CGO.h:772`), that `op` is non-null when `c > 0`, and that **every** opcode reached by
...
(`layer2/RepCartoon.cpp:4292-4294`). That is exactly the block the plan says to emit verbatim, and
...
`layer1/CGO.h:183-186`: once a CGO has been uploaded, the CPU copy is deliberately dropped. The
...
**Trap 1 — `RepCartoonCGOGenerate` calls `disposePreshaderCGO()`** (`layer2/RepCartoon.cpp:240`).
...
`AtomInfoIsMasked(obj, I->AT[idx])` at `layer2/RepSurface.cpp:402`). Verified against
`cmd.get_model()`:
...
`draw_arrays` block layout mirrors `layer1/CGO.cpp:1645-1672` exactly: vertex `3·nverts`, normal
...
interleaved stream `[count, r, g, b, (nx ny nz x y z)*count]…` (`layer2/RepDot.cpp:402-425`) and
...
`layer4/Cmd.cpp` gains **7 lines and loses none**, in two sentinel-marked regions:
...
`layer4/CmdWebGeometry.cpp` is a file upstream does not have and can never conflict.
**No build-file change was needed** — the build succeeded on the first attempt, confirming
`setup.py:808-816`'s `layer4/*.cpp` glob.
...
| mirror | upstream |
|---|---|
| `mirror::RepSurface` | `layer2/RepSurface.cpp:59-100` |
| `mirror::RepCartoon` | `layer2/RepCartoon.cpp:65-91` |
| `mirror::RepCylBond` | `layer2/RepCylBond.cpp:39-49` |
| `mirror::RepWireBond` | `layer2/RepWireBond.cpp:34-45` |
| `mirror::RepRibbon` | `layer2/RepRibbon.cpp:37-50` |
| `mirror::RepNonbonded` | `layer2/RepNonbonded.cpp:33-44` |
| `mirror::RepNonbondedSphere` | `layer2/RepNonbondedSphere.cpp:35-44` |
| `mirror::RepEllipsoid` | `layer2/RepEllipsoid.cpp:33-44` |
| `mirror::RepMesh` | `layer2/RepMesh.cpp:39-63` |
...
rm -rf modules/pymol.egg-info      # pip leaves this behind, it is not gitignored
```
...
```bash
$ venv/bin/pymol -ckq testing/testing.py --run all      # from the repo root
...
`M layer4/Cmd.cpp` and `?? layer4/CmdWebGeometry.cpp` from this work package.

...
* **Linux / `egl.py` — RUN, on real Linux, end to end.** In a `debian:bookworm-slim`
  ...
  **OpenGL 4.5 (Compatibility Profile)** llvmpipe context, one FBO bound before the
  ...
  5,684-atom cartoon of `test/dat/1tii.pdb`** with `glGetError() == 0`, and the
...
| `bridge/tenmol_bridge/glcontext/egl.py` | this spike | Linux / BSD — **working, hardware-verified (§2.8)** |
| `bridge/tenmol_bridge/glcontext/wgl.py` | this spike | Windows — **unverified** |
...
| Does GLEW complain under EGL with no X? | **No output at all.** `_cmd*.so` links `libGLEW.so.2.2`, `libGLX.so.0` and `libX11.so.6`, yet with `DISPLAY` unset a full PyMOL start + draw wrote **nothing** to stderr and `cmd.get("use_shaders")` was still `on`. `layer0/ShaderMgr.cpp:566-573` swallowing `GLEW_ERROR_NO_GLX_DISPLAY` is doing its job. |
...
== 4. resize keeps the FBO NAME (layer5/PyMOL.cpp:2236) ==========
...
== 8. the backend pick pass (layer1/ScenePicking.cpp) ============
...
(`layer0/ShaderMgr.cpp:590-597`) did **not** run. This settles residual risk §8.2 for
GLEW 2.2 on Mesa.
...
Re-verified portably after those edits (macOS, `bridge/.venv/bin/python`):
...
# On an actual Linux box with bridge/.venv already built:
...
| 7 | FBO id across four resizes | identical every time | `layer5/PyMOL.cpp:2236-2239` latches it |
...
2. **GLEW flavour (Linux).** ~~could still fail~~ — **resolved for GLEW 2.2 on Mesa.** The built
   ...
   `layer0/ShaderMgr.cpp:566-573` swallowing `GLEW_ERROR_NO_GLX_DISPLAY` is doing exactly its job.
...
14. **Screenshots owner** — `docs/webclient/screenshots/` has no Linux evidence. The artefact
...
15. **`.gitignore` owner** — add `/.tenmol-gl-out/`. That is `scripts/test-gl-linux.sh`'s default
    ...
    .tenmol-gl-out` matches nothing, while `.deps` and `bridge/.venv` are correctly ignored. CI
...
The same script, the same structure (`test/dat/1tii.pdb`, 5,684 atoms, cartoon, 320×240), run
twice in the same Linux container:

* **GL** — `egl.create_context()` first, then `invocation.options.no_gui = 0` (⇒ `pmgui = 1` ⇒
  `G->HaveGUI = 1`, `layer5/PyMOL.cpp:2248`).
* **NO-GL** — no EGL context is created at all, `no_gui = 1` (⇒ `HaveGUI = 0`, which makes the
  entire body of `SceneRender` a no-op, `layer1/SceneRender.cpp:270`).

...
(`layer1/Ortho.cpp:492-499`). `pmgui` is set from `!options.no_gui` (`layer1/P.cpp:1820`), and
`-c` sets `no_gui=1` (`modules/pymol/invocation.py:401`). Start with `no_gui = 0` and suppress the
...
through the file-scope `SingletonPyMOLGlobals` pointer (`layer1/P.cpp:2667`). With a non-singleton
instance that pointer is `nullptr`, so `pcatch` **silently discards every `print()`** — worse than
not installing it. `pmg_qt` already does the right thing: `modules/pmg_qt/pymol_gl_widget.py:3`
is `from pymol2 import SingletonPyMOL as PyMOL`.

`HaveGUI` is only assigned from `pmgui` inside `PyMOL_DrawWithoutLock()`
(`layer5/PyMOL.cpp:2244-2248`), which a headless bridge never calls. So `no_gui=0` costs nothing:
...
* `modules/pmg_qt/pymol_gl_widget.py:3` — `from pymol2 import SingletonPyMOL as PyMOL`
* `modules/pmg_qt/pymol_gl_widget.py:99-105` — `self.pymol.start()` then `import pcatch;
  pcatch._install()`
* `modules/pmg_qt/pymol_qt_gui.py:391-394` — a `QTimer` (single-shot, 100 ms initially)
* `modules/pmg_qt/pymol_qt_gui.py:941-958` — `update_feedback()`:
  `feedback = self.cmd._get_feedback()` → `colorprinting.text2html('\n'.join(feedback))` →
  `browser.appendHtml(...)` → `feedback_timer.start(500)`
* `modules/pmg_qt/pymol_qt_gui.py:964` — after each typed command, `feedback_timer.start(0)` for
  an immediate drain.
...
(`modules/pymol/colorprinting.py:29-32`), and `colored_feedback` reports
...
braces: `OrthoFeedbackOut` strips ANSI when `colored_feedback` is off, `layer1/Ortho.cpp:502-516`.)

---

## 4. Consume-once semantics — CONFIRMED DESTRUCTIVE

`modules/pymol/internal.py:593-606` loops `_cmd.get_feedback()` until it returns empty;
`layer4/Cmd.cpp:3892` calls `OrthoFeedbackOut`, which does `front()` + `pop()`
(`layer1/Ortho.cpp:502-516`). Measured (`e5b.py`):
...
2. **PyMOL keeps no scrollback for you.** (`I->Line[]` is a 256-entry ring, `OrthoSaveLines 0xFF`
   at `layer1/Ortho.cpp:62`, but it is not readable from Python.) The bridge must own the ring
...
The terminal `printf` in `OrthoNewLine` (`layer1/Ortho.cpp:1160-1169`) is gated on
`Feedback(G, FB_Python, FB_Output)`. So is `PCatchWrite` (`layer1/P.cpp:2668`). Measured
...
the ortho layer dirty (`layer1/Ortho.cpp:1119-1122`).
...
* **Long lines are hard-split at ~1018 chars** regardless of `wrap_output` (default `off`);
  `OrthoLineLength` fail-safe at `layer1/Ortho.cpp:1097-1104`. A 20000-char write became 20
...
Why this shape (all empirically established, see docs/webclient/spikes/02-feedback.md):

  * layer1/Ortho.cpp:492-499 -- OrthoFeedbackIn() only queues when G->Option->pmgui.
    pmgui is derived from `not invocation.options.no_gui` (layer1/P.cpp:1820).
    `pymol -c` sets no_gui=1 (modules/pymol/invocation.py:401), so a "-c" bridge gets
  ...
  * pcatch writes via SingletonPyMOLGlobals (layer1/P.cpp:2667). With a NON-singleton
    ...
    modules/pmg_qt/pymol_gl_widget.py:3,99-105).

  * cmd._get_feedback() is a DESTRUCTIVE read (modules/pymol/internal.py:593 loops
...
class FeedbackCapture:
    ...
    def _uninstall_pcatch(self):
        ...
        # (SingletonPyMOLGlobals is nulled in layer5/PyMOL.cpp:2075-2076).
        if self._installed:
...
1. **`docs/webclient/spikes/00-build.md` and `docs/webclient/build-and-tooling.md`** — the
...
2. **`docs/webclient/spikes/00-build.md`, action item (3)** — "must use `pymol2.PyMOL()`, not
...
3. **`docs/webclient/01-architecture.md`** — the bridge must set
...
4. **`docs/webclient/01-architecture.md` / `internal-gui.md`** — the console feature rows should
...
5. **`docs/webclient/internal-gui.md`** — progress reporting is a separate channel
   (`cmd.get_progress()`), not part of the feedback stream.
6. **`docs/webclient/02-completeness-critique.md`** — BLOCKER "headless feedback capture" can be

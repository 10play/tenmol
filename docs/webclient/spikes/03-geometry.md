...
`docs/webclient/geometry-extraction.md` gives.*
...
Local alternatives in `test/dat/` (`1tii.pdb`, `il2.pdb`, `3al1.pdb`) and `data/tut/1hpv.pdb` were
...
(`layer1/P.cpp:1321`), and elements 2, 7 and 14 of that tuple are Python **lists** (measured:
`['str','int','list','tuple','float','int','int','list',...]`). The exception is swallowed by
`PyErr_Print()` at `layer1/P.cpp:1374`, so the cache still functions (equality on `entry[2]` saves
...
1. `docs/webclient/geometry-extraction.md:347` — the per-call cost table (`10^0 – 10^1 s` for

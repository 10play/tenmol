...
| Does a plain `pip install .` work out of the box with Homebrew? | **No.** It fails on `'mmtf.hpp' file not found` (`layer3/MoleculeExporter.cpp:16`). You must either vendor `mmtf-cpp` headers or pass `--config-settings use-msgpackc=no`. |
| Do we need Qt / GLUT / a display to run the backend? | **No.** `pymol2.PyMOL()` (`modules/pymol2/__init__.py:79`) starts headless, loads molecules, computes surfaces and ray-traces to PNG with **zero GL context**. Verified. |
...
| Fastest dev loop? | `pip install -e .` (26.8 s cold, ~2 s warm for Python-only edits). Python under `modules/` becomes live-editable; C++ needs a rebuild. |
...
`_custom_build/backend.py:10-36` subclasses setuptools' `build_meta` and rewrites `sys.argv`:

```python
# _custom_build/backend.py:12-18
...
**Gotcha (verified):** `_custom_build/backend.py` only exports `build_wheel` and
...
1. **Codegen at import time** — `setup.py:550` calls `create_all(generated_dir)` at module
   ...
   - `create_shadertext()` (`setup.py:97-158`) slurps `data/shaders/*.{gs,vs,fs,shared,tsc,tse}`
...
3. **Prefix search** — `get_prefix_path()` (`setup.py:250-285`), then `setup.py:783-796`
   scans each prefix for `include`, `include/freetype2`, `include/libxml2`,
   `include/openvr`, `lib64`, `lib`.

4. **Two extensions declared** (`setup.py:860-878`):
   - `pymol._cmd` — the whole engine, sources from `pymol_src_dirs` (`setup.py:559-570`).
   - `chempy.champ._champ` — sources from `contrib/champ` (10 C files).
...
7. **Version** comes from `layer0/Version.h` via regex (`setup.py:804-805`).
...
| `contrib/uiuc/plugins/molfile_plugin/src` | 67 | yes (`--vmd-plugins`, default on, `setup.py:619-629`) |
| `contrib/mmtf-c` | 1 | only if msgpack enabled (`setup.py:650`) |
| `contrib/champ` | 10 | yes, separate `_champ` ext |
| `layerCTest` | 23 | only `--testing=true` (`setup.py:657-659`) |
| `contrib/vr` | 10 | only `--openvr=true` (`setup.py:661-665`) |
| `build/generated` | 1 (`ShaderText.cpp`) | yes |

Default macOS build linked **254 object files** (measured: `find build -name '*.o' | wc -l`).

Header-only include dirs: `include/` (`include/pymol/*.h`, `include/tnt/*.h` — 36 files),
`contrib/pocketfft` (`setup.py:669`), `contrib/uiuc/plugins/include`.
...
| `glm` | `layer0/Bezier.h:5`, `layer0/TTT.h:3-5` | installed, `/opt/homebrew/include/glm/glm.hpp` OK |
...
| `mmtf-cpp` | `layer3/MoleculeExporter.cpp:16` (`#include <mmtf.hpp>`) | **NOT PACKAGED BY BREW, NOT IN TREE.** `include/` contains only `pymol/` and `tnt/`. |
| `catch2` | `layerCTest/Test.cpp:11`, `layerCTest/Test.h:14` (`<catch2/catch.hpp>`) | brew ships **3.15.2** — v3 has no `catch2/catch.hpp`. **`--testing=true` cannot work from brew.** CI uses conda `catch2=2.13.3` (`build.yml:87`, `:146`). |
...
| `collada2gltf` binary | `modules/pymol/querying.py:679` | **not installed** → glTF export test errors (see 2.6) |
...
| full test suite `pymol -ckq testing/testing.py --run all` | 9.5 s | 5.5 s | 961 tests in 7.7 s + a pytest phase |
...
```python
import pymol2
with pymol2.PyMOL() as p:          # modules/pymol2/__init__.py:79
...
`pymol2.PyMOL.__init__` (`modules/pymol2/__init__.py:86-121`) calls
`_cmd._new(self, self.invocation.options)` and `PyMOL.start()`
(`modules/pymol2/__init__.py:129-131`) calls `_cmd._start(self._COb, self.cmd)`.
`SingletonPyMOL` (`modules/pymol2/__init__.py:30-76`) additionally exposes
...
1. **`fatal error: 'mmtf.hpp' file not found` (`layer3/MoleculeExporter.cpp:16`).**
   ...
   mmtf-cpp/include/* include/` exactly as CI does (`build.yml:40-43`, `:154-160`) — this
   dirties the upstream `include/` dir; or (b) `--config-settings use-msgpackc=no`, which
   costs you MMTF and BCIF I/O (see failure 5).

2. **`fatal error: 'catch2/catch.hpp' file not found` with `--config-settings testing=True`**
   (`layerCTest/Test.h:14`, `layerCTest/Test.cpp:11`). Homebrew only offers Catch2
...
5. **Runtime test failures caused by the `use-msgpackc=no` workaround.** With the working
   build, `pymol -ckq testing/testing.py --run all` gave
   ...
   - `ERROR testglTF` → `pymol.CmdException: could not find collada2gltf`
     (raised at `modules/pymol/querying.py:679`). CI installs a prebuilt
   ...
   - `FAIL test_commands (symop_py.TestBondSymOp)` →
     ...
     `testing/testing.py:443`. **Ray-traced image-diff tests are not bit-stable on
...
6. **`pymol -c script.py` exits 0 even when the script raises.** I reproduced this:
   `pymol -c testing/testing.py --run api/test_cmd.py` printed a full
   `ModuleNotFoundError: No module named 'pytest'` traceback and still returned
   `EXIT=0`. Worse, with the `-y` flag (`exit on error`, `modules/pymol/invocation.py:88`)
   the same run produced **zero output and exit 0**. Upstream CI's
   `pymol -ckqy testing/testing.py --run all` (`build.yml:53`, `:113`, `:171`) is therefore
...
8. **Editable install writes a `.so` into the source tree.** After
   ...
   *.d
   ...
   untracked 9.3 MB binary in `modules/pymol/`. Since we must not edit upstream files,
   fix this in `.git/info/exclude` (local, untracked by git) — see §5.6.

9. **`PYMOL_PATH` moves under editable installs.** `guess_pymol_path()`
   (`modules/pymol/__init__.py:177-204`) resolves `PYMOL_PATH` to the *parent of `modules/`*
   ...
   derive from it (`modules/pymol/__init__.py:207-211`). Anything in the bridge that
...
|---|---|---|---|
| `--glut` | `False` | `True` links `-framework GLUT` (`setup.py:679-681`) and **omits** `_PYMOL_NO_MAIN` (`setup.py:652-655`), enabling the legacy GLUT main loop in `layer5/main.cpp`. | **keep `false`** — we never want an OS window |
...
| `--openvr` | `False` | adds `contrib/vr` (10 files) + `openvr_api` | leave |
...
- `modules/pymol/pymolhttpd.py` (529 lines). `class PymolHttpd` at line **441**,
...
- `modules/pymol/rpc.py` (474 lines) — XML-RPC server, `launch_XMLRPC(hostname='',
  ...
  Enabled by the `-R` CLI flag (`modules/pymol/invocation.py:75`).
- `modules/web/javascript/pymol.js` + `json2.js`, and 16 sample apps under
  `modules/web/examples/`. This is 2009-era JS; treat as **reference for the JSON envelope
  shape only**.
- The `pwg` file format (`modules/pymol/invocation.py:103`, `get_pwg_options` at line 191)
...
`README*`, `.clang-format`, `.gitattributes`, everything under `layer*/`, `modules/`,
`contrib/`, `data/`, `include/`, `ov/`, `test/`, `testing/`, `examples/`, `_custom_build/`.
...
├── setup.py  pyproject.toml  CMakeLists.txt  layer0..layer5/  modules/  data/  ...
├── docs/
...
└── webclient/                           # NEW — the entire web world lives here
    ...
    │   │   # shader ports of data/shaders/*. MUST NOT contain any atom->representation
    │   │   # logic.
    │   ├── ui/                           # @tenmol/ui — shared React primitives/tokens
    │   └── tsconfig/                     # @tenmol/tsconfig — shared tsconfig presets
    │
    ├── services/
    │   └── bridge/                       # the Python side. NOT a pnpm package.
...
4. **PyMOL's own build artifacts** (`build/`, `build/generated/`, and the editable-install
   `modules/pymol/_cmd*.so`) are handled by `.git/info/exclude` (§5.6) — a local file git
   never merges.
...
# PREFIX_PATH, and layer3/MoleculeExporter.cpp:16 then needs <mmtf.hpp>, which no
# package manager ships. Vendor them OUT OF TREE and add to PREFIX_PATH, so the
# upstream include/ dir stays pristine.
...
for pat in "/webclient/node_modules/" "/webclient/.venv/" "/webclient/.deps/" \
           "/webclient/**/dist/" "modules/pymol/*.so" "modules/chempy/champ/*.so" \
...
on:
  push:
    paths: ["webclient/**", ".github/workflows/webclient.yml", "docs/webclient/**"]
  pull_request:
    paths: ["webclient/**", ".github/workflows/webclient.yml", "layer*/**", "modules/**", "setup.py"]
    ...
    steps:
      ...
      - name: Upstream PyMOL suite (non-blocking on macOS image tests)
        ...
        run: |
          ...
          set -o pipefail
          python -m pymol -ckq testing/testing.py --run all 2>&1 | tee /tmp/suite.log
...
- The `bridge` job's PyMOL build is the long pole. ~110 CPU-seconds of compilation; on a
  ...
  cache the built wheel keyed on `hashFiles('layer*/**','ov/**','contrib/**','setup.py','CMakeLists.txt')`.
...
2. **`use-msgpackc=no` silently amputates features.** It disables MMTF *and* BCIF
   (`layer2/CifFile.cpp:26`, `:491`; `layer3/MoleculeExporter.cpp:1510`, `:1991`, `:1999`;
   `layer2/CifMoleculeReader.cpp:3455`; `layer1/P.cpp:2127`). If the product needs to open
...
3. **The bridge holds a `pymol2.PyMOL` singleton with `os.chdir`-style global state.**
   `pymol2_lock` is an `RLock` (`modules/pymol2/__init__.py:22`) and the build itself
...
3. Does the bridge run PyMOL **in-process** (import `pymol2` inside the ASGI app) or as a
   **subprocess** the ASGI app talks to? In-process is simpler and matches
   `modules/pymol/pymolhttpd.py:443` (`self_cmd=`), but a hard C++ crash then takes the HTTP
   server down with it. I lean in-process + a supervisor that restarts.
4. Do we adopt the existing `PymolHttpd` JSON envelope (`modules/pymol/pymolhttpd.py:144-173`)
   for compatibility with `modules/web/javascript/pymol.js`, or design a fresh binary-capable
...
7. Should `docs/webclient/` and `webclient/` be merged into one directory to keep the

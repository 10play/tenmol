...
Area owner doc. Scope: `modules/pymol/wizard/` (every file), `modules/pymol/wizarding.py`,
`layer1/Wizard.{h,cpp}`, the wizard prompt renderer in `layer1/Ortho.cpp`, the pop-up menu
renderer in `layer4/PopUp.cpp`, wizard event dispatch sites in `layer1/SceneMouse.cpp`,
`layer3/Seeker.cpp`, `layer3/Executive.cpp`, `layer5/PyMOL.cpp`, and the Wizard menubar in
...
(`CWizard::Wiz`, `layer1/Wizard.cpp:69`). It is *not* a widget. The C++ core pulls a
...
wizards — plus the 15 builder wizards in `modules/pmg_qt/builder.py` and any third-party
...
|---|---|---|
| `get_prompt()` | `layer1/Wizard.cpp:205` | `list[str]` or `None` |
| `get_panel()` | `layer1/Wizard.cpp:227` | `list[[int type, str text, str code]]` or `None` |
| `get_event_mask()` | `layer1/Wizard.cpp:220` | `int` bitmask |
| `get_menu(tag)` | `layer1/Wizard.cpp:501` | `list[[int code, str text, str|list|callable]]` or `None` |
| `cleanup()` | `layer1/Wizard.cpp:275` | ignored |

The event methods (all invoked via `WizardCallPython`, which first does
`PyObject_HasAttrString` — `layer1/Wizard.cpp:162` — so **every method is optional**):

| Method | C entry point | Guarded by mask bit |
|---|---|---|
| `do_pick(bondFlag:int)` | `WizardDoPick`, `layer1/Wizard.cpp:303` | `cWizEventPick` = 1 |
| `do_pick_state(state:int)` | `layer1/Wizard.cpp:324` and `:189` | (rides on pick/select) |
| `do_select(name:str)` | `WizardDoSelect`, `layer1/Wizard.cpp:172` | `cWizEventSelect` = 2 |
| `do_key(k,x,y,mod)` | `WizardDoKey`, `layer1/Wizard.cpp:328` | `cWizEventKey` = 4 |
| `do_special(k,x,y,mod)` | `WizardDoSpecial`, `layer1/Wizard.cpp:462` | `cWizEventSpecial` = 8 |
| `do_scene()` | `WizardDoScene`, `layer1/Wizard.cpp:396` | `cWizEventScene` = 16 |
| `do_state(state:int)` | `WizardDoState`, `layer1/Wizard.cpp:428` | `cWizEventState` = 32 |
| `do_frame(frame:int)` | `WizardDoFrame`, `layer1/Wizard.cpp:445` | `cWizEventFrame` = 64 |
| `do_dirty()` | `WizardDoDirty`, `layer1/Wizard.cpp:412` | `cWizEventDirty` = 128 |
| `do_view()` | `WizardDoView`, `layer1/Wizard.cpp:371` | `cWizEventView` = 256 |
| `do_position()` | `WizardDoPosition`, `layer1/Wizard.cpp:344` | `cWizEventPosition` = 512 |

Python-side mirror of the mask constants: `modules/pymol/wizard/__init__.py:6-15`.
C-side mirror: `layer1/Wizard.cpp:49-58`. They agree exactly.

---

## 1. The base class

`modules/pymol/wizard/__init__.py:4` — `class Wizard`.
...
`modules/pymol/wizarding.py:176` `session_save_wizard` double-pickles
...
Registered as session tasks in `modules/pymol/cmd.py:50-54`.

---

## 2. The panel wire format (`get_panel`)

Parsed in `layer1/Wizard.cpp:227-252`. Each element must be a list of **≥3 items**
...
(`layer1/Wizard.cpp:254-259`); 0 lines → panel collapses to height 0 (`:258`).
`OrthoReshapeWizard` itself: `layer1/Ortho.cpp:2464`. Layout position (between the
Executive/object panel and the ButMode panel): `layer1/Ortho.cpp:2286-2300`.

Interaction detail worth cloning: pressing a button *highlights* it while held
(`I->Pressed`, `layer1/Wizard.cpp:490-494`), drag-off cancels the highlight
...
(`layer1/Wizard.cpp:575`). It is *not* JavaScript and must never be evaluated client-side.
...
Implementation: `TextStartsWithColorCode` `layer1/Text.cpp:507`, `TextSetColorFromCode`
`layer1/Text.cpp:530`. Consumers: `Wizard.cpp:670-681` (`draw_text`),
...
`WizardRefresh` calls `get_prompt()` (`layer1/Wizard.cpp:205`), converts the returned list
to a NUL-separated char VLA (`PConvPyListToStringVLA`, `:207`) and hands it to
`OrthoSetWizardPrompt` (`layer1/Ortho.cpp:314`).

Rendered by `OrthoDrawWizardPrompt` — `layer1/Ortho.cpp:2124`. Behaviour driven by the
global setting `wizard_prompt_mode` (`layer1/SettingInfo.h:461`, default `1`):
...
(`layer1/Wizard.cpp:501`), and if non-`None` opens `PopUpNew(G, x, my, x, y, false, menuList, nullptr)`
(`:507`).

Parsed in `layer4/PopUp.cpp:231-248`:
...
`modules/pymol/wizard/` that uses the *callable* form — that path is exercised by
`modules/pymol/menu.py`. Honest caveat: the bridge still must handle it because
...
- `cmd.wizard(name, *arg, **kwd)` — `modules/pymol/wizarding.py:62`. Legacy alias:
...
- `cmd.set_wizard(wizard=None, replace=0)` — `wizarding.py:110` → `_cmd.set_wizard`
  → `CmdSetWizard` `layer4/Cmd.cpp:2801` → `WizardSet` `layer1/Wizard.cpp:264`.
...
- `WizardUpdate` (`Wizard.cpp:101`) runs once per render pass from
  `layer3/Executive.cpp:11533` and additionally fires `do_dirty` / `do_frame` / `do_state` /
...
- `cmd.reinitialize()` clears the stack: `WizardSet(G, nullptr, false)` at
  `layer3/Executive.cpp:16586`.
- Python API export list: `modules/pymol/api.py:283-290`.
- Only `refresh_wizard` and `replace_wizard` are registered as command-language keywords
  (`modules/pymol/keywords.py:223,228`) — `wizard` itself is auto-registered elsewhere;
...
- Atom pick, `cButModePickAtom`: `layer1/SceneMouse.cpp:468` → `WizardDoPick(G, 0, state)`.
- Atom pick, editor path: `layer1/SceneMouse.cpp:427` → `WizardDoPick(G, 0, state)`.
- Bond pick / torsion: `layer1/SceneMouse.cpp:543` → `WizardDoPick(G, 1, state)`;
  `:545` is the non-bond fallback.
- Click-select (`cButModeSeleSet` etc.): `layer1/SceneMouse.cpp:135` and `:357` →
  `WizardDoSelect(G, selName, state)`.
- Rectangle/box select: `layer3/Executive.cpp:7548` → `WizardDoSelect(G, selName)`.
- Sequence-viewer select: `layer3/Seeker.cpp:150` and `:231` → `WizardDoSelect(G, selName)`.
- Keyboard: `layer5/PyMOL.cpp:2356` → `WizardDoKey`; `:2369` → `WizardDoSpecial`.
- Scene changed: `layer1/Scene.cpp:4812` → `WizardDoScene`.
- View/position: `layer1/Scene.cpp:4675-4676` → `WizardDoPosition` / `WizardDoView`.
...
Menubar entries (`modules/pymol/_gui.py:834-864`): Appearance, Measurement,
Mutagenesis▸(Protein | Nucleic Acids), Pair Fitting, ─, Density, Filter, Sculpting, ─,
Label, Charge, ─, Demo▸(11 demos + End Demonstration).
Context-menu launches: `modules/pymol/menu.py:1163, 1194, 1234, 1268, 1298, 1314, 1401,
...
in `modules/pymol/menu.py` (see §7 header list).
...
## 8. Related wizards outside `modules/pymol/wizard/`

`modules/pmg_qt/builder.py` defines **15 more `Wizard` subclasses** that use the identical
...
wizard.menu(tag: string) -> MenuItem[] | null
    ...
    #    (mirrors layer4/PopUp.cpp:88-105)
    # MenuItem = { code: 0|1|2, text: string, command?: string, submenu?: MenuItem[] }

wizard.exec(code: string) -> void
    # -> server-side cmd.do(code) / PParse equivalent.  layer1/Wizard.cpp:573-576
...
wizard.event(kind, payload) -> void
    ...
    #                                              layer1/Wizard.cpp:140-143, 177, 311, 331, ...
```
...
- `<WizardPanel/>` — renders `snapshot.panel`. One row component per type:
  ...
  Row height = `internal_gui_control_size` (mirror `layer1/Wizard.cpp:255`).
- `<WizardPrompt/>` — top-left viewport overlay, honours `wizard_prompt_mode` 0/1/2/3
  (`layer1/Ortho.cpp:2136-2190`), default text color `rgb(51,255,51)` and backdrop
  `rgb(51,51,51)` (`layer1/Ortho.cpp:2692-2697`).
- `<WizardPopupMenu/>` — codes 0/1/2 → separator / item / title
  (`layer4/PopUp.cpp:293-320`); nested `submenu` opens on hover; leaf runs
  `wizard.exec(command)`.
- `<ColorCodedText/>` — shared `\RGB` / `\---` parser (`layer1/Text.cpp:507-548`).
- `<WizardStack/>` — the panel stack; `snapshot.depth > 1` means nested wizards
  (`layer1/Wizard.cpp:280-282`).

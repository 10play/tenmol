...
| file | what it is |
|---|---|
| `modules/pmg_qt/builder.py` (1579 lines) | The Qt Builder panel + 13 "action wizard" classes. **Authoritative UI spec.** |
| `modules/pymol/editor.py` (1169 lines) | Python-level fragment/monomer attachment logic (`attach_fragment`, `attach_amino_acid`, `attach_nuc_acid`, `fab`, `fnab`). |
| `modules/pymol/editing.py` (3161 lines) | The `cmd.*` editing API (`edit`, `unpick`, `bond`, `unbond`, `valence`, `cycle_valence`, `replace`, `attach`, `remove_picked`, `h_fill`, `h_fix`, `h_add`, `fix_chemistry`, `invert`, `flag`, `reference`, `undo`, `redo`, `push_undo`, `sculpt_*`, `set_geometry`, `torsion`, `drag`). |
| `layer3/Editor.h`, `layer3/Editor.cpp` | The C++ editor state machine: `pk1..pk4`, `pkbond`, `pkset`, `pkmol`, `pkresi`, `pkchain`, `pkobject`, `_pkfrag*`, auto-measure, auto-dihedral. |
| `layer1/SceneMouse.cpp` | How a viewport click assigns the next `pkN` selection. |
| `modules/pymol/controlling.py` | `cmd.edit_mode`, `cmd.button` (mouse action codes `PkAt`/`PkBd`/`MovA`). |
| `modules/pymol/creating.py:943` | `cmd.fragment` — fragment library loader. |
| `modules/pymol/computing.py:20` | `cmd.clean` — **raises `IncentiveOnlyException` in this open-source tree.** |
| `modules/pymol/wizard/__init__.py` | Wizard base class contract (`get_prompt`, `get_panel`, `do_pick`, `do_select`, `cleanup`). |
| `data/chempy/fragments/*.pkl` (131 files) | The fragment library on disk. |
| `data/pmg_tk/bitmaps/builder/*.gif` (10 files) | Ring icons: `cyc3..cyc7`, `aro5`, `aro6`, `aro65`, `aro66`, `aro67`. |
| `modules/pmg_tk/skins/normal/builder.py` (1507 lines) | **Legacy Tk builder. Do NOT port.** It has no Nucleic Acid tab and an older AminoAcidWizard. The Qt file supersedes it. |
...
2. **Picking.** The editor's `pk1..pk4` selections are created by the C++ scene-click handler (`layer1/SceneMouse.cpp:429 cButModePickAtom`). Client-side WebGL picking must round-trip an `(object, atom_index)` to the backend, which then calls the same `EditorSelect` path (exposed as `cmd.edit(...)`). See §7.
...
Geometry codes come from `layer2/AtomInfo.h:129-133`: `1=Single, 2=Linear, 3=Planar, 4=Tetrahedral, 5=None`.
...
All 23 fragment `.pkl` files referenced by the Chemical tab were verified present in `data/chempy/fragments/`.
...
- Panel format from `Wizard.get_panel()` (`modules/pymol/wizard/__init__.py:50`): a list of `[type, text, command]`; `type==1` is a title row, `type==2` is a clickable button whose `command` is a **PyMOL command string** evaluated by the engine. Prompt from `get_prompt()` is a list of strings drawn in the viewport.
...
- ⚠ **`cmd.clean` raises `pymol.IncentiveOnlyException` in this tree (`modules/pymol/computing.py:20-29`).** The Clean button is dead in open-source PyMOL. Web client must either hide it, or surface the exception cleanly, or the team must supply an MMFF94 minimizer.
...
- Backend errors from `EditorInvert` (`layer3/Editor.cpp:634-638`): *"Must pick atom to invert as pk1"*, *"Must pick immobile atom in pk2"*, *"Must pick immobile atom in pk3"*.
...
### 7a. Reserved selection names (`layer3/Editor.h:30-48`)

| macro | name | meaning |
|---|---|---|
| `cEditorSele1..4` | `pk1`, `pk2`, `pk3`, `pk4` | the picked atoms, in click order |
| `cEditorSet` | `pkset` | union of all picked atoms (built by `SelectorSubdivide`, `layer3/Selector.cpp:4548`) |
...
### 7b. Pick assignment order (`layer3/Editor.cpp:499-536 EditorGetNextMultiatom`)
...
### 7c. Scene click → editor (`layer1/SceneMouse.cpp:404-470`)
...
### 7d. `EditorActivate` (`layer3/Editor.cpp:1786-1830`)
...
### 7f. Mouse modes touched by the Builder (`modules/pymol/controlling.py:57-125`)
...
1. `builder.py:417` calls `editor.combine_monomer()` — **this function does not exist** in `modules/pymol/editor.py` (grep-verified; only wizard *methods* named `combine_monomer` exist at `builder.py:402/491/511`). `BioPolymerWizard.do_pick` mode 1 therefore raises `AttributeError`.
...
15. `data/chempy/fragments/utpB.pkl` does **not** exist (all other `{a,c,g,t,u}tp{A,B}` and duplex fragments do). Only reachable if RNA form B is ever allowed; `attach_nuc_acid` currently forces RNA→form A (`editor.py:803-805`), so it is latent.

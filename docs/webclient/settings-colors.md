...
Read-only survey — no source outside `docs/webclient/` was touched.
...
`layer1/SettingInfo.h` is included twice (`layer1/Setting.h:242` for the enum,
`layer1/Setting.cpp:76-78` with `SETTINGINFO_IMPLEMENTATION` for the data table).

Record macros — `layer1/SettingInfo.h:15-23`:
...
Type constants are mirrored in Python at `modules/pymol/setting.py:19-26` and defined in
`layer1/Setting.h:113-120`. `cSetting_tuple = -1` is a pseudo-type used only as an argument to
`_cmd.get_setting_of_type` to request `(type, value)` tuples.

**Exact counts (computed from `layer1/SettingInfo.h`, 798 `REC_*` rows, indices 0..797):**

- 798 total records; index range `0 .. 797`; `cSetting_INIT = 798` (`layer1/SettingInfo.h:913`).
- By type: 219 boolean, 203 int, 294 float, 15 float3, 34 color, 32 string, 1 blank
  (`REC__( 83, )` at `layer1/SettingInfo.h:167`).
...
- **779 settings are visible to Python.** `SettingGetSettingIndices()`
  (`layer1/Setting.cpp:3227-3245`) skips every record whose level is `cSettingLevel_unused`,
  so the 18 retired settings + 1 blank never appear in `index_dict`.
- `modules/pymol/setting.py:48` then injects one legacy alias, `ray_shadows -> ray_shadow`, into
...
- Indices are frozen forever (they are serialized into `.pse` sessions) —
  `layer1/SettingInfo.h:5-6` "WARNING: do not delete or change indices"; regression-tested at
  `testing/tests/api/setting.py:5-8` (`collada_geometry_mode == 742`).
- The last three added settings are 795 `salt_bridge_distance`, 796 `use_tessellation_shaders`,
  797 `cell_color` (`layer1/SettingInfo.h:895-908`).

**No documentation strings live in the C table.** There is a `data/setting_help.csv` (875 rows,
...
`layer1/Setting.h:325-334` defines the enum; `layer1/Setting.cpp:64-74` defines the mask table:
...
Comment at `layer1/Setting.cpp:55-63` states the hierarchy:
...
- `SettingLevelGetName(index)` → level name string (`layer1/Setting.cpp:83-85`).
- `SettingLevelCheckMask` / `SettingLevelCheck` (`layer1/Setting.cpp:88-96`) validate that a
  setting may be written at a requested level.
- Writing at a wrong level is a **warning, not an error**: `layer3/Executive.cpp:12279-12286`
  prints `" Setting-Warning: '%s' is a %s-level setting"` and still performs the write (which then
  silently has no effect). `modules/pymol/setting.py:230-247` documents the per-atom whitelist and
  `setting.py:148-155` the per-bond whitelist as prose only.
- `alter`/`alter_state` enforce hard: `layer1/P.cpp:523-528` (astate) and `P.cpp:531-536` (atom)
  raise `TypeError`.

### A3. Runtime introspection surface exposed to Python

C entry points registered in `layer4/Cmd.cpp:6492-6496`:

| `_cmd` function | C impl | signature | returns |
|---|---|---|---|
| `get_setting_indices()` | `layer4/Cmd.cpp:2080-2083` → `layer1/Setting.cpp:3227` | `()` | `{name: index}` dict, 779 entries |
| `get_setting_type(index)` | `layer4/Cmd.cpp:4394-4401` | `(int)` | int type code |
| `get_setting_level(index)` | `layer4/Cmd.cpp:4403-4409` | `(uint)` | level **name string** |
| `get_setting_of_type(COb, index, object, state, type)` | `layer4/Cmd.cpp:4411-4422` → `layer3/Executive.cpp:7995` | | typed value |
| `get_setting_updates(COb, name, state)` | `layer4/Cmd.cpp:2058-2078` → `layer1/Setting.cpp:1121` | | list of changed indices |

**`get_setting_level` is exported by C but never wrapped in `modules/pymol/setting.py`** — it is
...
Python wrappers in `modules/pymol/setting.py` (re-exported into `cmd` at
`modules/pymol/api.py:415-432`):
...
Value coercion rules — `modules/pymol/setting.py:83-112`:
...
- float3: list/tuple passthrough; string with commas goes through `cmd.safe_eval`
  (`modules/pymol/constants.py:106-117`), otherwise whitespace split; always returns 3 floats.
- color: `str(value)` — resolution happens in C (`SettingSet_color`, `layer1/Setting.cpp:1728-1760`).
- string: outermost matching quotes stripped.

### A4. Read/write semantics the web client must reproduce

**Resolution order.** `_SettingGetFirstDefined` (`layer1/Setting.cpp:3272-3281`): `set1` (most
specific) → `set2` → `G->Setting` (global). `ExecutiveGetSettingOfType`
(`layer3/Executive.cpp:7995-8021`) resolves `object` → object CSetting, `state >= 0` → object-state
CSetting, and errors `object "%s" not found` / `object "%s" lacks state %d`.

**Return shapes** — `SettingGetTuple` (`layer1/Setting.cpp:1448-1479`):
- boolean/int/color → `(type, int)`
- float → `(type, float)` via `pretty_f2d` (7 significant digits, `testing/tests/api/setting.py:49-55`)
- float3 → `(type, (f,f,f))`
- string → `(type, str)`
- blank → `None`

`SettingGetPyObject` (`layer1/Setting.cpp:1405-1443`) is different again: **color settings return an
RGB triple**, not an index (`Setting.cpp:1428-1437`), and only when `index > 0`.

**Text representation** — `SettingGetTextPtr` (`layer1/Setting.cpp:1183-1237`):
...
table displays for float/float3/color (`modules/pmg_qt/advanced_settings_gui.py:63-64`).

**Min/max.** Only 30 records carry min/max, and **only int settings are actually clamped**, and only
for global writes: `layer1/Setting.cpp:1890-1911` — `rec.type == cSetting_int && rec.hasMinMax() &&
...
(`layer1/SettingInfo.h`, rows 46, 87, 131, 157, 180, 188, 189, 326, 335, 421, 438, 455, 577, 667,
689, 690, 693, 706, 713, 721, 727, 753, 754, 768, 771, 776, 777, 787).

**Side effects.** `SettingGenerateSideEffects` (`layer1/Setting.cpp:1872-…`) is a giant `switch`
...
(`layer1/Setting.cpp:1876-1887`). **Implication for the web client: after any `set`, geometry that
...
`SettingGetUpdateList` (`layer1/Setting.cpp:1121-1147`) walks all `cSetting_INIT` records, collects
...
The Qt GUI is the only consumer today: `modules/pmg_qt/pymol_qt_gui.py:952-956` polls it inside
...
(`wrapper.SettingWrapper`, `layer1/P.cpp:393`):

- `s[key]` read — `layer1/P.cpp:455-486`. Key may be an int index or a name string
...
- `s[key] = v` write — `layer1/P.cpp:502-539`; read-only outside `alter`/`alter_state`
  (`P.cpp:512-516`), and level-checked as noted in A2.
- `iter(s)` — `layer1/P.cpp:581-606` → `SettingUniqueGetIndicesAsPyList`
  (`layer1/Setting.cpp:3249-3268`): yields the **indices** actually defined on that atom/atom-state.
...
- Storage is the `CSettingUnique` sparse map keyed by `unique_id`
  (`layer1/Setting.h:34-50`; `layer2/AtomInfo.cpp:74-95`; `layer2/CoordSet.cpp:1704-1719`).
- Bond level is read back via `cmd.get_bond` (`modules/pymol/setting.py:449-514`,
  C at `layer3/Executive.cpp:11843`, level gate at `Executive.cpp:11969`).
- `unset_deep` docstring (`modules/pymol/setting.py:519-526`) documents the escape hatch for
  atom-state settings: `alter_state 1, *, del s[728]` (728 = `label_screen_point`).

### A7. Session / defaults lifecycle

- Blacklist for `.pse` persistence — `layer1/Setting.cpp:627-660`: `unused`-level plus
...
- `SettingInitGlobal` (`layer1/Setting.cpp`, after line 1180) resets from `SettingInfo`, honours
...
- `cmd.reinitialize(what)` with `reinit_code` (`modules/pymol/commanding.py:350-356`):
  `everything=0, settings=1, store_defaults=2, original_settings=3, purge_defaults=4`.
  Surfaced in the File menu (`modules/pymol/_gui.py:128-131`).
- `SettingStoreDefault` / `SettingPurgeDefault` / `SettingRestoreDefault`
  (`layer1/Setting.h:161-162`, `layer1/Setting.cpp:1528-1573`).
...
`modules/pymol/_gui.py:55-58` `PyMOLDesktopGUI.get_menudata(cmd)` returns a pure nested list; both
the Qt window (`modules/pmg_qt/pymol_qt_gui.py:353-357`) and the Tk skin
(`modules/pmg_tk/skins/normal/__init__.py:1072-1075`) consume it. Item grammar
...
`SettingAction` (`modules/pmg_qt/pymol_qt_gui.py:1041-1080`) builds a checkable action for types
...
### B2. Full enumeration — the `Setting` menu (`modules/pymol/_gui.py:492-773`)
...
- command **Modernize** → `cmd.util.modernize_rendering(1, cmd)` (`modules/pymol/util.py:553`)
- *Shadows*: 9 commands → `cmd.util.ray_shadows(v)` (`modules/pymol/util.py:821`) for
...
### B3. Settings that also live in the `Display` menu (`modules/pymol/_gui.py:377-491`)
...
### B4. Advanced settings table (`modules/pmg_qt/advanced_settings_gui.py`)
...
Legacy Tk equivalent for reference: `modules/pmg_tk/SetEditor.py` — 15 visible rows scrolled by a
...
### B5. Lighting Settings plugin (`data/startup/lightingsettings_gui/main.py`)

A second, curated settings panel shipped as a startup plugin (menu entry registered at
`data/startup/lightingsettings_gui/__init__.py:14-15`). It is the model for "grouped slider panel"
in the web UI.
...
`ColorReset` (`layer1/Color.cpp:825-1322`) rebuilds `CColor::Color` from scratch. Registration is
the `reg_named_color(name,R,G,B)` macro (`layer1/Color.cpp:1017-1022`) which appends to the vector
and registers the name in `CColor::Idx` (`layer1/Color.h:85`).
...
(`layer1/Color.cpp:35-75`) and `_color_cycle` (`modules/pymol/util.py:27-70`).

`ColorGetStatus` (`layer1/Color.cpp:784-807`) returns 1 for names with no digits, −1 for names
...
(`layer4/Cmd.cpp:1340-1381`).

### C2. Special / encoded color indices

`layer1/Color.h:36-47`:
...
`ColorGetIndex` (`layer1/Color.cpp:661-750`) resolution order: all-digit string → literal index or
...
`ColorGetName` (`layer1/Color.cpp:759-782`) inverts this, printing `0x%06x`/`0x%08x` for TRGB
...
exposes and what `ramp_new` uses (`modules/pymol/creating.py:472`).
`ColorGetEncoded` (`Color.cpp:1847`) and `Color3fToInt` (`Color.cpp:1883`) do the 0x40RRGGBB
packing used by `spectrumany` (`modules/pymol/viewing.py:2053`: `0x40000000 + r*0x10000 + g*0x100 + b`).
...
| API | source | notes |
|---|---|---|
| `cmd.get_color_indices(all=0)` | `modules/pymol/querying.py:843-849` | `[(name, index), ...]`; 178 or 5388 entries |
...
| `cmd.set_object_color(name, color)` | `modules/pymol/editing.py:2866-2874` | |
| `cmd.set_color(name, rgb, mode=0, quiet=1)` | `modules/pymol/viewing.py:2153-2211` | accepts a string that `safe_list_eval`s; auto-detects 0–1 vs 0–255 by "any component > 1.0" (viewing.py:2205-2207); calls `_cmd.colordef` then `_invalidate_color_sc` |
...
| `cmd.space(space, gamma)` | `modules/pymol/importing.py:227-288` | color LUT |
| `cmd.get_colorection(key)` / `set_colorection(dict,key)` / `del_colorection` | `viewing.py:907-918` | scene color snapshots |
| `cmd.ramp_new / ramp_update` | `modules/pymol/creating.py:374-494` | |
| `cmd.volume_color / get_volume_color / volume_ramp_new / volume_panel` | `modules/pymol/colorramping.py:56-227` | |
...
(`modules/pymol/internal.py:575-586`), `_interpret_color` resolves user input
...
(`modules/pymol/cmd.py:382`); completion hooks are at `modules/pymol/completing.py:96,103-104`.

### C4. Palettes and spectrum

`modules/pymol/constants_palette.py:1-89` — `palette_dict`, **57 palettes**, each
...
`palette_sc = Shortcut(palette_dict.keys())` at `modules/pymol/constants.py:223`.
...
- Ramps are real objects (`object:ramp`) registered as color "extensions":
  `ColorRegisterExt` (`layer1/Color.cpp:347-365`), `ColorForgetExt` (`Color.cpp:367`),
...
- `cmd.ramp_new(name, map_name, range, color, state, selection, beyond, within, sigma, zero, quiet)`
  — `modules/pymol/creating.py:374-490`. `color` may be a list of names/tuples (each resolved with
...
- Volume ramps are separate: `modules/pymol/colorramping.py:17-54` `namedramps` presets
...
### C7. The internal-GUI "C" (color) menu — `modules/pymol/menu.py`
...
**Qt** — `PyMOLQtGUI.edit_colors_dialog` (`modules/pmg_qt/pymol_qt_gui.py:547-611`) using
`modules/pmg_qt/forms/colors.ui`. Widgets: `list_colors` (QListWidget, sorting enabled,
...
**Tk (legacy)** — `modules/pmg_tk/ColorEditor.py`: `ColorEditor` (81-138) is a
...
`cmd.space(space, gamma)` (`modules/pymol/importing.py:227-288`) maps through
`constants.space_dict` (`modules/pymol/constants.py:141-148`):
...
(`layer1/Color.cpp:1324-…`). `'rgb'` purges the table; `'greyscale'` and `'pymol'` synthesize a
...
`ColorRec::LutColor/LutColorFlag` (`layer1/Color.h:58-59`) hold the mapped value, so
...
1. Add a small C accessor beside `CmdGetSettingLevel` (`layer4/Cmd.cpp:4403`) exporting
...
3. Parse `layer1/SettingInfo.h` + `data/setting_help.csv` at build time into a static JSON asset
...
| `settings.atomOverrides(selection)` | `cmd.iterate(sel, 'r.append((model, index, list(s)))')` (`layer1/P.cpp:581`) | returns defined per-atom indices |
...
| `<LightingPanel>` | `data/startup/lightingsettings_gui/main.py` | grouped sliders + 5 presets |
...
1. `cmd.get_setting_updates()` clears the `changed` flags as it reads (`layer1/Setting.cpp:1128-1132`).
...
2. Defaults, min/max and help text are **not exposed to Python** anywhere
   (`SettingInfo` is `static` in `Setting.cpp`; `data/setting_help.csv` has zero consumers). Without
   a new accessor or a build-time extraction, the advanced table cannot show "default" or validate ranges.
3. `get_setting_level` exists in C (`layer4/Cmd.cpp:4403`) but has no Python wrapper — must be
   called as `cmd._cmd.get_setting_level(i)`, which is undocumented/private and could change.
4. Setting indices are session-file ABI (`layer1/SettingInfo.h:5-6`). Any cached client-side
...
5. Wrong-level writes are silently accepted with only a console warning
   (`layer3/Executive.cpp:12279-12286`). The web UI must gate by level client-side or users will see
...
11. Color name lookup falls back to **case-insensitive prefix matching** in C
    (`layer1/Color.cpp:736-748`), which the source itself questions ("TODO does this even make
...
13. `SettingGenerateSideEffects` can invalidate reps and force geometry rebuilds
    (`layer1/Setting.cpp:1872+`). Setting changes are not cosmetic — the three.js scene may need a
...
- Should `data/setting_help.csv` (875 rows, currently orphaned) become the tooltip source, and who

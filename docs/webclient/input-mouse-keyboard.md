...
| File | Role |
|---|---|
| `modules/pmg_qt/pymol_gl_widget.py` | Qt widget: mouse/wheel/gesture → `pymol.button/drag`, DPR handling, reshape |
| `modules/pmg_qt/keymapping.py` | Qt key/modifier/wheel → PyMOL key codes |
| `modules/pmg_qt/pymol_qt_gui.py:50` | Window-level `keyPressEvent` → `pymol.button(...)` |
| `modules/pmg_qt/shortcut_menu_gui.py` | "Keyboard Shortcut Menu" dialog (415 lines) |
| `modules/pymol/controlling.py` | `button()`, `mouse()`, `config_mouse()`, `set_key()`, `edit_mode()`, `mask()`, all mouse-mode tables |
| `modules/pymol/internal.py:388-511` | `_special`, `_ctrl`, `_alt`, `_ctsh`, `_cmmd`, `_invoke_key`, `special_key_codes`, `modifier_keys` |
| `modules/pymol/keyboard.py` | `editing_ring` (cut/copy/paste/invert), `get_default_keys()` |
| `modules/pymol/shortcut_dict.py` | The 100+ default key bindings table |
| `modules/pymol/shortcut_manager.py` | Reconciles defaults / user / saved bindings; reserved keys |
| `modules/pymol/shortcut.py` | `Shortcut` prefix-matcher (used for `button`/`action`/`ring` name abbreviation) |
| `modules/pymol/save_shortcut.py` | Persists to `~/.pymol/shortcuts_save.json` |
| `layer4/Cmd.cpp:3626/3646/3569/3665` | `_button`, `_drag`, `_reshape`, `_sdof` Python entry points |
| `layer5/PyMOL.cpp:2353/2361/2397/2890/2908/2979` | `PyMOL_Key`, `PyMOL_Special`, `PyMOL_Reshape`, `PyMOL_Drag`, `PyMOL_Button`, `PyMOL_SetDefaultMouse` |
| `layer1/Ortho.cpp:2493/2575/841/322` | `OrthoButton`, `OrthoDrag`, `OrthoKey`, `OrthoSpecial` (block hit-test, grab, CLI keys) |
| `layer1/ButMode.cpp` / `.h` | The 80-slot button→action table, `ButModeTranslate`, action code labels |
| `layer1/SceneMouse.cpp` (2036 lines) | `SceneClick` / `SceneDrag` / `SceneRelease` — all viewport semantics |
| `layer1/ScenePicking.cpp:17` | `SceneDoXYPick` — GPU color-pick + `glReadPixels` |
| `layer1/Control.cpp` | Movie control block (mouse), SDOF/spaceball, `ControlRock` |
| `layer3/Executive.cpp:7417` | `ExecutiveSelectRect` — box-select semantics |
...
Qt event
  ...
  → pymol2 SingletonPyMOL.button/drag                          (modules/pymol2/__init__.py:46,49)
  → _cmd._button / _cmd._drag                                  (layer4/Cmd.cpp:3626, 3646)
  → PyMOL_Button / PyMOL_Drag                                  (layer5/PyMOL.cpp:2908, 2890)
  → OrthoButton / OrthoDrag                                    (layer1/Ortho.cpp:2493, 2575)
  → Block::click/drag/release of the hit block                 (Ortho.cpp:2536 findBlock)
  → CScene::click → OrthoDefer(...) → SceneClick(...)          (layer1/Scene.cpp:4113-4155)
```
...
* **All scene mouse handling is deferred.** `CScene::click/drag/release` only push a lambda onto
  `OrthoDefer` (`layer1/Scene.cpp:4113`, `4118`, `4129`, `4146`); the actual `SceneClick`/`SceneDrag`/
...
* **`PyMOL_Button` multiplexes keys and mouse** on the `state` argument
  (`layer5/PyMOL.cpp:2896-2917`): `state == -1` → `PyMOL_Key` (ascii), `state == -2` →
  `PyMOL_Special` (GLUT special code), `state == 0` → mouse down, `state == 1` → mouse up.
* **Modifier mask** is `SHIFT=1, CTRL=2, ALT=4` (`layer1/Ortho.h:20-22`), built on the Qt side by
  `get_modifiers` (`modules/pmg_qt/keymapping.py:44-58`); note Qt **Meta** (⌘ on macOS) is folded into
  the same bit as Control (`keymapping.py:51-52`).
* **Buttons** are `left=0, middle=1, right=2`, wheel-forward=3, wheel-backward=4
  (`pymol_gl_widget.py:45-49`, `layer0/os_gl_glut.h:21-22`), plus synthetic
  `SINGLE_LEFT=100, SINGLE_MIDDLE=101, SINGLE_RIGHT=102, DOUBLE_LEFT=200, DOUBLE_MIDDLE=201,
  DOUBLE_RIGHT=202` (`layer0/os_gl_glut.h:23-28`).

---

## 2. Coordinate system: y-flip, framebuffer scale, reshape

`modules/pmg_qt/pymol_gl_widget.py:170-176`:
...
3. `fb_scale` is also pushed into the engine as an integer setting:
   ...
   `_gScaleFactor` (`layer1/Setting.cpp:2946-2951`) and hence `DIP2PIXEL()`
   (`layer0/PyMOLGlobals.h:28-29`), which sizes **every internal-GUI hit rectangle** (e.g.
   `cControlBoxSize DIP2PIXEL(17)` in `layer1/Control.cpp:36`, `SceneScrollBarWidth DIP2PIXEL(13)` in
   `layer1/Scene.h:47`). Changing it triggers `OrthoCommandIn(G, "viewport")`
   (`layer1/Setting.cpp:2951`).
4. **Reshape** is `pymol.reshape(w, h, force)` → `_cmd._reshape` → `PyMOL_Reshape`
   (`layer5/PyMOL.cpp:2397-2405`), which sets `G->Option->winX/winY` then `OrthoReshape`.
...
(`layer1/Ortho.cpp:2513-2521`, helper at `:204-231`), and `SceneClick`/`SceneDrag` do the same with
`get_stereo_x` (`layer1/SceneMouse.cpp:599-627`) for side-by-side stereo modes. For the web client
...
`OrthoButton` (`layer1/Ortho.cpp:2493-2557`):
...
* Blocks attached: Scene (`cOrthoScene`, `layer1/Scene.cpp:4243`), and as `cOrthoTool`: ButMode
  (`layer1/ButMode.cpp:575`), Control (`layer1/Control.cpp:862`), Movie (`layer1/Movie.cpp:56`),
  Seq (`layer1/Seq.cpp:710`), Wizard (`layer1/Wizard.cpp:835`), Executive/object panel
  (`layer3/Executive.cpp:16671`), PopUp menus (`layer4/PopUp.cpp:263`), plus hidden Pop
  (`layer1/Pop.cpp:71`).
* `OrthoGrab`/`OrthoUngrab` (`:1191`, `:1215`) implement pointer capture. `OrthoFakeDrag`
  ...
  it is triggered from Python via `cmd._fake_drag` (`modules/pymol/cmd.py:171`, `layer4/Cmd.cpp:540`).
...
(`layer1/ButMode.h:216`). Slot layout (`ButMode.h:118-214`):
...
### 4.2 `ButModeTranslate(G, button, mod)` (`layer1/ButMode.cpp:603-757`)
...
From `layer1/ButMode.h:23-113` and the labels in `ButModeInit` (`layer1/ButMode.cpp:500-555`), and
the Python names in `modules/pymol/controlling.py:57-123`:
...
`modules/pymol/controlling.py:799-868`. Names are resolved through the abbreviation matcher
`Shortcut` (`modules/pymol/shortcut.py:20-203`), so `cmd.button('l','shft','+Box')` works.
...
(`layer1/ButMode.h:225`) but is **NOT exposed to Python**. The web UI must therefore mirror the
...
* `cmd.edit_mode(active)` (`controlling.py:688-717`) is the legacy toggle between
  ...
  (`modules/pmg_qt/builder.py:1341`).
...
* The in-viewport ring cycler: clicking the ButMode block cycles modes; clicking its top two lines
  ...
  right-button or scroll-backward reverses direction (`layer1/ButMode.cpp:147-188`).
* `mouse_config` menu items (`modules/pymol/menu.py:82-101`): 3-Button Motions / 3-Button Editing /
...
Transcribed verbatim from `modules/pymol/controlling.py:234-548`. `w` = wheel.
...
Which light is edited is `edit_light` clamped 1..9 (`layer1/SceneMouse.cpp:1973-1974`, `:1998-1999`).
...
### 7.11 `default` (`:535-547`) — mirrors `PyMOL_SetDefaultMouse` (`layer5/PyMOL.cpp:2979-3021`)
...
`cButModePotentialClick` (`layer5/PyMOL.cpp:3011-3019`), and `FB_Scene` results feedback is masked off
...
* **Deferred single-click dispatch** (`layer1/Scene.cpp:2439-2450`): in `SceneIdle`, once
...
## 9. Drag semantics per action (`SceneDrag`, `layer1/SceneMouse.cpp:1205-2036`)

* **`rota` (RotXYZ)** — virtual trackball, `scale = 0.45 * min(W,H)` (`:1312-1315`).
  `virtual_trackball` (default 1, `layer1/SettingInfo.h:414`) selects three formulations
...
* Entered from `cButModeRectAdd/RectSub/Rect/SeleAddBox/SeleSetBox/SeleSubBox` →
  ...
  `OrthoSetLoopRect(G, true, rect)` (`layer1/Ortho.cpp:253-260`) and `OrthoGrab`.
...
* `ExecutiveSelectRect` (`layer3/Executive.cpp:7417-7500+`) runs `SceneMultipick`
  (`layer1/ScenePicking.cpp:332`) over the rect, creates `_tmp_rect_sele`, then set/add/subtract
...
`SceneDoXYPick(G, x, y, click_side)` (`layer1/ScenePicking.cpp:17-38`) does a **GPU color-index
...
`SceneGetSeleModeKeyword` (`layer1/Scene.cpp:504-510`) maps the setting to a selection-expansion
...
Labels at `layer1/ButMode.cpp:370-392`; default 1 at `SettingInfo.h:449`. Cycled by
...
* `drgm` / `drgo` — issue `cmd.drag("bymol (...)")` / `cmd.drag("byobject (...)")`
  (`:264-281`); `cmd.drag` is defined at `modules/pymol/editing.py:1020`.
* `menu` — `MenuActivate2Arg(..., "pick_sele", name, name)` if the atom is in the active selection,
  ...
  (`:883-887`). Menu contents live in `modules/pymol/menu.py:1682` (`main_menu`), `:1709`
  (`pick_sele`).
...
`PyMOL_GetClickString` (`layer5/PyMOL.cpp:2624-2725`) returns a newline-separated key=value blob:
...
Exposed as `pymol._cmd.get_click_string(_COb, reset)` (`layer4/Cmd.cpp:1420-1436`, table entry
`:6451`). **There is no `cmd.get_click_string` Python wrapper** — I grepped `modules/` and found
...
Rendered by `ExecutiveRenderSelectionsFromTargets` (`layer3/Executive.cpp:8433+`) as a point CGO.
...
* **Movie control bar** (`layer1/Control.cpp`): 9 buttons, `NButton = 9` (`:62`), hit-test
...
* **Scene buttons / scrollbar** inside the scene block (`SceneMouse.cpp:179-223`, `:642-709`,
  ...
  `cmd.scene_order([...])`.
* **Wizard block** (`layer1/Wizard.cpp:483+`) — buttons and pop-ups.
...
* **Spaceball / SDOF** (`layer1/Control.cpp:83-216`, `layer4/Cmd.cpp:3665` `_sdof`): 6-DOF queue,
...
### 15.1 Qt → PyMOL key codes (`modules/pmg_qt/keymapping.py`)
...
`modules/pymol/internal.py:398-421` (`special_key_codes`) and `layer0/os_gl_glut_pretend.h:14-21`.
...
Sent from the *window* (`modules/pmg_qt/pymol_qt_gui.py:50-54`), i.e. keyboard is global to the app,
...
`PyMOL_Key` (`layer5/PyMOL.cpp:2353-2359`): try `WizardDoKey` (`layer1/Wizard.cpp:328-342`, calls
...
`OrthoKey` (`layer1/Ortho.cpp:841-1032`) is the internal command line:
...
### 15.3 Python key dispatch (`modules/pymol/internal.py`)
...
Verbatim from `modules/pymol/shortcut_dict.py:10-136` (key → command, description):
...
**Clipboard ring** (`modules/pymol/keyboard.py:38-84`): `editing_ring` supports
...
(`modules/pymol/shortcut_manager.py:21`). Note `CTRL-M` is ASCII 13 (Enter) and `CTRL-E`/`CTRL-A`
are consumed by line editing when the command line has content (`Ortho.cpp:905-916`).

### 15.6 Shortcut editor dialog (`modules/pmg_qt/shortcut_menu_gui.py`)
...
* Persistence: `~/.pymol/shortcuts_save.json` (`modules/pymol/save_shortcut.py:6`,
...
2. `cmd.set('display_scale_factor', Math.round(dpr))` (`pymol_gl_widget.py:222`) — note it must be
   an **integer** or the C side warns and forces 1 (`layer1/Setting.cpp:2953-2958`),
...
* Everything authoritative depends on the backend view matrix:
  * **Picking** is a GPU colour-pick pass in the backend's own GL context
    (`layer1/ScenePicking.cpp:17-38`, `PyMOLReadPixels` at `:149`) — it renders *with the backend's
...
2. **Before the drag starts**, the backend tells the client which action the current
   ...
   (`layer1/ButMode.cpp:603`) is needed; **`ButModeGet`/`ButModeTranslate` are NOT exposed to Python
   today** (grepped `modules/` — only `cmd.button` writes exist).
...
5. Expose `cmd.set_view` / `cmd.get_view` / `cmd.turn` / `cmd.move` (`modules/pymol/viewing.py:734,

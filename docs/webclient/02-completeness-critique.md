...
Read-only audit of `docs/webclient/00-parity-inventory.md` and `01-architecture.md` plus the 12
...
* deferred image (`cmd.png`): `OrthoDeferImage` → `OrthoDefer` (`layer1/Ortho.cpp:3141-3169`)
* deferred ray (`ExecutiveRay` with `auto_copy_images`): `layer1/SceneRay.cpp:754-769`,
  forced at `layer3/Executive.cpp:11638-11641`
* `SceneDeferImage`: `layer1/Scene.cpp:4095-4110`

The queue is drained by `OrthoExecDeferred` (`layer1/Ortho.cpp:268-277`), whose **only** caller in
the entire tree is `ExecutiveDrawNow` (`layer3/Executive.cpp:11521-11523`). `ExecutiveDrawNow` is
reached from `PyMOL_Draw` (`layer5/PyMOL.cpp:2334`), `CmdRefresh` / `CmdRefreshNow`
(`layer4/Cmd.cpp:4728`, `:4748`), `CmdPNG` (`:4811`) and the movie loop (`layer1/Movie.cpp:315,746`).
...
(`layer3/Executive.cpp:11532-11534`), which is what runs `Rep::update()` and builds the very
...
(`layer1/Ortho.cpp:1852`) whose pre-guard prologue touches `G->ShaderMgr->topLevelConfig =
...
`MoviePNG` (`layer1/Movie.cpp:822`) sets `PyMOL_SetModalDraw(..., MovieModalDraw)`
(`layer1/Movie.cpp:866`, re-armed at `:816`) when `modal` is set. `cmd.mpng` reaches it at
`layer4/Cmd.cpp:4854`.

While `I->ModalDraw` is non-null, the engine's API entry macros short-circuit:
`#define PYMOL_API_LOCK if(I->PythonInitStage && (!I->ModalDraw)) {` (`layer5/PyMOL.cpp:93`,
non-Python variant `:99`). `PyMOL_Idle` returns `did_work || I->ModalDraw` (`:2466-2470`) — i.e.
"always busy". The flag is only cleared inside `PyMOL_Draw` (`layer5/PyMOL.cpp:2279-2286`).
...
(`layer1/Ortho.cpp:1470-1478`), itself called only from `OrthoDoDraw` (`:1882`), and gated on the
...
(`modules/pymol/locking.py:80-86`). `pymol.glutThread` is module-level `None`
(`modules/pymol/__init__.py:543`) and is only ever set by `prime_pymol`/`launch`
(`modules/pymol/__init__.py:378-383`); `pymol2.SingletonPyMOL.start()`
(`modules/pymol2/__init__.py:52-63`) never sets it (`self.glutThread = None` at `:124` is on the
`PyMOL` subclass and is a different attribute).

Therefore, under `SingletonPyMOL`, **every** thread is "the GUI thread". `cmd.refresh()`
(`modules/pymol/viewing.py:1769-1772`), `cmd.sync()` (`modules/pymol/commanding.py:415-419`),
...
* `00-parity-inventory.md:519`: "**Picking stays backend-authoritative.** Provision an offscreen GL
  context." (grounded in `layer1/ScenePicking.cpp:17-38,146-149`)
...
`SceneMultipick` (`layer1/ScenePicking.cpp:332-360`, rectangle pass at `:239-244`), which is how
...
`selection_visible_only` and the `cPickableThrough` sentinels (`layer1/CGO.h:141-142`).
...
| Denied | Required by |
|---|---|
| `cmd.run`, `cmd.do('@file')` | File > Run Script (`00:61`, `modules/pymol/_gui.py:118`); demo wizard runs `run $PYMOL_DATA/demo/cgo03.py` (`modules/pymol/wizard/demo.py:195`) |
...
| `cmd._ctrl` / `_alt` / `_ctsh` | ortho CLI chord fallback (`00:110`; real symbols at `modules/pymol/internal.py:488,494,509`, registered in `modules/pymol/keywords.py:46`) |
| `t:'do'` | **Every** `pymol.menu` popup leaf and wizard button — the menu system returns *command strings* (`00:100`, `layer4/PopUp.cpp:471-475`, e.g. `modules/pymol/menu.py:824` `cmd.symexp(...)`) |
...
not in `docs/webclient/` (only the 14 `.md` files exist). Agents given the docs cannot determine
...
| `bridge/pymol_bridge/panels/{objects,movie,seqview,menus}.py` | **unassigned** | WP-12, WP-19, WP-20, WP-13 |
| `bridge/pymol_bridge/allowlist.py`, `dispatch.py` | WP-02 | every feature WP that needs a symbol un-denied |
| `bridge/pymol_bridge/shims.py` | WP-02 | `_copy_image` → WP-18; `window_cmd` → WP-09; blocking dialogs → WP-17/21 |
...
`layer4/CmdWebGeometry.cpp` + the `Cmd.cpp` method table are **WP-06 only** (`:791`). But 00 §14
items 6–9 also require new C:

* item 6 `cmd.get_click_string` (C at `layer4/Cmd.cpp:1420-1436`, table row `:6451`, zero Python callers) → needed by **WP-08**
* items 7/8 `ButModeGet`/`ButModeTranslate` and the 5-char code table (`layer1/ButMode.h:225,227`, `layer1/ButMode.cpp:497-520`) → needed by **WP-22** and the ButMode grid (WP-12/09)
* item 9 setting default/min/max/help (`layer1/SettingInfo.h:46-58`, `hasMinMax()` at `:56`) → needed by **WP-10**
...
`data/startup/apbs_gui/` — `__init__.py` (450 lines), `electrostatics.py` (195),
...
(`modules/pymol/plugins/__init__.py:39`), `PluginInfo.autoload` defaults True (`:174-175`),
`initialize()` loads them at `:408-431`, and it registers a menu item
`addmenuitem('APBS Electrostatics', dialog)` (`data/startup/apbs_gui/__init__.py:448-450`).
...
for `button_mode_name` and `scene_current_name`. `grep -rn get_setting_str modules/ layer*` → 0
hits. The real symbol is `cmd.get_setting_text` (`modules/pymol/setting.py:435-438`); the family is
...
* `scenes_changed` is setting index 254 (`layer1/SettingInfo.h:339`), set by
  ...
  (`modules/pmg_tk/skins/normal/__init__.py:1191-1193`). `00 §14 item 17` lists
  `scenes_changed` as a **NEW** required event.
* `session_changed` is setting index 521 (`layer1/SettingInfo.h:621`, `layer1/Setting.cpp:659`).
  `00 §15 risk 20` says "Shutdown has no safe hook" — the Tk skin's `confirm_quit`
  (`modules/pmg_tk/skins/normal/__init__.py:207-221`) implements exactly the unsaved-session guard
...
`modules/pymol/__init__.py:415-426`: if importing `pmg_qt` raises `ImportError`, PyMOL prints
"Qt not available … using GLUT/Tk interface" and sets `invocation.options.gui = 'pmg_tk'`.
`PMGApp.setSkin` / `addSkinMenuItems` (`modules/pmg_tk/PMGApp.py:253-290`) discovers skins by glob.
...
|---|---|---|
| `assign_stereo` | `modules/pymol/stereochemistry/__init__.py:29` | L-menu "stereochemistry" labels by the `stereo` property (`modules/pymol/menu.py:1536`) — silently blank |
| `morph` | `modules/pymol/morphing.py:53` | api.py symbol |
| `focal_blur` | `modules/pymol/experimenting.py:244` | api.py symbol |
| `callout` | `modules/pymol/experimenting.py:266` | api.py symbol |
| `desaturate` | `modules/pymol/experimenting.py:280` | api.py symbol |
| `find_pi_interactions` | `modules/pymol/querying.py:545` | `find > pi interactions` popup (`menu.py`, see `internal-gui.md:302`) |
| `help_setting` | `modules/pymol/helping.py:99` | it is the intended consumer of `data/setting_help.csv` |
...
(`modules/chempy/models.py`), `cmd.get_session()` → nested dict with binary,
`cmd.get_coords`/`get_coordset` → numpy arrays and, with `copy=0`, a **live view onto C++ memory**
(`layer2/CoordSet.cpp:326-361`, noted in `geometry-extraction.md:259-261`),
`cmd.get_volume_field` → numpy. `grep -rn "get_model\|serializable" docs/webclient/cmd-api-rpc.md`
...
(`modules/pymol/util.py:385`), which calls `protein_assign_charges_and_radii`
...
`mass_align`, `ff_copy` (all `modules/pymol/util.py`).
...
* **C1.** `01-architecture.md:1213` asks "Does `pmg_qt/file_dialogs.load_dialog` call
  `recent_filenames_add`?" — yes: `modules/pmg_qt/file_dialogs.py:42` and `:593`. The open question
  is answerable in 30 seconds and its wrong resolution would have cloned a non-existent bug.
* **C2.** `_cmd.get_setting_level` is already in the method table (`layer4/Cmd.cpp:6494`,
...
* **C5.** `01-architecture.md:63-67` principle 1 ("upstream tree is not moved… a git merge must
  ...
  `layer4/Cmd.cpp` (`:218-220`). The doc flags the brief contradiction (`:227-230`) but not its own.
* **C6.** The invocation surface is only spot-checked (`-R`, `-x`, `-q`, `-y`, `-k`,
  `modules/pymol/invocation.py` ~532 lines). Flags that change front-end behaviour and have no web

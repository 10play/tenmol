...
| --- | --- | --- |
| Volume Color Map Editor | Internal-GUI object menu `A > volume > panel`, and `volume_panel` command | `packages/engine/modules/pymol/menu.py:648`, `packages/engine/modules/pymol/colorramping.py:183-227` |
| Properties Inspector | "Properties" quick-button in the lower button row | `packages/engine/modules/pmg_qt/pymol_qt_gui.py:251`, opener `pymol_qt_gui.py:625-632` |
| Scene Panel | menubar `Scene > Scenes...` | `packages/engine/modules/pymol/_gui.py:776`, opener `pymol_qt_gui.py:891-897` |
| Keyboard Shortcut Menu | menubar `Setting > Keyboard Shortcuts...` | `packages/engine/modules/pymol/_gui.py:494`, opener `pymol_qt_gui.py:885-889` |
| Advanced Settings | menubar `Setting > Edit All...` | `packages/engine/modules/pymol/_gui.py:493`, opener `pymol_qt_gui.py:878-883` |
| Text Editor (pymolrc) | menubar `File > Edit pymolrc` | `packages/engine/modules/pymol/_gui.py:124`, opener `pymol_qt_gui.py:634-637` |
...
runtime `.ui` parsing via `pymol.Qt.utils.loadUi` (`packages/engine/modules/pymol/Qt/utils.py:269-283`).
...
**No generated `.py` forms exist in the tree** (`packages/engine/modules/pmg_qt/forms/` contains
only `.ui` + an empty `__init__.py`), so the `.ui` XML is the single source of truth.

---

## 1. Volume Color Map Editor (`packages/engine/modules/pmg_qt/volume.py`, 877 lines)
...
- `cmd.get_volume_histogram(objName, bins=64, range=None)`
  (`packages/engine/modules/pymol/querying.py:62-72`) -> `[min, max, mean, stdev, h0..h(bins-1)]`;
  C entry `CmdGetVolumeHistogram` / `ExecutiveGetHistogram`
  (`packages/engine/layer4/Cmd.cpp:738-752`).
- `cmd.volume_color(name, ramp='', state=CURRENT_STATE, quiet=1, _guiupdate=True)`
  (`packages/engine/modules/pymol/colorramping.py:123-181`). With empty `ramp` it is a **getter**
...
- Named presets `namedramps` (`colorramping.py:17-54`): `2fofc`, `fofc`, `esp`,
  ...
  (`packages/engine/modules/pymol/menu.py:644-654`).
...
## 2. Properties Inspector (`packages/engine/modules/pmg_qt/properties_dialog.py`, 415 lines + `forms/props.ui`)
...
| --- | --- | --- |
| object list in combo | `cmd.get_object_list()` | `properties_dialog.py:37-41`, API `packages/engine/modules/pymol/querying.py:131` |
...
(`properties_dialog.py:8`, `291`, `298`; built at `packages/engine/modules/pymol/setting.py:39-42`
...
| edited row | call |
| --- | --- |
| TTT Matrix | `cmd.set_object_ttt(model, new_value)`; `ValueError`/`IndexError` -> treated as failure (`properties_dialog.py:166-171`); API `packages/engine/modules/pymol/editing.py:2219` |
...
| Object-State Properties child | `cmd.set_property(key, new_value, model, state, quiet=0)` (`:187-188`; Incentive-only path, API `packages/engine/modules/pymol/properties.py:123`) |
...
`packages/engine/modules/pymol/Qt/utils.py:323-360`). On any failure the whole tree is reloaded
...
## 3. Scene Panel (`packages/engine/modules/pmg_qt/scene_bin_gui.py`, 397 lines)
...
- `cmd.get_scene_list()` -> `_cmd.get_scene_order()` (`scene_bin_gui.py:244-251`;
  API `packages/engine/modules/pymol/viewing.py:919-921`).
- `cmd.get_scene_thumbnail(name)` -> raw PNG bytes loaded into a `QPixmap`
  (`scene_bin_gui.py:183-192`; API `viewing.py:923-925`; C
  `packages/engine/layer4/Cmd.cpp:1159-1173`, backed by `MovieSceneGetThumbnail`,
  `packages/engine/layer3/MovieScene.cpp:45`).
- Message and actions columns are **hardcoded placeholders**
  ...
  (`packages/engine/modules/pymol/viewing.py:927-933`).
...
quiet=1, sele="all")` (`packages/engine/modules/pymol/viewing.py:1034-1036`). Actions:
...
## 4. Keyboard Shortcut Menu (`packages/engine/modules/pmg_qt/shortcut_menu_gui.py`, 415 lines)
...
- Keys in `ShortcutManager.reserved_keys` are swallowed and never displayed
  ...
  (`packages/engine/modules/pymol/shortcut_manager.py:21`).
...
### 4.3 Backing model (`packages/engine/modules/pymol/shortcut_manager.py`, `packages/engine/modules/pymol/shortcut_dict.py`)
...
- `cmd.shortcut_dict` is a deep-ish copy of `shortcut_dict_ref`
  ...
  `packages/engine/modules/pymol/shortcut_dict.py:11-136` — **135 default bindings**, covering
...
- `get_default_keys` (`packages/engine/modules/pymol/keyboard.py:87-93`) projects
...
- Persistence: `save_shortcuts` (`shortcut_manager.py:70-76`) prunes unused keys
  ...
  (`packages/engine/modules/pymol/save_shortcut.py:6`, `18-36`). Load on startup:
...
- `key.rpartition('-')` -> modifier must be in
  ...
  (`packages/engine/modules/pymol/internal.py:390-396`), else
...
- Qt->PyMOL key translation for the *viewport* lives in
  `packages/engine/modules/pmg_qt/keymapping.py:10-97` (`keyMap`, `specialMap`,
...
## 5. Advanced Settings (`packages/engine/modules/pmg_qt/advanced_settings_gui.py`, 99 lines)
...
`sorted(setting.get_name_list())` (`packages/engine/modules/pymol/setting.py:80-81`, backed by
...
(`packages/engine/modules/pymol/setting.py:413-418`).

Rendering by type (constants at `packages/engine/modules/pymol/setting.py:19-26`):
...
## 6. Text Editor (`packages/engine/modules/pmg_qt/TextEditor.py`, 195 lines)
...
(`packages/engine/modules/pymol/Qt/utils.py:249-267`: Monaco+3 on darwin, Consolas on win32,
...
- `setSyntax(filetype='plain')` (`:48-60`): checks the menu action, detaches the
  ...
  `packages/engine/modules/pmg_qt/syntax/python.py` (147 lines),
  `packages/engine/modules/pmg_qt/syntax/pml.py` (119 lines, `class Highlighter` at
...
## 7. Full `packages/engine/modules/pmg_qt/forms/` inventory

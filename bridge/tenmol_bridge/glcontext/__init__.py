...
(``layer5/PyMOL.cpp:2248``) and then calls ``glGetString``
(``layer5/PyMOL.cpp:2307``) — and because with ``HaveGUI == 0`` the entire body
of ``SceneRender`` is skipped (``layer1/SceneRender.cpp:270``), which silently
kills every pick.  See ``docs/webclient/spikes/04-picking.md``.
...
2. **Never regenerate the FBO on resize.**  ``check_gl_stereo_capable``
   ...
   ``GL_FRAMEBUFFER_BINDING`` on the first draw (``layer5/PyMOL.cpp:2236-2239``).

...
``glewInit()`` on the first draw (``layer0/ShaderMgr.cpp:566``); on Windows GLEW
...
(``layer1/ScenePicking.cpp:283,306,234,272``).  So ANGLE is reachable only via
    ...
    def _create(self) -> None:
        ...
        if gl.fbo_suffix == "EXT":
            self.warnings.append(
                ...
                "the ARB entry points (layer0/ShaderMgr.cpp:1829)"
            )
...
def _angle_context(width: int, height: int):
    ...
    ctx.warnings.append(
        ...
        "and calls glewInit() (layer0/ShaderMgr.cpp:566), which resolves "

...
`Parser.complete` (`modules/pymol/parser.py:524-596`), reached exactly the way
every PyMOL front end reaches it::

    modules/pymol/_gui.py:899-903
        st = self.cmd._parser.complete(self.command_get())
        if st:
            self.command_set(st)
            self.command_set_cursor(len(st))

    modules/pmg_qt/pymol_qt_gui.py:421-424   Key_Tab -> self.complete()
...
`Shortcut` tables built by `modules/pymol/completing.py`), `cmd.get_names` and
...
GRANT = Grant(
    wp="WP-11",
    note="tab completion: cmd._parser.complete (modules/pymol/parser.py:524)",

...
not Python's ``sys.stdout``.  ``pcatch`` (``layer1/P.cpp:2662-2674``) is a
...
(``bridge/tests/test_wf_apbs.py::test_a_bare_inheriting_subprocess_never_reaches_the_browser``):
...
That is exactly the hole ``data/startup/apbs_gui/electrostatics.py:156`` falls
...
``OrthoNewLine`` (``layer1/Ortho.cpp:1143-1169``) **already** ``printf``s every
...
def console_write(text: str) -> None:
    ...
    ``pcatch`` module (``layer1/P.cpp:2716-2720``) and its ``write`` is the

...
Block``, ``layer3/ExecutiveDef.h:54``) rebuilt from the live ``Spec`` list at up
...
caption                      ``ObjectMolecule::getCaption``
                             (``layer2/ObjectMolecule.cpp:386-460``)
===========================  ==================================================
...
Record layout, verified against a live session::
    ...
    rec[0] name         rec[4] cObject_t enum (layer1/PyMOLObject.h:40-56)
...
lists (``layer4/PopUp.cpp:131-260``: 0 = separator, 1 = item, 2 = title).  The
...
#: ``cObject_t`` (``layer1/PyMOLObject.h:40-56``) -> the string ``cmd.get_type``
#: returns (``modules/pymol/querying.py`` ``get_type``).  Reading the enum out
...
def _rep_bitmask(indices: Optional[Sequence[int]]) -> int:
    ...
    back together so the client can test ``cRep*Bit`` (``layer1/Rep.h:84-104``).
    """
    mask = 0
...
def caption_for(cmd: Any, name: str, kind: str) -> str:
    """``ObjectMolecule::getCaption`` (``layer2/ObjectMolecule.cpp:386-460``).
...
def snapshot(cmd: Any, name_color_mode: Optional[int] = None) -> Dict[str, Any]:
    ...
    tree** — it is ``cmd.get_setting_text`` (``modules/pymol/setting.py:435``).
    """

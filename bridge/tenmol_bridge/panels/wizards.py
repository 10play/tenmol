...
(``CWizard::Wiz``, ``layer1/Wizard.cpp:69``).  It is not a widget.  The core
...
wizards -- plus the 16 builder wizards in ``modules/pmg_qt/builder.py`` and any
...
``PyObject_HasAttrString`` first (``layer1/Wizard.cpp:162``).  So does this
...
pulling from Python (``WizardRefresh``, ``layer1/Wizard.cpp:195``).  The bridge
...
#: ``layer4/PopUp.cpp:231-248`` / ``:293-320``.
MENU_CODES: Dict[int, str] = {0: "separator", 1: "item", 2: "title"}

#: The Wizard menu of the Qt menubar, verbatim from ``modules/pymol/_gui.py:834``.
...
#: ``name.capitalize()`` (``modules/pymol/wizarding.py:35,41``), and rewrites
...
def _call(wiz: Any, name: str, *args: Any) -> Tuple[bool, Any, Optional[str]]:
    ...
    ``layer1/Wizard.cpp:162`` probes with ``PyObject_HasAttrString`` before
...
def _panel_rows(raw: Any) -> List[Dict[str, Any]]:
    ...
    ``layer1/Wizard.cpp:241`` requires at least three items per row and reads
...
def snapshot() -> Dict[str, Any]:
    ...
    never on a timer.  Mirrors ``WizardRefresh`` (``layer1/Wizard.cpp:195``),
...
def _prompt_mode(cmd: Any) -> int:
    """Setting ``wizard_prompt_mode`` (``layer1/SettingInfo.h:461``, default 1).

    0 = off, 1 = text + opaque backdrop, 2 = text only, 3 = text only flush to
    the top-left corner (``layer1/Ortho.cpp:2136-2190``).
    """
    try:
...
def _encode_menu(raw: Any, level: int, budget: List[int]) -> List[Dict[str, Any]]:
    ...
    ``layer4/PopUp.cpp:231-248``: code 0 is a separator bar (text and command
    ...
    **callable** -- the lazy submenu form, which ``SubGetItem``
    ...
    ``modules/pymol/wizard/`` uses the callable form today (``menu.py`` does),
...
def exec_code(code: str) -> Dict[str, Any]:
    ...
    ``layer1/Wizard.cpp:573-577`` PLogs it and hands it to ``PParse`` -- the
    **PyMOL command language**, not Python and emphatically not JavaScript.
    ``cmd.do`` is the same parser (``modules/pymol/parser.py``), which is why
...
def event(kind: str, **payload: Any) -> Dict[str, Any]:
    ...
    ``CWizard::isEventType`` (``layer1/Wizard.cpp:140``) is checked *before*
...
def _picked_state(cmd: Any) -> int:
    ...
    (``layer1/Wizard.cpp:189``, ``:324``).  ``cmd.get_object_state`` is the
...
def launch(name: str, args: Sequence[Any] = (), kwargs: Any = None) -> Dict[str, Any]:
    """``cmd.wizard(name, *args, **kwargs)`` -- ``modules/pymol/wizarding.py:62``.
...
def dismiss(all: bool = False) -> Dict[str, Any]:
    ...
    ``WizardSet`` (``layer1/Wizard.cpp:264-283``) pops exactly one; ``all=True``
    empties the stack the way ``cmd.reinitialize`` does
    (``layer3/Executive.cpp:16586``) but without touching anything else.
    """
    cmd = _pymol_cmd()
    _ensure(cmd)
    if all:
...
def catalog() -> Dict[str, Any]:
    ...
    The list is discovered from ``modules/pymol/wizard/`` rather than hardcoded,

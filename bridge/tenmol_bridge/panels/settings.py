...
Every number below was measured on this build (``bridge/.venv``): 780
...
----------------------
``_cmd.get_setting_level(index)`` (``layer4/Cmd.cpp:4403-4409``) returns the
level NAME and is **never wrapped in** ``modules/pymol/setting.py`` — it is
...
(``layer3/Executive.cpp:12279-12286``).  Without the level the UI would silently
offer scopes that do nothing.  This module is where it enters the web client.

THE UPDATE TAP (never a second consumer)
----------------------------------------
``cmd.get_setting_updates()`` (``layer1/Setting.cpp:1121-1147``) **clears the
...
#: ``layer1/Setting.h:113-120`` / ``modules/pymol/setting.py:19-26``.
...
#: ``layer1/Setting.cpp:64-74``.  The C enum suffix used in ``SettingInfo.h``
...
#: Which scopes a level may be written at (``layer1/Setting.cpp:55-96``).  The
...
def parse_setting_info(path: str) -> Dict[int, Dict[str, Any]]:
    """``layer1/SettingInfo.h`` -> ``{index: {name, kind, level, default, ...}}``.

    Pure text parsing of the record macros documented at
    ``layer1/SettingInfo.h:15-23``; it never imports PyMOL and never guesses —
    ...
    #: `#define MAX_SPHERE_QUALITY 4` (`layer1/SettingInfo.h:79`) is used as a
...
def _repo_root() -> str:
    #: panels/settings.py -> panels -> tenmol_bridge -> bridge -> <repo>
...
def setting_info_path() -> Optional[str]:
    for path in _candidate_paths("TENMOL_SETTINGINFO_H", "layer1/SettingInfo.h"):
...
def load_help(path: Optional[str] = None) -> Dict[str, str]:
    """``data/setting_help.csv`` -> ``{name: description}``.
...
def catalogue(cmd: Any = None, refresh: bool = False) -> Dict[str, Any]:
    ...
    serialize them, ``layer1/SettingInfo.h:5-6``), so this is built once and
    ...
    _catalogue_cache = {
        ...
        "meta": {
            ...
            "minMaxNote": (
                ...
                "(layer1/Setting.cpp:1890-1911); ranges here are hints, not "
...
def _validated_from_header(
    ...
    if path is None:
        return (
            {},
            "layer1/SettingInfo.h not found next to the bridge; defaults and "
...
def scope(
    ...
    (``layer1/P.cpp:455-606``): ``iter(s)`` yields the indices actually defined
...
#: ``layer1/SettingInfo.h`` level ``cSettingLevel_bond``, and the same six names
...
def coerce(kind: str, raw: Any, cmd: Any = None) -> Any:
    """``modules/pymol/setting.py:83-112`` by way of the real implementation.

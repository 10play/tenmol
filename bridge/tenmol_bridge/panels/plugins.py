...
itself is in ``modules/pymol/plugins/__init__.py`` and is not edited here.
"""
...
class PluginMenuAPI:
    ...
    def load(self, name: str, quiet: int = 1) -> Dict[str, Any]:
        ...
        THE DEFECT THIS GUARDS (``modules/pymol/plugins/__init__.py``,
...
def install(cmd: Optional[Any] = None) -> Dict[str, Any]:
    ...
    (``modules/pymol/gui.py:20-25``) and only then falls back to

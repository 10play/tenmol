...
(``layer4/Cmd.cpp:1321-1391``) has four modes and the only one that returns RGB
...
(``bridge/tenmol_bridge/dispatch.py:_ROOT_MODULES``) and ``policy/base.py``
...
#: (``layer1/Color.cpp:825-1322``).  Asserted, not trusted.
COLOR_TABLE_SIZE = 5388

#: ``ColorGetStatus`` == 1 only for names with no digits
#: (``layer1/Color.cpp:784-807``); that is what mode 1 returns.
...
def palette(cmd: Any, all: bool = True) -> List[Dict[str, Any]]:
    ...
    ``ColorRec::LutColor`` (``layer1/Color.cpp:1680``) and ``get_color_tuple``
...
def ramps(cmd: Any) -> List[Dict[str, Any]]:
    ...
    ramp can be used AS (``layer1/Color.h:46``).
    """
    out: List[Dict[str, Any]] = []
...
def space(cmd: Any, space_name: str = "rgb", gamma: float = 1.0) -> Dict[str, Any]:
    ...
    ``ColorRec::LutColor`` (``layer1/Color.h:58-59``) means every colour's RGB
...
#: internal GUI uses (``modules/pymol/menu.py:643-712``).
_MENU_PROVIDERS = (
    "mol_color",
    "measurement_color",
    "general_color",
...
def menu_tree(cmd: Any, sele: str = "(all)", kind: str = "mol_color") -> List[Any]:
    ...
    0 = separator (``layer4/PopUp.cpp``), and ``action`` is either a command

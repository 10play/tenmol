...
---------------------------
``cmd._get_feedback()`` (``modules/pymol/internal.py:580-606`) is a
...
-------------------------------------------------------
``OrthoFeedbackIn`` is gated on ``G->Option->pmgui`` (``layer1/Ortho.cpp:493-497``)
and ``pmgui = !no_gui`` (``layer1/P.cpp:1820``).  ``01-architecture.md:38-49``
...
``docs/webclient/spikes/00-build.md``'s venv), with ``pymol2.SingletonPyMOL``
    ...
    def __init__(self, cmd) -> None:
        ...
        if not hasattr(cmd, "_get_feedback"):
            self.unavailable_reason = (
                ...
                "(expected modules/pymol/internal.py:580)"
            )

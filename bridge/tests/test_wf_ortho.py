...
Both rows describe things PyMOL draws in its OWN ortho layer:
  ...
  * ``OrthoBusyDraw`` (``layer1/Ortho.cpp:609-724``) — a 240x60 black box in the
    top-left with a message line and up to TWO bars (the slow and the fast
    counter), gated by ``show_progress`` and throttled.
  * ``OrthoDrawLoop`` (``layer1/Ortho.cpp:1695-1745``) — the 1 px rubber band
...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_wf_ortho.py -q
"""
...
def rayable(ws: WSClient):
    ...
    (``modules/pymol/viewing.py:1733-1738``), and it renders whatever the
...
def test_the_progress_topic_carries_a_live_fraction_while_pymol_is_busy(
    ...
    # never a count (`layer4/Cmd.cpp:4334-4348`).
...
def test_progress_goes_back_to_MINUS_ONE_when_the_job_is_over(
    ...
    counters hold (`layer4/Cmd.cpp:4341`), and `SceneRay` clears busy on its
    way out (`layer1/SceneRay.cpp:741`).  A client that rendered the value as a
...
def test_the_update_rate_is_throttled_in_the_C_not_by_the_poller(
    ...
    (`layer1/Ortho.cpp:547`, `:577`), and the drawn box has its own 0.2 s gate
...
def test_show_progress_0_silences_the_whole_channel(rayable: WSClient) -> None:
    ...
    (`layer1/Ortho.cpp:547`, `:577`), not merely the drawing, so turning it off
    ...
    # (`layer1/SettingInfo.h:347`), so `cmd.get` renders it "1", not "on".
...
def test_ray_turns_sculpting_off_which_hides_the_C_sculpting_guard(
    ...
    (`layer4/Cmd.cpp:4328`), so reading the source suggests "sculpting kills
    the progress bar".  Driving it says otherwise: `cmd.ray` sets `sculpting`
    to off BEFORE it starts (`modules/pymol/viewing.py:1736-1737`), so the
...
def test_the_busy_MESSAGE_and_the_two_BARS_are_not_reachable_from_python(
    ...
    `I->BusyMessage` (set by `OrthoBusyMessage`, `layer1/Ortho.cpp:530`) and the
    ...
    fast one (`layer4/Cmd.cpp:4334-4348`), so the two-bar layout of
    ...
    `_cmd.get_busy`/`_cmd.set_busy` DO exist (`layer4/Cmd.cpp:6455`, `:6633`)
...
def test_the_marquee_rectangle_is_not_readable_from_python(
    ...
    into `COrtho` with `OrthoSetLoopRect` (`layer1/SceneMouse.cpp:44-66`); the
...
def test_the_splash_FLAG_is_invisible_but_the_splash_TEXT_is_a_command(
    ...
    (`layer1/Ortho.cpp:2523`, `:967`).  The TEXT, though, is a normal command --
    `cmd.splash(0)` runs `OrthoSplash` (`layer4/Cmd.cpp:2865-2869`) and the
    ...
    (`modules/pymol/commanding.py:337-339`) -- a global that changes what the
...
def test_the_bridge_boots_with_the_splash_off(ws: WSClient, bridge) -> None:
    ...
    (`bridge/tenmol_bridge/engine.py:132`), so `OrthoInit` never raises

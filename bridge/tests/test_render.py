...
PDB_1AON = REPO / "testing" / "data" / "1aon.pdb.gz"
...
def test_pixel_frames_are_byte_identical_to_the_protocol_packages_encoder():
    """WP-01: "a second hand-rolled encoder in bridge/ will silently break
...
def test_the_dirty_gate_probes_before_the_draw(render, gl_bridge):
    """``PyMOL_Draw`` clears ``RedisplayFlag`` (layer5/PyMOL.cpp:2331).
...
def test_benchmark_1aon_at_1280x960(render, gl_bridge):
    ...
    if not PDB_1AON.exists():
        pytest.skip("testing/data/1aon.pdb.gz is not in this tree")

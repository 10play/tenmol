...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_colors.py -q
"""
...
def test_specials_resolve_live_and_auto_is_not_minus_two(bridge: Any) -> None:
    ...
    # Constants (layer1/Color.h:36-44).
...
def test_protocol_constants_match_the_c_header() -> None:
    """``topics/colors.ts`` mirrors ``layer1/Color.h:36-47`` exactly."""
    ...
    # members (``layer1/Color.h:36-47``), and ``cColorExtCutoff`` is
    # parenthesised, so the pattern allows both spellings.
    header = (REPO / "layer1" / "Color.h").read_text()
    for name, literal in (
        ("cColorDefault", "-1"),
        ("cColorNewAuto", "-2"),
        ("cColorCurAuto", "-3"),

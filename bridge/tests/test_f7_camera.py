...
``test/dat/il2.pdb`` and the 10-state ``test/dat/ligs3d.sdf``:
  ...
  2. THE SWEEP IS NOT CONTINUOUS.  ``SceneLoadAnimation`` builds
     ``int(duration * 30)`` key frames (``layer1/Scene.cpp:401``, capped at
  ...
  4. ``SceneUpdateAnimation`` IS ONLY CALLED FROM ``SceneRender``
     (``layer1/SceneRender.cpp:246``) AND ``SceneRay`` (``SceneRay.cpp:125``).
...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_f7_camera.py -q
"""
...
#: ``SceneLoadAnimation`` (``layer1/Scene.cpp:401``): ``int(duration * 30)``.
ANI_ELEM_HZ = 30
#: ``layer1/SceneDef.h:39``.
MAX_ANI_ELEM = 300
...
def cam(ws: WSClient):
    ...
    (``layer1/Scene.cpp:4504-4520``) instead of the wall-clock one and every
...
def test_an_animated_command_has_not_moved_the_camera_when_it_returns(
    ...
    (``layer1/Scene.cpp:420``) — key frame 0, i.e. exactly where the camera
...
def test_animate_minus_one_is_animation_times_animation_duration(
    ...
    ``ExecutiveWindowZoom`` (``layer3/Executive.cpp:13433-13438``), and the
    identical block in ``ExecutiveCenter`` (``:13494``), ``ExecutiveOrient``
    (``:10065``) and ``SceneSetView`` (``layer1/Scene.cpp:924-929``).
...
def test_the_sweep_is_thirty_discrete_key_frames_per_second(
    ...
    ``SceneLoadAnimation`` (``layer1/Scene.cpp:401``) sizes the key-frame array
...
def test_an_instant_command_aborts_a_sweep_but_turn_does_not(
    ...
    ``ExecutiveWindowZoom``'s else-branch (``layer3/Executive.cpp:13443``) and
    ``SceneSetView`` (``layer1/Scene.cpp:933``) abort.  ``_cmd.turn`` does not
...
def test_a_second_animated_command_abandons_the_first_where_it_stands(
    ...
    ``layer1/Scene.cpp:326-328`` sets ``cur_ani_elem = n_ani_elem`` and then
...
def test_zoom_buffer_and_complete(cam: WSClient) -> None:
    ...
    ``ExecutiveWindowZoom`` (``layer3/Executive.cpp:13404-13419``): ``buffer``
...
def test_reset_origin_is_weighted_not_the_raw_extent_midpoint(cam: WSClient) -> None:
    ...
    (``layer3/Executive.cpp:11494-11496``) and that call asks
...
def test_set_view_animates_and_lands_on_the_exact_eighteen_floats(
    ...
    AND aborts a sweep already running (``layer1/Scene.cpp:930-934``).
    """
    cam.call("cmd.reset")
    home = cam.call("cmd.get_view")
    wanted = list(home)

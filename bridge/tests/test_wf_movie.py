...
the duration of the encode (``modules/pymol/movie.py:748,763``); the tests that
...
def test_watch_only_ever_reports_bytes_for_mpeg_encode(mws: Any) -> None:
    ...
    behind ``if not quiet`` (``modules/pymol/movie.py:750-753``).  The ffmpeg
...
def test_upstream_produce_opens_a_modal_window_the_bridge_endpoint_does_not(
    ...
    (``modules/pymol/movie.py:973``) and ``MoviePNG`` only demotes a negative
    ``modal`` to 0 when the mode is ray (``layer1/Movie.cpp:836``).  So a plain
...
def test_panel_rows_are_executive_count_motions_not_camera_plus_objects(mws: Any) -> None:
    """``ExecutiveCountMotions`` (``layer3/Executive.cpp:664``), row for row.
    ...
    ``layer1/Movie.cpp:924-929``), so a movie program is enough; no ``mview``
...
def test_presentation_collapses_the_panel_to_the_camera_row(mws: Any) -> None:
    ...
    ``MovieGetPanelHeight`` agrees (``layer1/Movie.cpp:1716``): one
...
def test_the_c_panel_is_off_in_the_bridge_and_the_row_height_is_still_the_model(
    ...
    (``layer1/Ortho.cpp:2383``), so leaving the GL panel on would silently cost
    the web canvas ~15 px the moment any object had more than one state.  The
    default is 1 (``layer1/SettingInfo.h:718``), so this is a deliberate
...
def test_label_indent_is_the_sixty_four_pixel_c_gutter(mws: Any) -> None:
    """``CMovie::LabelIndent = DIP2PIXEL(8 * 8)`` (``layer1/Movie.cpp:1867``).
...
def test_object_none_is_the_camera_row_and_empty_is_every_row(mws: Any) -> None:
    ...
    (``layer3/Executive.cpp:538-560``) reads ``none`` as "camera, and stop" and
...
def test_every_panel_gesture_command_executes_over_the_wire(mws: Any) -> None:
    ...
    POSITIONAL is ``freeze``, not ``object`` (``modules/pymol/moving.py:493,
...
def test_minsert_on_the_camera_row_alone_does_not_undo_with_mdelete(mws: Any) -> None:
    ...
    ``ExecutiveMotionViewModify`` (``layer3/Executive.cpp:544-560``) takes a
...
def test_the_motion_context_menu_targets_come_from_the_same_row(mws: Any) -> None:
    ...
    ``ExecutiveMotionMenuActivate`` (``layer3/Executive.cpp:737``) passes
    ...
    ``modules/pymol/menu.py``; what is checked here is that the commands those

...
* Row 328's last open item, the **presentation auto-quit chain**
  (``modules/pymol/viewing.py:1107-1113``).  Wave 8 left it open with the note
...
def test_the_presentation_auto_quit_chain_fires_end_to_end_with_quit_stubbed(
    ...
    ``pymol.cmd`` MODULE (``modules/pymol2/__init__.py:53-63``) and
...
def test_an_unselected_cell_raises_seq_option_on_a_seeker_temporary(
    ...
    (``modules/pymol/menu.py:1801-1804``).
    """
    ws = seqview
...
def test_scene_button_geometry_matches_Scene_cpp(ws: WSClient) -> None:
    ...
    """
    defines = _c_defines("layer1/Scene.cpp", "layer1/Scene.h")
    consts = _ts_consts()
    ...
    with open(os.path.join(_REPO, "layer1", "Scene.cpp"), "r", encoding="utf-8") as fh:
        body = fh.read()
    assert (
...
def test_builder_sculpt_tick_is_the_idle_loop_the_bridge_does_not_run(
    ...
    Same gate (the ``sculpting`` setting, ``layer1/Control.cpp:397-403``), same

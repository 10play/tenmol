...
* **drags the mouse over the WebSocket and asserts ``get_view()[:9]``
  ...
  (``layer1/Scene.cpp:4113-4155`` -> ``layer1/Ortho.cpp:268-277`` ->
  ``layer3/Executive.cpp:11521-11523``, gated on ``IdleAndReady == 3``) and the
...
def test_healthz_reports_glut_thread_equals_thread_ident(bridge):
    ...
    # (modules/pymol/locking.py:80-86).
    import pymol
...
def test_gl_context_is_real_hardware(gl_bridge):
    ...
    # from these exact glGetIntegerv values (layer1/ScenePicking.cpp:38-84).
...
def test_mouse_drag_rotates_the_camera(gl_bridge, ws):
    ...
    (layer0/os_gl_glut_pretend.h:11-26).
    """
    ws.call("delete", "all")
...
def test_feedback_carries_c_origin_and_python_origin_lines(bridge, ws):
    ...
    assert any(line == echo for line in lines), (
        ...
        "pmgui (layer1/Ortho.cpp:492-499) -- did something set no_gui=1? "
        "lines=%r" % lines[-10:]
    )
    assert any(line == marker for line in lines), (
        "the Python-origin print() is missing; pcatch writes through the "
        "file-scope SingletonPyMOLGlobals pointer (layer1/P.cpp:2667) and is "
...
def test_mpng_completes_and_the_engine_survives(gl_bridge, ws):
    ...
    (``layer5/PyMOL.cpp:2279-2286``); a pump that draws clears it on the next
    tick.
    """
    ws.call("delete", "all")
    ws.call("fragment", "ala")
...
def test_status_thread_only_uses_lock_attempting_calls(bridge, ws):
    ...
    def hold_the_api_lock(engine):
        ...
        # lock_api (modules/pymol/locking.py:26-30).  A generic poller thread
    ...
    # returns None, not [], on a lock miss (modules/pymol/internal.py:596-606).

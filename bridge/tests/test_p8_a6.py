...
* **255** ``seqalign.load_aln_multi`` really runs and builds an alignment
  object, with upstream's own atom counts (``testing/tests/api/seqalign.py``).
...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_p8_a6.py -q
"""
...
class TestLoadAlnMulti:
    def test_load_aln_builds_a_real_alignment_object(self, scratch):
        ...
        (``testing/tests/api/seqalign.py::testLoadAlnMappingStr``): the
...
class TestMtzImportDialog:
    ...
    def test_the_three_column_lists_come_from_three_mtz_type_codes(self, files):
        ...
        MEASURED against ``testing/data/4rwb.mtz``: there are no G (anomalous

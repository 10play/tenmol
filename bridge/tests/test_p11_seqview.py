...
branch (``layer3/Seeker.cpp:1583-1793``) plus ``SeekerFindTag`` (``:928-966``)
plus ``CSeq::draw``'s fill/unaligned colouring (``layer1/Seq.cpp:322-449``,
``:488-504``) missing.
...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_p11_seqview.py -q
"""
...
def _row(codes: int, spec) -> dict:
    ...
    follows another (``layer3/Seeker.cpp:1289``) — because those two flags are
...
class TestAlignRows:
    """The SECOND PASS in alignment mode (``layer3/Seeker.cpp:1583-1793``)."""
...
class TestFillAndUnalignedColour:
    """``CSeq::draw``'s two settings reads (``layer1/Seq.cpp:316-338``)."""
...
class TestStagger:
    """``seq_view_unaligned_mode`` 0-5 (``layer3/Seeker.cpp:1590-1596``)."""
...
class TestFillChar:
    ...
    def test_a_space_switches_the_fill_off(self, aln_ws: WSClient) -> None:
        ...
        try:
            ...
            # the drawing (`layer1/Seq.cpp:488`), which is where the client
            # skips it too.
    ...
    def test_the_background_comes_back_for_the_dim_blend(
        ...
        """Modes 1/4 average with `bg_rgb` (``layer1/Seq.cpp:424-431``)."""
...
class TestGapsAreSuppressed:
    """``&& align_sele < 0`` (``layer3/Seeker.cpp:1235``)."""

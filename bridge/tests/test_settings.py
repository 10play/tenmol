...
* pure parsing of ``layer1/SettingInfo.h`` and of the update tap, which need no
  PyMOL at all;
...
def test_record_payloads_match_the_documented_table() -> None:
    ...
    # The one blank slot, REC__( 83, ) — layer1/SettingInfo.h:167.
...
def test_exactly_thirty_records_declare_a_range() -> None:
    ...
    (`layer1/SettingInfo.h:18`) rather than declaring a range.
    """
    parsed = S.parse_setting_info(SETTING_INFO_H)
...
def test_object_and_atom_scope_report_real_overrides(ws: Any) -> None:
    ...
    # (`SettingUniqueGetIndicesAsPyList`, layer1/P.cpp:455-606). sphere_scale
...
def test_bond_level_settings_need_set_bond_not_set(ws: Any) -> None:
    ...
    ``modules/pymol/setting.py:245-248``: "if you attempt to use the set
...
def test_unset_deep_clears_object_and_atom_but_not_the_global(ws: Any) -> None:
    ...
    ``modules/pymol/setting.py:516-544`` — the docstring is explicit that

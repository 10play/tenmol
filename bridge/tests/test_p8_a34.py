...
``docs/webclient/00-parity-inventory.md``. The sentence is quoted in the
docstring so a reader can check the test against the claim it retires.

Run::

    bridge/.venv/bin/python -m pytest bridge/tests/test_p8_a34.py -q
"""
...
def test_the_action_codes_are_the_ButMode_h_constants(butmode) -> None:
    """A slot value is `cButMode*` from `layer1/ButMode.h`, not an index into
...
# prefix table (`modules/pymol/shortcut.py:46-48,94`).
# =====================================================================
...
# `self.keywords = []` (`modules/pymol/shortcut.py:46-48, 94-101`).  It wipes
...
# `modules/pymol/shortcut_dict.py`), the F-key rows, and the three chord entry
...
MEAS_PDB = os.path.join(REPO, "testing", "data", "1rx1.pdb")

...
``bridge/tests/test_wf_colors.py`` pins the nine ``connect()`` calls of
...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_wf_verifycolors.py -q
"""
...
def test_only_0x_is_an_inline_colour_name_not_css_hash(ws: Any) -> None:
    ...
    ``ColorGetIndex`` parses ``0x``/``0X`` (``layer1/Color.cpp:704-712``) and
...
def test_exactly_which_negative_indices_have_no_tuple(ws: Any) -> None:
    ...
    ``if(index >= 0)``, ``layer4/Cmd.cpp:1336``).  -2 and -3 are the two the

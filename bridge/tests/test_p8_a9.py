...
``docs/webclient/00-parity-inventory.md`` §9, and every claim is an OBSERVATION
...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_p8_a9.py -q
"""
...
def test_auto_measure_is_a_distance_then_an_angle_then_a_dihedral(auto_measure):
    """``EditorAutoMeasure`` (``layer3/Editor.cpp:1765-1782``).
...
def test_a_bond_pick_draws_pkdihe_but_only_after_the_pump_draws(builder):
    """``EditorDrawDihedral`` (``layer3/Editor.cpp:96-141``) runs from
...
def test_a_real_click_deletes_a_BOND_while_the_unbond_wizard_is_armed(builder):
    ...
    declared at ``layer1/ButMode.h:225`` and never bound in ``layer4/Cmd.cpp``;

...
each the gap clause of one PARTIAL row in ``docs/webclient/00-parity-inventory.md``:
...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_p8_a5.py -q
"""
...
CCP4 = REPO / "testing" / "data" / "emd_1155.ccp4"
...
def test_all_48_by_element_entries_actually_recolour(ws: Any) -> None:
    ...
    for color_name in names:
        ...
        # `auto_color` is on (`layer2/AtomInfo.cpp:1340-1347`) — i.e. the auto
...
def test_volume_color_over_the_five_named_ramps_on_a_real_volume(ws: Any) -> None:
    ...
    CCP4 map does work, so this loads `testing/data/emd_1155.ccp4`.
...
SETTING_CPP = REPO / "layer1" / "Setting.cpp"
...
def test_the_session_blacklist_port_matches_setting_cpp() -> None:
    ...
    bool` (`layer1/Setting.cpp:627`) — so the client has to carry the list, and
...
def test_the_live_ramps_section_of_the_c_menu_really_colours(ws: Any) -> None:
    ...
    `layer1/Color.cpp:347`) with index `-10 - slot`, so picking one out of the

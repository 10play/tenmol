...
WHAT ``OrthoReshape`` / ``OrthoLayoutPanel`` DO (``layer1/Ortho.cpp:2261-2456``):
...
(``layer1/Scene.cpp:2885-2905``) draws into ``I->rect`` — the SCENE's
...
#: ``layer1/Ortho.cpp:2267`` — ``controlHeight = DIP2PIXEL(20)``.
CONTROL_H = 20
#: ``ButModeGetHeight`` (``layer1/ButMode.cpp:72-78``), without / with
#: ``mouse_grid``.  Both measured by click in ``test_f7_layout.py``.
BUTMODE_H, BUTMODE_GRID_H = 40, 124

#: ``movie_panel_row_height`` (``layer1/SettingInfo.h:722``).
MOVIE_ROW_H = 15
...
def test_the_movie_panel_is_charged_to_the_window_not_to_the_column(
    ...
    """
    ws = bench
    ws.call("cmd.load", "test/dat/1tii.pdb", "p11l_a")
    ws.call("cmd.load", "test/dat/1tii.pdb", "p11l_b")
    ws.call("cmd.load", "test/dat/1tii.pdb", "p11l_c")
...
def test_the_scene_bin_reserves_zero_column_height(bench: WSClient) -> None:
    ...
    ``SceneDrawButtons`` (``layer1/Scene.cpp:2885``) draws into ``I->rect``, the
...
def test_the_executive_block_is_measured_in_18_px_rows(bench: WSClient) -> None:
    ...
    ``ExecutiveDrawPanel`` (``layer3/Executive.cpp:16192-16221``) sizes the

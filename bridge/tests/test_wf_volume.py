...
(``docs/webclient/00-parity-inventory.md``), whose React plan reads
...
``SettingGenerateSideEffects`` (``layer1/Setting.cpp:1872-2400``) runs after
...
1. **Geometry invalidation.**  Dozens of its cases call
   ...
   measured below: ``_cmd.web_get_versions`` (``layer4/CmdWebGeometry.cpp:2172``)
...
(``grep -rn TOPIC_SETTINGS bridge/tenmol_bridge`` finds only the registry
...
#: ``cRep_t`` (``layer1/Rep.h``) — the two reps this module builds.
REP_STICKS = 0
REP_SPHERES = 1

#: ``layer1/SettingInfo.h`` rows.  Resolved from the live catalogue in
...
def setting_value(ws: Any, index: int) -> Any:
    ...
    ``SettingGetTuple`` (``layer1/Setting.cpp:1445``) returns ``(type, (v,))``
...
def test_the_indices_this_module_uses_are_the_ones_in_settinginfo_h(ws: Any) -> None:
    """Indices are FROZEN (``layer1/SettingInfo.h:5-6``); names are not."""
    ws.do(BOOTSTRAP)
    ...
    for name, index in IDX.items():
        if name == "unused_boolean_def_true":
            ...
            # reports 0 rows at level 'unused' while `layer1/SettingInfo.h`
...
def test_pickable_rebuilds_the_rep_without_changing_a_single_shipped_byte(
    ...
    (``layer1/CGOGL.cpp:696,756,1890``), not while the primitive CGO is built,

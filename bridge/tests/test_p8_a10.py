...
---------------------
Five rows in ``docs/webclient/00-parity-inventory.md`` §10 carry a gap clause
...
#: read, with their ``layer1/SettingInfo.h`` defaults.  They are forced and
...
def test_get_volume_histogram_crosses_the_wire_as_68_inline_floats(ws, volume) -> None:
    ...
    ``bridge/tenmol_bridge/codec.py`` used to list ``get_volume_histogram`` in
...
def test_the_web_fixture_is_this_engines_own_histogram(ws, volume) -> None:
    ...
    # (`layer3/Executive.cpp:4730-4733`) -- which is why `histogramFromField`
...
def test_the_selection_topic_publishes_nothing_when_pk1_moves(ws, picked) -> None:
    ...
    ``grep -rn TOPIC_SELECTION bridge/tenmol_bridge`` finds only the two

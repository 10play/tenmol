...
* **53** Main window shell / dock layout (``modules/pmg_qt/pymol_qt_gui.py:88``)
* **54** External GUI dock: toggle dockable / visible (``:171``)
* **88** Internal GUI layout column (``layer1/Ortho.cpp:2261-2340``)
* **103** Control-bar left gutter: drag-resize + double-click collapse
  (``layer1/Control.cpp:443-478``)
...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_wf_shell.py -q
"""
...
QT_GUI = os.path.join(REPO, "modules", "pmg_qt", "pymol_qt_gui.py")
...
#: ``layer1/Ortho.h:24-26`` — the DIP constants the arithmetic below is made of.
ORTHO_RIGHT_SCENE_MARGIN = 220
ORTHO_BOTTOM_SCENE_MARGIN = 18
ORTHO_LINE_HEIGHT = 12
#: ``layer1/Control.cpp:46`` — NOT a DIP value, a raw 5.
CONTROL_MIN_WIDTH = 5
...
def scene_rect(ws: WSClient, width: int, height: int) -> Tuple[int, int]:
    ...
    (``layer1/Setting.cpp:2824-2834``), and ``viewport`` with no arguments ends
    in ``PyMOL_NeedReshape`` (``layer5/PyMOL.cpp:2511-2536``), which GROWS the
...
def test_the_invocation_options_the_qt_window_sizes_itself_from(
    ...
    ``bridge/tenmol_bridge/engine.py:126-131`` before ``start()``, so on the
    ...
    the full suite: ``bridge/tests/test_modeg_lines.py:41`` sets
    ...
    (``layer1/P.cpp:1800-1830``), so that write changed NOTHING about the
    ...
    # Untouched invocation defaults (modules/pymol/invocation.py).
    ...
    # (`layer1/P.cpp:1820`) is what makes `OrthoFeedbackIn()` queue console text
    # at all (`layer1/Ortho.cpp:492-499`). This line came back, so it queued.
...
def test_ext_hide_and_ext_show_are_PRINTED_no_ops_exactly_as_the_row_says(
    ...
    takes the *window exists* branch (``modules/pymol/gui.py:42-64``) and the
...
def test_internal_gui_reserves_exactly_internal_gui_width_from_the_scene(
    ...
    ``layer1/Ortho.cpp:2391-2394``: ``sceneRight = DIP2PIXEL(internal_gui_width)``
...
def test_internal_gui_mode_2_floats_over_the_scene_and_0_and_1_reserve(
    ...
    ``layer1/Ortho.cpp:2396-2407``: only ``Transparent`` sets ``sceneRight = 0``
...
def test_ortho_reshape_also_reserves_the_bottom_band(
    ...
    (``layer1/Ortho.cpp:2382-2390``).  On an 800x600 window:
    ...
    # MovieGetPanelHeight (layer1/Movie.cpp:1701-1722) returns
...
def test_mouse_grid_changes_the_column_but_nothing_the_wire_can_see(
    ...
    ``ButModeGetHeight`` (``layer1/ButMode.cpp:72-78``) returns DIP2PIXEL(124)
...
def test_cmd_set_does_NOT_clamp_internal_gui_width_so_the_floor_is_the_clients(
    ...
    ``layer1/Control.cpp:263-276`` clamps only the value the gutter drag
...
def test_the_gutter_write_back_round_trips_and_is_inert_while_internal_gui_is_0(
    ...
    (``layer1/Ortho.cpp:2392-2394``).  Measured: 800x600 at every width.
    """
    assert int(ws.call("cmd.get_setting_int", "internal_gui")) == 0
...
def test_a_session_save_carries_internal_gui_width(ws: WSClient, shell_settings, tmp_path) -> None:
    ...
    ``reset_gui`` (``layer1/Setting.cpp:3020-3025``), so a session round trip is

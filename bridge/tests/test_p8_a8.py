...
* **WizardUpdate** -- the parity row claimed there is no server-side equivalent.
  ...
  ``WizardUpdate`` (``layer3/Executive.cpp:11554``), which diffs frame/state and
...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_p8_a8.py -q
"""
...
def test_toggle_wizard_has_no_self_cmd_at_all_and_still_renders(a8):
    ...
    `WizardCallPython` (`layer1/Wizard.cpp:162`).
    """
    ws = a8
...
#: (`layer1/Wizard.cpp:216-224`), so `cmd.refresh_wizard()` is what pushes the
...
def test_the_core_diffs_frame_state_position_and_view_with_no_client_event(a8, gl_bridge):
    ...
    `WizardUpdate` (`layer3/Executive.cpp:11554`), which is the same
    LastUpdatedFrame/State/Position/View diff as in the Qt build
    (`layer1/Wizard.cpp:101-131`).  Nothing in this test sends
...
#: GLUT special codes (`modules/pymol/internal.py:398-421`) and the `_button`
#: state that means "special, not ascii" (`layer5/PyMOL.cpp:2910-2916`).
...
def press(ws, code, mod=0):
    ...
    (`layer5/PyMOL.cpp:2386-2391`) and `internal._special` looks the key up in

...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_f7_legacyfiles.py -q
"""
...
class TestTheGeneratorIsCompiledOut:
    def test_the_console_says_why_while_the_call_reports_success(
        ...
        ``ExecutiveMapGenerate`` (``layer3/Executive.cpp:6929-6935``) prints
...
def reuse_helper(ws, bridge):
    ...
    at ``_cmd._new`` (``layer1/P.cpp:1800-1830``) — nothing about the running
...
class TestPresentationFiles:
    ...
    def test_the_preset_is_the_whole_of_what_the_handler_adds(self, files):
        ...
        ``CmdFullScreen`` (``layer4/Cmd.cpp:5352-5362``) returns an ``ok`` it

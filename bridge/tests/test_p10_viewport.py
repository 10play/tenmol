...
Both are answered here, against the real engine, with no client tick anywhere:
  ...
  * :func:`test_the_engine_idle_loop_sculpts_with_no_client_tick_at_all` —
    ...
    lives (``layer5/PyMOL.cpp:2424``).  Measured drift, printed.
...
Run::
    ...
    bridge/.venv/bin/python -m pytest bridge/tests/test_p10_viewport.py -q
"""
...
def sculpting(bridge):
    ...
    (``layer1/Control.cpp:397-403``), i.e. leaving it on would make every later

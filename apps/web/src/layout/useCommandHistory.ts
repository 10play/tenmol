 ...
 *  - `back_search()` scans forward from `history_cur + 1` for the first entry that
 *    startswith slot 0 (`_gui.py:916-923`) -- bound to Ctrl+Up
 *  - `_jump_history` clamps to `len(history) - 1` (`_gui.py:936`)
 */
export interface CommandHistory {
  /** Push a submitted command. Returns nothing; mirrors doTypedCommand's bookkeeping. */
  ...
  /** Down. */
  forward(): string | null;
  /** Full list, newest first, slot 0 = scratch. For debugging / a future popup. */
...
export function useCommandHistory(): CommandHistory {
  ...
  const back = useCallback(
    (current: string): string | null => {
      if (!cur.current) history.current[0] = current;

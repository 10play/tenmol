  ...
  return (
    ...
    >
      <div className="plugmgr modern:text-pm-text">
        <div
          className="plugmgr__tabs modern:bg-pm-panel-alt modern:border-b modern:border-line"
          role="tablist"
        >
          {TABS.map((t) => (
            <Button
        ...
        {reg.error !== null && (
          <div className="plugmgr__error">plugin scan failed: {reg.error}</div>
        )}
        ...
        {tab === 'installed' && (
          <div className="plugmgr__body">
            <table className="plugmgr__table">
              <thead>
                <tr>
                  <th className="plugmgr__enabled modern:text-pm-text-dim modern:border-line">
                    Enabled
                  </th>
                ...
                {reg.plugins.map((p) => (
                  <tr key={p.filename}>
                    ...
                    <td className="plugmgr__path" title={p.filename}>
                      {p.startupPath === ''
                        ? p.filename
                        : p.filename.slice(p.startupPath.length + 1)}
                    </td>
                  </tr>
                ))}
        ...
        {tab === 'settings' && (
          <div className="plugmgr__body">
            ...
            <p className="plugmgr__note">
              Stored in <code>{PLUGINSRC}</code>, alongside the per-plugin <code>autoload</code>{' '}
              flags and the startup paths — one file carries all three, which is why every write
              here asks first.
            </p>
            <p className="plugmgr__note">
              ...
              <code>instantsave</code> off is therefore the one write that never reaches the file:
              it applies to this session and the file goes on saying <code>True</code>.
            </p>
          </div>
        )}
        ...
        {tab === 'paths' && (
          <div className="plugmgr__body">
            ...
            <p className="plugmgr__note">
              <strong>Installation paths</strong> ({reg.paths.installation.length}) cannot be
              edited: <code>set_startup_path</code> assigns to{' '}
              <code>startup.__path__[:-N_NON_USER_PATHS]</code>, so these survive every edit. They
              are listed here rather than mixed in above, because a delete button that silently did

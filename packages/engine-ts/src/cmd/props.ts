 ...
 *   - atom-level: a Map keyed by the atom's stable identity (object name + PyMOL
 *     atom id, exactly the selector's {@link atomKey}).
 *
...
import { atomKey } from '../select/selector';
...
export function registerProps(ctx: RegistrarCtx): void {
  ...
  /** `${objName}|${atomId}` -> (property name -> value). */
  const atomProps = new Map<string, Map<string, PropValue>>();
  ...
  ctx.command('set_atom_property', (args, kwargs): Json => {
    ...
    for (const ua of matched) {
      const key = atomKey(ua.objName, ua.atom);
      let m = atomProps.get(key);
      if (!m) atomProps.set(key, (m = new Map()));
      m.set(name, value);
    }
  ...
  ctx.command('get_atom_property', (args, kwargs): Json => {
    ...
    for (const ua of ex.atomsMatching(sel)) {
      const m = atomProps.get(atomKey(ua.objName, ua.atom));
      const v = m?.get(name);

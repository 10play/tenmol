 ...
 * Wave 0 baseline: flat 263/390 real, namespaces 8/116, reps 13/21.
 * Bump these up as each backlog wave lands.
 */
const FLOORS = {
  flatReal: Number(process.env.TENMOL_COVERAGE_FLAT ?? 263),
  nsReal: Number(process.env.TENMOL_COVERAGE_NS ?? 8),
...
function flatSurface(): Set<string> {
  ...
  for (const m of src.matchAll(/from\s+\.(\w+)\s+import\s+(.+)/g)) {
    for (const part of m[2]!.split(',')) {
      ...
      const name = tok.split(/\s+as\s+/).pop()!.trim();

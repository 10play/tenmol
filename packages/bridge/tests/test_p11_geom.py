...
(``packages/engine/layer2/ObjectMesh.cpp:45-75``) serialises MapName / Level / Range / Crystal /
Extent / Field and NEVER ``V`` or ``N``, so not even a session pickle carries
the vertices.

THE FIX is an ObjectMesh branch in that same switch (``packages/engine/layer4/
...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_p11_geom.py -q
"""
...
PDB = REPO / "packages" / "engine" / "test" / "dat" / "pept.pdb"
...
def test_it_is_one_uniform_colour_because_nothing_ramps_it(bridge, service, density):
    ...
    # (packages/engine/layer2/ObjectMesh.cpp:470-476), so an unramped isomesh must arrive with
...
def test_isodot_says_so_in_mesh_type_because_lines_would_be_wrong(
    ...
    # cIsomeshMode::isodot == 1 (packages/engine/layer0/PyMOLEnums.h:9-13).  ObjectMesh::render
...
def test_a_width_only_change_bumps_the_version_so_the_client_refetches(
    ...
    MEASURED before the fix (`packages/engine/layer4/CmdWebGeometry.cpp`): mesh_width 1 -> 3 left

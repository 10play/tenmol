...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_p10_infra.py -q
"""
...
def test_cmd_viewport_now_resizes_the_scene_and_the_offscreen_buffer(
    ...
    (``packages/engine/layer4/Cmd.cpp:4968``), which with ``G->HaveGUI`` true only raises
    ``I->ReshapeFlag`` for the host to collect with ``PyMOL_GetReshapeInfo``.
    That accessor is NOT wrapped in Python anywhere (``packages/engine/layer5/PyMOL.h:294,300``
    vs ``packages/engine/modules/pymol2/__init__.py``), so "drain the flag in the pump" — what
...
def test_the_bare_viewport_command_forces_the_reshape_layout_settings_need(
    ...
    (``packages/engine/layer1/Setting.cpp:2824-2833``).  Wave 7 measured what that used to
...
def test_drain_lint_is_clean_with_the_push_in_place() -> None:
    ...
    violations = drain_lint.check([os.path.join(repo, "packages", "bridge", "tenmol_bridge")])

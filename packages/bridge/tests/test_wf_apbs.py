...
``packages/engine/data/startup/apbs_gui/electrostatics.py:156`` is::

    p = subprocess.Popen([exe, infile], cwd=tempdir)

with no ``stdout=``, so the child inherits the **server's** file descriptors.
``pcatch`` (``packages/engine/layer1/P.cpp:2662-2674``) is a Python object installed as
...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_wf_apbs.py -q
...
APBS_UI = os.path.join(REPO, "packages", "engine", "data", "startup", "apbs_gui", "apbs.ui")
...
def test_the_plugin_is_on_the_path_but_has_NOT_been_imported(
    ...
    (``packages/engine/data/startup/apbs_gui/__init__.py:448-450``) — a Qt entry point that
...
def test_the_in_pymol_route_the_panel_recommends_actually_runs(
    ...
    ``packages/engine/modules/pymol/util.py:418-419``), NOT Poisson-Boltzmann — no solvent
...
def test_output_arrives_while_the_child_is_still_running(ws: WSClient, bridge) -> None:
    ...
    ``packages/engine/modules/pymol/internal.py:596-606``).
...
def test_capture_children_rescues_code_that_is_not_ours_to_edit(
    ...
    ``packages/engine/data/startup/``.  Inside ``capture_children()`` that exact call gets a

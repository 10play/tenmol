...
``packages/bridge/tests/test_wf_ortho.py`` already measures the ``progress`` push
...
actually observed to fire:
  ...
  * ``cmd.splash(0)`` doing ``set text, 1`` was read out of
    ``packages/engine/modules/pymol/commanding.py:338-339`` but cannot fire under the bridge as
...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_wf_orthoverify.py -q
"""
...
def test_no_cmd_symbol_mentions_a_loop_and_the_only_rect_is_a_colorection(
    ...
    positive.  So ``I->LoopRect`` (``packages/engine/layer1/SceneMouse.cpp:44-66``) has no
...
def test_the_banner_is_produced_BY_the_call_not_already_in_the_ring(
    ...
    (``packages/engine/modules/pymol/commanding.py:315-352``).
    """
...
def test_the_splash_set_text_1_hazard_only_fires_when_internal_feedback_is_ON(
    ...
    (``packages/engine/modules/pymol/commanding.py:338-339``).  The bridge boots with

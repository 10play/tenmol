...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_f7_startup.py -q
"""
...
def _subprocess(source: str) -> subprocess.CompletedProcess:
    ...
    ``sys.executable`` is ``packages/bridge/.venv/bin/python``, the same interpreter the
...
def test_a_failed_plugin_load_LEAKS_cmd_extend(bridge, ws: WSClient, legacy) -> None:
    ...
    Reported, not fixed: ``packages/engine/modules/pymol/plugins/__init__.py`` is upstream code,
...
def test_call_with_opengl_context_is_LOAD_BEARING_and_here_is_exactly_when(
    ...
    (``packages/engine/layer1/Scene.cpp:2347-2361``) and writes nothing.

...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_files.py -q
"""
...
class TestSaveSurface:
    ...
    def test_multifilenamegen_is_materialised_because_it_is_a_generator(
        ...
        (``packages/engine/modules/pymol/exporting.py:781``), so ``{t:'call'}`` on it returns a
...
class TestMovieProduce:
    ...
    def test_produce_detaches_fd0_so_the_encoder_cannot_eat_it(
        ...
        (``packages/engine/modules/pymol/movie.py:770-800``), so the encoder inherits
...
#: `packages/engine/modules/pymol/importing.py:40-108`, asserted against the running build.
FORMAT_CASES = (
...
def test_copy_image_returns_the_PRIOR_render_as_png_bytes(installed, ws, tmp_path):
    ...
    data = os.path.join(
        ...
        "packages", "engine", "test", "dat", "il2.pdb",
    )

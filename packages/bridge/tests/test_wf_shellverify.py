...
``packages/bridge/tests/test_wf_shell.py`` pins the arithmetic of the internal GUI
...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_wf_shellverify.py -q
"""
...
ENGINE_PY = os.path.join(REPO, "packages", "bridge", "tenmol_bridge", "engine.py")
...
def test_external_gui_is_forced_off_before_start_and_reads_0(ws: WSClient) -> None:
    ...
    * the runtime read — WEAK.  ``pymol.invocation.options`` is a mutable
      process global; ``packages/bridge/tests/test_modeg_lines.py:40-44`` writes four of
      its fields from a module fixture, and PyMOL snapshotted the real values
      into ``CPyMOLOptions`` at ``_cmd._new`` long before that
      (``packages/engine/layer1/P.cpp:1800-1830``).  So a 0 here does NOT prove the engine
      booted with 0.
...
def test_pymol_binds_nothing_to_CTRL_E_so_the_browser_may_swallow_it(ws: WSClient) -> None:
    ...
    ``cmd.key_mappings``, because ``packages/bridge/tests/test_key_bindings.py`` clears
...
def test_set_internal_gui_width_typed_at_the_prompt_moves_the_setting(
    ...
    different code (``packages/engine/modules/pymol/parser.py`` -> ``cmd.set``) and only this

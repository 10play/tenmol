...
Two things were missing and both are fixed here:
  ...
  * ``RepMesh::Width`` was in the accessor's layout mirror but never returned
    (``packages/engine/layer4/CmdWebGeometry.cpp``, ``extractMesh``).  It is now
...
``SceneGetDynamicLineWidth(info, I->Width)`` (``packages/engine/layer2/RepMesh.cpp:535``,
``packages/engine/layer1/CGOGL.cpp:1095`` for the shader path), whose factor depends on the
...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_p11_mesh.py -q
"""
...
def _set_width(bridge, value: float) -> None:
    def body(engine):
        ...
        # The width is read in RepMesh's builder (packages/engine/layer2/RepMesh.cpp:612), so
...
def test_mesh_width_is_the_raw_setting_not_the_dynamic_one(bridge, meshed):
    ...
    ``dynamic_width`` is ON by default (``packages/engine/layer1/SettingInfo.h:710``), so what

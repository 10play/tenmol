...
Run with the venv that has the PyMOL built from this tree::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_seqview.py -q
...
* the click grammar drives *real* PyMOL selections: the same
  ...
  (``packages/engine/layer3/Seeker.cpp:203-221``) writes to the log.
"""
...
PDB_1TII = os.path.join(REPO, "packages", "engine", "test", "dat", "1tii.pdb")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # into "print the full resn" (`packages/engine/layer3/Seeker.cpp:1291-1303`)
        ("A", ""),
        ("DA", ""),
        ("ATP", ""),
        ("", ""),
...
def test_sele_mode_keyword_table_is_the_c_table() -> None:
    # packages/engine/layer1/Scene.cpp:459-467
...
def test_selection_algebra_strings(inc: int, start_over: int, expected: str) -> None:
    """``SeekerSelectionToggle`` (``packages/engine/layer3/Seeker.cpp:203-221``)."""
...
def test_gap_count(mode: int, gap: int, expected: int) -> None:
    """``packages/engine/layer3/Seeker.cpp:1236-1240``: ``resv - last_resv - 1``, SINGLE clamps."""
...
def test_seq_view_off_means_no_row(seq_ws: WSClient) -> None:
    """``packages/engine/layer3/Seeker.cpp:991-993`` — gated per object by the setting."""
...
def test_water_renders_as_O_and_ligands_as_their_resn(seq_ws: WSClient) -> None:
    """``SeekerGetAbbr(..., water='O', unknown=0)`` at ``packages/engine/layer3/Seeker.cpp:1285``."""
    cells = _all_cells(seq_ws)
...
def test_mouse_selection_mode_changes_the_keyword(seq_ws: WSClient) -> None:
    """``mouse_selection_mode`` 2 -> ``bychain`` (``packages/engine/layer1/Scene.cpp:504``)."""
    seq_ws.call("cmd.select", "sele", "none")
...
def test_double_click_outside_clears_the_selection(seq_ws: WSClient) -> None:
    """``packages/engine/layer3/Seeker.cpp:328-341``."""
    row = _row(seq_ws)
...
def test_discrete_state_names_are_coordset_titles_and_stay_selectable(
    ...
    """``codes == 4`` on a DISCRETE object (``packages/engine/layer3/Seeker.cpp:1428-1447``).
    ...
    """
    sdf = os.path.join(REPO, "packages", "engine", "test", "dat", "ligs3d.sdf")

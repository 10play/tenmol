...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_wf_apbsverify.py -q
...
SUBPROC_PY = os.path.join(REPO, "packages", "bridge", "tenmol_bridge", "subproc.py")
GRANT_PY = os.path.join(
    REPO, "packages", "bridge", "tenmol_bridge", "policy", "grants", "wp-25-apbs.py"
)
...
def test_confirm_once_gates_system_AND_execute() -> None:
    ...
    * Wave 11 took the decision to the product owner, who chose "confirm once,
    ...
    permitted by design.  See ``packages/bridge/tests/test_p11_confirm.py``.
    """

...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_notifications.py -q
"""
...
DATA = os.path.join(REPO, "packages", "engine", "test", "dat")
# Relative to the GIT root: the upstream tree lives under `packages/engine/`.
LAYERS = [
    "packages/engine/layer0", "packages/engine/layer1", "packages/engine/layer2",
    "packages/engine/layer3", "packages/engine/layer4", "packages/engine/layer5",
]
...
def test_the_lint_actually_passes_on_this_tree() -> None:
    ...
    problems = drain_lint.check([os.path.join(REPO, "packages", "bridge", "tenmol_bridge")])

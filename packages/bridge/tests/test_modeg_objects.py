...
2. **The unit cell rendered white.**  ``CrystalGetUnitCellCGO`` emits vertices
   ...
   (``packages/engine/layer2/CoordSet.cpp:1281-1291``), i.e. the ``cell_color`` setting or, when
...
Runs against a real PyMOL built from this tree, on the product pump::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_modeg_objects.py -q
"""
...
PDB = REPO / "packages" / "engine" / "testing" / "data" / "1rx1.pdb"

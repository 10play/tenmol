...
``packages/bridge/tests/test_wf_files.py`` is the first agent's file.  This one exists to
...
* **298** — the refusal is CLIENT-SIDE ONLY.  ``cmd.load`` is a plain allowed
  ...
  server-side gate lands in ``packages/bridge/tenmol_bridge/policy/`` this test fails and
  says so.

Run::

    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_wf_verifyfiles.py -q
"""
...
class TestPwgIsReallyExecuted:
    ...
    def test_cmd_load_executes_a_pwg_with_no_confirmation(self, ws):
        ...
        is reading (``packages/engine/modules/pymol/importing.py:597-598``).  With no ``port``
    ...
    def test_the_refusal_is_client_side_only(self, files):
        ...
        server-side gate lands in ``packages/bridge/tenmol_bridge/policy/`` this test

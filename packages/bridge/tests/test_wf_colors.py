...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_wf_colors.py -q
"""
...
QT_GUI_PY = REPO / "packages" / "engine" / "modules" / "pmg_qt" / "pymol_qt_gui.py"
COLORS_UI = REPO / "packages" / "engine" / "modules" / "pmg_qt" / "forms" / "colors.ui"
...
def test_list_colors_is_the_digit_free_names_not_the_whole_table(ws: Any) -> None:
    ...
    Mode 1 keeps ``ColorGetStatus(a) == 1`` (``packages/engine/layer4/Cmd.cpp:1341``), and that
    status is -1 as soon as the name contains a digit
    (``packages/engine/layer1/Color.cpp:784-800``).  178 of the 5388 built-in slots qualify.
    """
...
def test_load_color_resolution_is_ColorGetIndex_not_a_dict_lookup(ws: Any) -> None:
    ...
    ``ColorGetIndex`` (``packages/engine/layer1/Color.cpp:661-748``) tries, in order: a bare
...
def test_get_color_tuple_is_None_for_the_four_negative_keywords(ws: Any) -> None:
    ...
    (``packages/engine/layer4/Cmd.cpp:1336``), so ``get_color_tuple`` hands back None and

...
unset semantics (`packages/engine/modules/pmg_qt/properties_dialog.py:69-286`). The React side
...
Run::
    ...
    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_properties.py -q
"""
...
def test_an_atom_state_setting_writes_through_alter_state(obj):
    ...
    settings (`packages/engine/layer1/SettingInfo.h`, level `astate`), all label geometry.
    """
    sel = "%s and index 1" % M
...
def test_unset_an_atom_state_setting_restores_the_default(obj):
    ...
    # `packages/engine/layer1/SettingInfo.h:830` REC_f(723, label_connector_width, astate, 2.f)

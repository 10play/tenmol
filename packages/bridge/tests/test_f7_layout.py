...
* **209** Setting side effects / geometry invalidation —
  ``SettingGenerateSideEffects``, ``packages/engine/layer1/Setting.cpp:1872-2400``.
...
(``packages/engine/layer1/Setting.cpp:2824-2833``).  That queues the ``viewport`` command, which
...
button table (``packages/engine/modules/pymol/controlling.py:646-680``).

Run::

    packages/bridge/.venv/bin/python -m pytest packages/bridge/tests/test_f7_layout.py -q
"""
...
SETTING_CPP = os.path.join(REPO, "packages", "engine", "layer1", "Setting.cpp")
...
#: ``packages/engine/layer1/Ortho.cpp:2265`` — ``controlHeight = DIP2PIXEL(20)``.
CONTROL_H = 20
#: ``ButModeGetHeight`` (``packages/engine/layer1/ButMode.cpp:72-78``).
BUTMODE_H, BUTMODE_GRID_H = 40, 124
#: ``cButModeLineHeight`` (``packages/engine/layer1/ButMode.cpp:36``) — the ``dy < 2`` split.
BUTMODE_LINE = 12
#: The ``+ 4`` in ``LineHeight * NLine + 4`` (``packages/engine/layer1/Wizard.cpp:255``).
#: NOT DIP-scaled, unlike the ``LineHeight`` it is added to.
WIZARD_PAD = 4

#: Settings whose only side effect is ``OrthoCommandIn(G, "viewport")``
#: (``packages/engine/layer1/Setting.cpp:2824-2833``).  Asserted against the C++ below.
LAYOUT_SETTINGS = (
    "internal_gui_mode",
    "internal_gui_width",
    "internal_gui",
...
def probe_column(ws: WSClient, y: int) -> str:
    ...
    no-op.  ``CExecutive::click`` (``packages/engine/layer3/Executive.cpp:15050,15302``) treats
...
def test_the_control_band_is_20_px_read_off_the_butmode_line_split(
    ...
    (``packages/engine/layer1/ButMode.cpp:151-186``).  So the y at which the answer flips is
    ...
    (``packages/engine/layer1/Control.cpp:466``), while its buttons start and stop the movie.
...
def test_the_wizard_block_is_control_size_times_nline_plus_4(
    ...
    (``packages/engine/layer1/Wizard.cpp:253-259``).  Measured on an 800x600 window: the pixels
    ...
    (``packages/engine/layer1/Ortho.cpp:518-527``), so the cached ortho CGO is not rebuilt and
    ...
    - 0) / LineHeight`` (``packages/engine/layer1/Wizard.cpp:487``).
    """
    ws = gui_on
    ws.do(WIZ_BOOTSTRAP)
    ws.call("cmd.set", "mouse_grid", 0)
...
def test_a_layout_setting_relayouts_immediately_now(
    ...
    which ends in ``PyMOL_NeedReshape`` (``packages/engine/layer5/PyMOL.cpp:2511-2536``) — a

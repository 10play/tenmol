...
* row 203 — setting defaults / min-max / help.  The data was already correct;
  ...
  installed bridge (no ``packages/engine/layer1/`` next to it) had no defaults at all.
...
def test_the_asset_carries_the_whole_c_table_and_the_whole_csv() -> None:
    ...
    assert document["settingInfoOrigin"] == "packages/engine/layer1/SettingInfo.h"
    assert document["helpOrigin"] == "packages/engine/data/setting_help.csv"
...
def test_an_installed_layout_has_defaults_only_because_of_the_asset(
    ...
    An installed PyMOL ships no ``packages/engine/layer1/`` — headers are not in a wheel — so
    ...
    assert restored["meta"]["defaultsSource"] == "packages/engine/layer1/SettingInfo.h"
...
def test_a_stale_asset_still_loses_to_the_header_on_a_source_checkout(
    ...
    assert built["meta"]["defaultsSource"] == "packages/engine/layer1/SettingInfo.h"
...
def test_help_survives_a_layout_with_no_csv_next_to_the_bridge(
    ...
    meta = built["meta"]
    assert meta["helpSource"] == "packages/engine/data/setting_help.csv"
...
def test_scene_menu_is_reachable_with_a_null_self_cmd(ws: Any) -> None:
    ...
    (`packages/engine/modules/pymol/menu.py:1842`) never touches its `self_cmd` argument — it
...
def test_stereo_modes_against_a_real_gl_context(ws: Any, bridge: Any) -> None:
    ...
    try:
        ...
        #    `stereo_mode` values are `packages/engine/layer1/SettingInfo.h`'s: 2 crosseye,

...
Implements `docs/03-implementation-plan.md` §1.1 (the pump), §1.2
...
in this package has a file:line citation into `layer*/` or `packages/engine/modules/` next to it.
...
`drag` and `release` all go through `OrthoDefer` (`packages/engine/layer1/Scene.cpp:4113-4155`),
as do deferred `cmd.png` and deferred `cmd.ray`. The queue is drained by
`OrthoExecDeferred` (`packages/engine/layer1/Ortho.cpp:268-277`), whose only caller is
`ExecutiveDrawNow` (`packages/engine/layer3/Executive.cpp:11521-11523`) — and that call is gated
on `PyMOL_GetIdleAndReady`, i.e. `IdleAndReady == 3` (`packages/engine/layer5/PyMOL.cpp:105`,
...
click after `I->SingleClickDelay = 0.15` (`packages/engine/layer1/SceneMouse.cpp:1152`).

### 2. `no_gui = 0`, `SingletonPyMOL`, `pcatch` — and never `-c`.

`OrthoFeedbackIn()` is gated on `G->Option->pmgui` (`packages/engine/layer1/Ortho.cpp:492-499`),
which is `!options.no_gui` (`packages/engine/layer1/P.cpp:1820`). `pymol -c` sets `no_gui=1`
(`packages/engine/modules/pymol/invocation.py:401`) and the feedback queue is then dead for the
...
(`packages/engine/layer1/P.cpp:2667`): with a non-singleton `pymol2.PyMOL()` that pointer is
...
import time. `packages/bridge/tests/test_process_model.py` asserts no bridge log line
...
(`packages/engine/modules/pymol/internal.py:596-606`). `get_setting_updates()` returning `[]` on
a lock miss is indistinguishable from "nothing changed"
(`packages/engine/modules/pymol/setting.py:440-447`) — never build quiescence detection on it.
...
inside `PyMOL_Draw` (`PyMOL_PushValidContext`, `packages/engine/layer5/PyMOL.cpp:2940-2949`,
...
`packages/engine/layer4/Cmd.cpp:6379-6380`); with it, the same two calls produce a 20,428-byte
...
other way round. `packages/engine/modules/pymol/viewing.py:731` slices the C accessor's 25 down
...
2. **Never regenerate the FBO on resize.** `check_gl_stereo_capable` latches
   ...
   the first draw (`packages/engine/layer5/PyMOL.cpp:2236-2239`). `resize()` re-storages the

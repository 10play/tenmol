...
2. **The draw-arrays block is rebuilt VERBATIM.**  The accessor hands back the
   ...
   original consecutive layout of ``packages/engine/layer1/CGO.cpp:1650-1671``:
...
(``packages/engine/layer4/CmdWebGeometry.cpp:2145``) returns a monotonic version and an
...
#: ``enum cRep_t``, ``packages/engine/layer1/Rep.h:48-74``.  Mirrors ``REP_NAMES`` in
...
#: built from this source (``packages/bridge/tests/test_modeg_objects.py`` is the
...
#:                   nothing in ``packages/engine/test/dat`` carries one.  NOT claimed here
...
class RepInv:
    """``cRepInv_t``, ``packages/engine/layer1/Rep.h:133-184``.  Mirrors ``RepInv`` in the TS."""
...
#: ``CGOArrayBit``, ``packages/engine/layer1/CGO.h:272-277``.
BIT_VERTEX = 0x01
BIT_NORMAL = 0x02
BIT_COLOR = 0x04
BIT_PICK = 0x08
...
def _concat_f32(parts: Sequence[Tuple[Any, int]], nverts: int) -> bytes:
    ...
    ``packages/engine/layer1/CGO.cpp:1650-1671`` and ``cgoArraysLayout()`` in ``geometry.ts``:
    ``[vertex 3N][normal 3N]?[color 4N]?[pickRGBA 1N + pickIndex 2N]?[access N]?``
...
class GeometryService:
        ...
        if not self._accessor_checked:
            ...
            if self._accessor is None:
                log(
                    ...
                    "_cmd.web_get_rep_geometry (packages/engine/layer4/CmdWebGeometry.cpp); "
        ...
        if not self._versions_checked:
            ...
            if self._versions_fn is None:
                log(
                    ...
                    "_cmd.web_get_versions (packages/engine/layer4/CmdWebGeometry.cpp); falling "
    ...
    def fetch(
        ...
        # `ResolveCellColor` (`packages/engine/layer2/CoordSet.cpp:1281-1291,1412-1416`), which
    ...
    def _ellipsoids(
        ...
        # is NOT redundant: `CGOSimpleEllipsoid` (packages/engine/layer1/CGO.cpp:6355-6376)

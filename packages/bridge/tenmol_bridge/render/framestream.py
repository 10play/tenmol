...
---------------------------------------------------------------------
``PyMOL_Draw`` sets ``I->RedisplayFlag = false`` at ``packages/engine/layer5/PyMOL.cpp:2331``
...
#: ``cRepCnt``, ``packages/engine/layer1/Rep.h:73``.  Mirrored, not imported, so ``encode.py``'s
...
class CoverageProbe:
    ...
    * ``cmd.get_vis()`` — 0.001 ms — gives the enabled flag and the OBJECT-level
      ``visRep`` (``ExecutiveGetVisAsPyDict``, ``packages/engine/layer3/Executive.cpp:4496``),
...
class RedisplayGate:
    ...
    def attach(self, pump: Any) -> str:
        ...
        log(
            ...
            "at packages/engine/layer5/PyMOL.cpp:2331, so a post-draw probe is always False)"
        )
        return self.mode
...
class FrameStream:
    ...
    def ray_now(
        ...
        **kwargs: Any,
        ...
        whenever the filename is empty (``packages/engine/layer4/Cmd.cpp:4830-4834``), and
        ``prior=1`` takes the fast path that never touches
        ``_call_with_opengl_context`` (``packages/engine/modules/pymol/exporting.py:588-593``).

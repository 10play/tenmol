...
``cmd._copy_image``           default raises ``NotImplementedError``
                              (``packages/engine/modules/pymol/internal.py:272-274``, "may be
                              monkey-patched by GUI implementations"); it is
                              what ``cmd.copy_image`` calls
                              (``packages/engine/modules/pymol/exporting.py:36``)
``cmd._call_in_gui_thread``   default ``lambda func: func()``
                              (``packages/engine/modules/pymol/cmd.py:164``) — correct only
...
``window_cmd``                ``cmd.window()`` routes to
                              ...
                              object exists (``packages/engine/modules/pymol/viewing.py:1449-1453``);
...
``packages/engine/layer5/PyMOL.cpp:2940-2949``, called at ``:2281`` in the ``ModalDraw`` branch
...
* ``SceneMakeMovieImage``'s ``cSceneImage_Normal`` branch
  (``packages/engine/layer1/Scene.cpp:2347-2361``) — so ``cmd.png()`` **with no explicit size**
...
The fix is not the Qt shim.  ``packages/engine/modules/pmg_qt/pymol_gl_widget.py:51-62`` is
...
``packages/engine/layer4/Cmd.cpp:6379-6380`` — and it is exactly what is missing.  So this
module installs::
...
class BridgeWindow:
    """Duck-types just enough of ``pmg_qt``'s window for ``packages/engine/modules/pymol``.
...
class Shims:
    ...
    def _copy_image(self, _self: Any = None, quiet: int = 1) -> Any:
        ...
        (``packages/engine/modules/pymol/exporting.py:36``), which passes ``_self``
        positionally.
        """
    ...
    def _call_in_gui_thread(self, func: Callable[[], Any]) -> Any:
        ...
        (``packages/engine/layer5/PyMOL.cpp:2307``).
        """
        pump = self.pump
    ...
    def _call_with_opengl_context(self, func: Callable[[], Any]) -> Any:
        ...
        (``packages/engine/layer4/Cmd.cpp:6379-6380``); they take the raw ``_COb`` handle and do

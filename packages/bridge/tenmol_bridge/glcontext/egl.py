...
(``packages/engine/layer1/ScenePicking.cpp:283,306``) and ``glShadeModel`` (``:234,272``), and
...
PyMOL itself calls: ``packages/engine/layer0/ShaderMgr.cpp:1829-1831`` and
``packages/engine/layer1/ScenePicking.cpp:64-81`` use ``glBindFramebuffer`` /
...
``packages/engine/test/dat/1tii.pdb`` (5,684 atoms, cartoon) rendered a non-blank image through
...
class GLFramebuffer:
    ...
    (``packages/engine/layer5/PyMOL.cpp:2236-2239``).  Spike 04 §4 verified this on macOS.
    """
    ...
    def _create(self, allow_gles: bool) -> None:
        ...
        if not desktop:
            message = (
                ...
                "(packages/engine/layer1/ScenePicking.cpp:283,306,234,272) and links -lGL "
        ...
        if gl.fbo_suffix == "EXT":
            self.warnings.append(
                ...
                "the ARB entry points (packages/engine/layer0/ShaderMgr.cpp:1829), which may "

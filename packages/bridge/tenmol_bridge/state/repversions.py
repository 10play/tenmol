...
Wave 2 landed ``_cmd.web_get_versions`` (``packages/engine/layer4/CmdWebGeometry.cpp:2145``),
...
this tree (``packages/bridge/.venv``, 1UBQ, no GL context):
...
#: ``cRepInv_t`` (``packages/engine/layer1/Rep.h:133-184``), mirrored by ``RepInv`` in
...
def build_table(
    ...
    ``packages/engine/layer4/CmdWebGeometry.cpp:322-350``) onto ``cRep_t`` indices; it is passed

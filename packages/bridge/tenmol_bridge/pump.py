...
(``packages/engine/modules/pymol/monitoring.py:5-7`` ``lock_api_status``;
``packages/engine/modules/pymol/internal.py:596-606`` and ``packages/engine/modules/pymol/setting.py:440-447``
use ``lock_attempt`` = ``acquire(blocking=0)``, ``packages/engine/modules/pymol/locking.py:29-30``).
...
``std::queue<std::string>`` (``packages/engine/layer1/Ortho.cpp:492-499``; 20,000 undrained
...
class Task:
    ...
    #: (``packages/engine/layer1/Ortho.cpp:268-277``, ``packages/engine/layer3/Executive.cpp:11521-11523``).
...
class Pump:
    ...
    def pump_for(self, seconds: float) -> None:
        ...
        consumed at ``packages/engine/layer1/Scene.cpp:2441``) and in tests.  The engine thread
    ...
    def add_pre_tick_hook(self, hook: Callable[[Engine], None]) -> None:
        ...
        ``I->RedisplayFlag = false`` at ``packages/engine/layer5/PyMOL.cpp:2331`` *before*
...
class StatusPoller:
    ...
    ``packages/engine/layer1/Ortho.cpp:62`` with no Python route).  ``sinks`` receive
    ...
    def poll_once(self) -> int:
        ...
        #    (packages/engine/modules/pymol/internal.py:596-606).  Treating None as empty
        ...
        #    returns -1.0 when idle (packages/engine/modules/pymol/monitoring.py:5-7).
        try:
        ...
        #    (packages/engine/modules/pymol/setting.py:440-447), so never build quiescence
        #    detection on it.
        try:

...
(``packages/engine/layer2/CoordSet.cpp:326-361``) — measured as a ``(4779,3)`` float32 view.  A
...
#: chempy ``Atom`` fields we put on the wire (``packages/engine/modules/chempy/__init__.py``).
...
def encode(value: Any, _depth: int = 0) -> Any:
    ...
    raise NotSerializable(
        "no codec entry for %s.%s; add it to packages/bridge/tenmol_bridge/codec.py "

"""The molecular Builder — a Qt-free port of ``packages/engine/modules/pmg_qt/builder.py``.
...
---------------------------------------------------
``packages/bridge/tenmol_bridge/panels/__init__.py`` is a frozen barrel that does not
...
* ``clean`` — ``cmd.clean`` raises ``IncentiveOnlyException`` in this tree
  (``packages/engine/modules/pymol/computing.py:20-29``).  :func:`builder_state` reports
...
#: ``packages/engine/layer3/Editor.h:30-48`` — the picked-atom slots, in click order.
...
# ``packages/engine/data/chempy/fragments`` is caught by a test rather than by a user.

#: label, tooltip, symbol, geometry, valence, wizard text.  Geometry codes are
#: ``packages/engine/layer2/AtomInfo.h:129-133``: 1=Single 2=Linear 3=Planar 4=Tetrahedral 5=None
...
def fragment_names() -> List[str]:
    """Every ``packages/engine/data/chempy/fragments/<name>.pkl`` the Chemical tab can request."""
...
def editor_state(cmd: Any) -> Dict[str, Any]:
    ...
    ``pkbond`` is the C++ editor's bond selection (``packages/engine/layer3/Editor.h``); its
...
def wizard_state(cmd: Any) -> Optional[Dict[str, Any]]:
    ...
    (``packages/engine/modules/pymol/wizard/__init__.py:50``).  The strings are preserved
...
class BioPolymerWizard(RepeatableActionWizard):
    ...
    ``packages/engine/modules/pymol/editor.py`` (grep-verified).  Upstream never reaches them
...
class _Panel:
    ...
    def set_undo_enabled(self, enabled: bool) -> List[str]:
        ...
        invents none (defect 6 in ``docs/builder.md`` §9).
        """
...
def builder_pick(
    ...
    """A viewport pick, routed exactly like ``packages/engine/layer1/SceneMouse.cpp:404-470``.
...
def builder_select(cmd: Any, selection: str) -> Dict[str, Any]:
    """``WizardDoSelect`` (``packages/engine/layer1/Wizard.cpp:172-190``) for the web client.

    The engine fires ``wizard.do_select(name)`` ONLY from its own mouse paths —
    ``packages/engine/layer1/SceneMouse.cpp:135,357`` (a click that lands in a selection),
    ``packages/engine/layer3/Seeker.cpp:150,231`` (the sequence viewer) and
    ``packages/engine/layer3/Executive.cpp:7563`` (a box/rect select).  It is never fired by
...
def builder_sculpt_tick(cmd: Any, cycles: Any = None) -> Dict[str, Any]:
    ...
    ``ControlIdling(G)`` is true (``packages/engine/layer5/PyMOL.cpp:2424``), and
    ``ControlIdling`` is true whenever the ``sculpting`` SETTING is on
    (``packages/engine/layer1/Control.cpp:397-403``).  That loop walks every molecular object
    whose per-object ``sculpting`` is set and runs ``sculpting_cycles``
    iterations of it at the CURRENT state (``packages/engine/layer3/Executive.cpp:7106-7135``).

    THERE IS AN IDLE LOOP HERE, and wave 9 said there was not.  The pump's tick
    calls ``PyMOL_Idle`` (``packages/bridge/tenmol_bridge/engine.py:236``), so the engine
    ...
    (``packages/bridge/tests/test_p10_viewport.py``).

...
literal returned by ``PyMOLDesktopGUI.get_menudata`` (``packages/engine/modules/pymol/_gui.py:55``)
and every front end walks that same literal: Qt in ``_addmenu``
(``packages/engine/modules/pmg_qt/pymol_qt_gui.py:295-345``), Tk in
``packages/engine/modules/pmg_tk/skins/normal/__init__.py:1072``.  Transcribing ~700 leaves into
...
* ``packages/bridge/tests/test_menus.py`` asserts the shape against the real upstream
...
#: come from PyMOL's key map, ``packages/engine/modules/pymol/keyboard.py``), but the client
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> "_Recorder":
        ...
        for value in args:
            if isinstance(value, _Recorder):
                ...
                # `_self` argument (`packages/engine/modules/pymol/util.py:553`).  Over the wire
...
def settings(cmd: Any, names: Optional[Sequence[str]] = None) -> Dict[str, Any]:
    ...
    ``cmd.get_setting_tuple`` (``packages/engine/modules/pymol/setting.py:413``) returns

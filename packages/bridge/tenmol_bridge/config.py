...
**and** ``sys.stderr`` (``packages/engine/modules/pmg_qt/pymol_gl_widget.py:99-105``; spike 02
...
def log(message: str) -> None:
    ...
    ``exit()`` (``packages/engine/layer5/main.cpp``; spike 00 §6.2), skipping ``atexit`` and
...
class BridgeConfig:
    ...
    #: single-click floor (``I->SingleClickDelay``, packages/engine/layer1/SceneMouse.cpp:1152).
    tick_hz: float = 60.0
    #: the 10 Hz lock-ATTEMPTING status thread (plan §1.1 threading table)
    status_hz: float = 10.0
    #: >= 3 (IDLE_AND_READY == 3, packages/engine/layer5/PyMOL.cpp:105). 5 for margin.
    warmup_draws: int = 5

...
Pick up the next work package from docs/webclient/03-implementation-plan.md section 6 and implement it. Respect the file-ownership boundaries there; run 'pnpm ownership' if unsure who owns a file.

Before finishing a package: pnpm typecheck && pnpm build && pnpm lint, plus the bridge suite (bridge/.venv/bin/python -m pytest bridge/tests -q) and pnpm test. Tick the rows you genuinely verified in docs/webclient/00-parity-inventory.md so this check advances -- do not tick rows you did not test.
